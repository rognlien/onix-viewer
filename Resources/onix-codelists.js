// onix-codelists.js — AUTO-GENERATED. Do not edit by hand.
//
// Generated by tools/generate-codelists.js from the EDItEUR ONIX 3.1
// code-lists JSON (issue 74). Each list is a Map<code, label> in
// EDItEUR-defined order; iteration order is preserved across all consumers.
//
// Several elements share a list (e.g. multiple text-type or scheme-id
// elements). They reference the same Map instance via _lists below.

(function () {
  const _lists = Object.create(null);

  // List 1 — Notification or update type
  _lists[1] = new Map([
    ["01", "Early notification"],
    ["02", "Advance notification (confirmed)"],
    ["03", "Notification confirmed on publication"],
    ["04", "Update (partial)"],
    ["05", "Delete"],
    ["08", "Notice of sale"],
    ["09", "Notice of acquisition"],
    ["88", "Test update (partial)"],
    ["89", "Test record"],
  ]);

  // List 2 — Product composition
  _lists[2] = new Map([
    ["00", "Single-component retail product"],
    ["01", "Single-component, not available separately"],
    ["10", "Multiple-component retail product"],
    ["11", "Multiple-item collection, retailed as separate parts"],
    ["20", "Trade-only product"],
    ["30", "Multiple-item trade-only pack"],
    ["31", "Multiple-item pack"],
  ]);

  // List 3 — Record source type
  _lists[3] = new Map([
    ["00", "Unspecified"],
    ["01", "Publisher"],
    ["02", "Publisher’s distributor"],
    ["03", "Wholesaler"],
    ["04", "Bibliographic agency"],
    ["05", "Library bookseller"],
    ["06", "Publisher’s sales agent"],
    ["07", "Publisher’s conversion service provider"],
    ["08", "Conversion service provider"],
    ["09", "ISBN Registration Agency"],
    ["10", "ISTC Registration Agency"],
    ["11", "Retail bookseller"],
    ["12", "Education bookseller"],
    ["13", "Library"],
  ]);

  // List 5 — Product identifier type
  _lists[5] = new Map([
    ["01", "Proprietary product ID scheme"],
    ["02", "ISBN-10"],
    ["03", "GTIN-13"],
    ["04", "UPC"],
    ["05", "ISMN-10"],
    ["06", "DOI"],
    ["13", "LCCN"],
    ["14", "GTIN-14"],
    ["15", "ISBN-13"],
    ["17", "Legal deposit number"],
    ["22", "URN"],
    ["23", "OCLC number"],
    ["24", "Co-publisher’s ISBN-13"],
    ["25", "ISMN-13"],
    ["26", "ISBN-A"],
    ["27", "JP e-code"],
    ["28", "OLCC number"],
    ["29", "JP Magazine ID"],
    ["30", "UPC-12+5"],
    ["31", "BNF Control number"],
    ["34", "ISSN-13"],
    ["35", "ARK"],
    ["36", "Digital file internal version number"],
  ]);

  // List 9 — Product classification type
  _lists[9] = new Map([
    ["01", "WCO Harmonized System"],
    ["02", "UNSPSC"],
    ["03", "HMRC"],
    ["04", "Warenverzeichnis für die Außenhandelsstatistik"],
    ["05", "TARIC"],
    ["06", "Fondsgroep"],
    ["07", "Sender’s product category"],
    ["08", "GAPP Product Class"],
    ["09", "CPA"],
    ["10", "NCM"],
    ["11", "CPV"],
    ["12", "PKWiU"],
    ["13", "HTSUS"],
    ["14", "US Schedule B"],
    ["15", "Clave SAT"],
    ["16", "CN (EU Combined Nomenclature)"],
    ["17", "CCT"],
    ["18", "CACT"],
    ["19", "NICO"],
    ["20", "TARIC additional code"],
    ["21", "HTSUS additional code"],
    ["22", "CPPAP"],
    ["50", "Electre genre"],
  ]);

  // List 12 — Trade category
  _lists[12] = new Map([
    ["01", "UK open market edition"],
    ["02", "Airport edition"],
    ["03", "Sonderausgabe"],
    ["04", "Pocket book"],
    ["05", "International edition (US)"],
    ["06", "Library audio edition"],
    ["07", "US open market edition"],
    ["08", "Livre scolaire, déclaré par l’éditeur"],
    ["09", "Livre scolaire (non spécifié)"],
    ["10", "Supplement to newspaper"],
    ["11", "Precio libre textbook"],
    ["12", "News outlet edition"],
    ["13", "US textbook"],
    ["14", "E-book short"],
    ["15", "Superpocket book"],
    ["16", "Beau-livre"],
    ["17", "Podcast"],
    ["18", "Periodical"],
    ["19", "Catalog"],
    ["20", "Atlas"],
    ["21", "Newspaper"],
  ]);

  // List 13 — Collection identifier type
  _lists[13] = new Map([
    ["01", "Proprietary collection ID scheme"],
    ["02", "ISSN"],
    ["03", "German National Bibliography series ID"],
    ["04", "German Books in Print series ID"],
    ["05", "Electre series ID"],
    ["06", "DOI"],
    ["15", "ISBN-13"],
    ["22", "URN"],
    ["27", "JP Magazine ID"],
    ["29", "BNF Control number"],
    ["35", "ARK"],
    ["38", "ISSN-L"],
  ]);

  // List 14 — Text case flag
  _lists[14] = new Map([
    ["00", "Undefined"],
    ["01", "Sentence case"],
    ["02", "Title case"],
    ["03", "All capitals"],
  ]);

  // List 15 — Title type
  _lists[15] = new Map([
    ["00", "Undefined"],
    ["01", "Distinctive title (book); Cover title (serial); Title of content item, collection, or resource"],
    ["02", "ISSN key title of serial"],
    ["03", "Title in original language"],
    ["04", "Title acronym or initialism"],
    ["05", "Abbreviated title"],
    ["06", "Title in other language"],
    ["07", "Thematic title of journal issue"],
    ["08", "Former title"],
    ["10", "Distributor’s title"],
    ["11", "Alternative title on cover"],
    ["12", "Alternative title on back"],
    ["13", "Expanded title"],
    ["14", "Alternative title"],
    ["15", "Alternative title on spine"],
    ["16", "Translated from title"],
  ]);

  // List 16 — Work identifier type
  _lists[16] = new Map([
    ["01", "Proprietary work ID scheme"],
    ["02", "ISBN-10"],
    ["06", "DOI"],
    ["11", "ISTC"],
    ["15", "ISBN-13"],
    ["18", "ISRC"],
    ["19", "ISAN"],
    ["31", "EIDR Content ID"],
    ["32", "GLIMIR"],
    ["33", "OWI"],
    ["39", "ISCC"],
  ]);

  // List 17 — Contributor role code
  _lists[17] = new Map([
    ["A01", "By (author)"],
    ["A02", "With"],
    ["A03", "Screenplay by"],
    ["A04", "Libretto by"],
    ["A05", "Lyrics by"],
    ["A06", "By (composer)"],
    ["A07", "By (artist)"],
    ["A08", "By (photographer)"],
    ["A09", "Created by"],
    ["A10", "From an idea by"],
    ["A11", "Designed by"],
    ["A12", "Illustrated by"],
    ["A13", "Photographs by"],
    ["A14", "Text by"],
    ["A15", "Preface by"],
    ["A16", "Prologue by"],
    ["A17", "Summary by"],
    ["A18", "Supplement by"],
    ["A19", "Afterword by"],
    ["A20", "Notes by"],
    ["A21", "Commentaries by"],
    ["A22", "Epilogue by"],
    ["A23", "Foreword by"],
    ["A24", "Introduction by"],
    ["A25", "Footnotes by"],
    ["A26", "Memoir by"],
    ["A27", "Experiments by"],
    ["A28", "Interpreted through"],
    ["A29", "Introduction and notes by"],
    ["A30", "Software written by"],
    ["A31", "Book and lyrics by"],
    ["A32", "Contributions by"],
    ["A33", "Appendix by"],
    ["A34", "Index by"],
    ["A35", "Drawings by"],
    ["A36", "Cover design or artwork by"],
    ["A37", "Preliminary work by"],
    ["A38", "Original author"],
    ["A39", "Maps by"],
    ["A40", "Inked or colored by"],
    ["A41", "Paper engineering by"],
    ["A42", "Continued by"],
    ["A43", "Interviewer"],
    ["A44", "Interviewee"],
    ["A45", "Comic script by"],
    ["A46", "Inker"],
    ["A47", "Colorist"],
    ["A48", "Letterer"],
    ["A49", "Cover inker"],
    ["A50", "Cover colorist"],
    ["A51", "Research by"],
    ["A52", "Original character design"],
    ["A56", "Cover graphic design by"],
    ["A99", "Other primary creator"],
    ["B01", "Edited by"],
    ["B02", "Revised by"],
    ["B03", "Retold by"],
    ["B04", "Abridged by"],
    ["B05", "Adapted by"],
    ["B06", "Translated by"],
    ["B07", "As told by"],
    ["B08", "Translated with commentary by"],
    ["B09", "Series edited by"],
    ["B10", "Edited and translated by"],
    ["B11", "Editor-in-chief"],
    ["B12", "Guest editor"],
    ["B13", "Volume editor"],
    ["B14", "Editorial board member"],
    ["B15", "Editorial coordination by"],
    ["B16", "Managing editor"],
    ["B17", "Founded by"],
    ["B18", "Prepared for publication by"],
    ["B19", "Associate editor"],
    ["B20", "Consultant editor"],
    ["B21", "General editor"],
    ["B22", "Dramatized by"],
    ["B23", "General rapporteur"],
    ["B24", "Literary editor"],
    ["B25", "Arranged by (music)"],
    ["B26", "Technical editor"],
    ["B27", "Thesis advisor or supervisor"],
    ["B28", "Thesis examiner"],
    ["B29", "Scientific editor"],
    ["B30", "Historical advisor"],
    ["B31", "Original editor"],
    ["B32", "Translation revised by"],
    ["B33", "Transcribed by"],
    ["B34", "Sensitivity reader / editor"],
    ["B35", "Image descriptions by"],
    ["B36", "Text modernized or updated by"],
    ["B99", "Other adaptation by"],
    ["C01", "Compiled by"],
    ["C02", "Selected by"],
    ["C03", "Non-text material selected by"],
    ["C04", "Curated by"],
    ["C99", "Other compilation by"],
    ["D01", "Producer"],
    ["D02", "Director"],
    ["D03", "Conductor"],
    ["D04", "Choreographer"],
    ["D99", "Other direction by"],
    ["E01", "Actor"],
    ["E02", "Dancer"],
    ["E03", "Narrator"],
    ["E04", "Commentator"],
    ["E05", "Vocal soloist"],
    ["E06", "Instrumental soloist"],
    ["E07", "Read by"],
    ["E08", "Performed by (orchestra, band, ensemble)"],
    ["E09", "Speaker"],
    ["E10", "Presenter"],
    ["E11", "Introduction read by"],
    ["E99", "Performed by"],
    ["F01", "Filmed / photographed by"],
    ["F02", "Editor (film or video)"],
    ["F99", "Other recording by"],
    ["Z01", "Assisted by"],
    ["Z02", "Honored / dedicated to"],
    ["Z03", "Enacting jurisdiction"],
    ["Z04", "Peer reviewed"],
    ["Z05", "Posthumously completed by"],
    ["Z06", "In association with"],
    ["Z98", "(Various roles)"],
    ["Z99", "Other"],
  ]);

  // List 18 — Person / organization name type
  _lists[18] = new Map([
    ["00", "Unspecified"],
    ["01", "Pseudonym"],
    ["02", "Authority-controlled name"],
    ["03", "Earlier name"],
    ["04", "‘Real’ name"],
    ["05", "Transliterated / translated form of primary name"],
    ["06", "Later name"],
    ["07", "Fictional character name"],
    ["08", "Acronym / initialism"],
    ["09", "Professional name"],
  ]);

  // List 19 — Unnamed person(s)
  _lists[19] = new Map([
    ["01", "Unknown"],
    ["02", "Anonymous"],
    ["03", "et al"],
    ["04", "Various"],
    ["05", "Synthesized voice – male"],
    ["06", "Synthesized voice – female"],
    ["07", "Synthesized voice – unspecified"],
    ["08", "Synthesized voice – based on real voice actor"],
    ["09", "AI (Artificial intelligence)"],
  ]);

  // List 20 — Event role
  _lists[20] = new Map([
    ["01", "Publication linked to conference"],
    ["02", "Complete proceedings of conference"],
    ["03", "Selected papers from conference"],
    ["11", "Publication linked to sporting event"],
    ["12", "Programme or guide for sporting event"],
    ["21", "Publication linked to artistic event"],
    ["22", "Programme or guide for artistic event"],
    ["31", "Publication linked to exposition"],
    ["32", "Programme or guide for exposition"],
  ]);

  // List 21 — Edition type
  _lists[21] = new Map([
    ["ABR", "Abridged edition"],
    ["ACT", "Acting edition"],
    ["ADP", "Adapted edition"],
    ["ALT", "Alternate"],
    ["ANN", "Annotated edition"],
    ["AVS", "Anniversary edition"],
    ["BLL", "Bilingual edition"],
    ["BLP", "Bilingual ‘facing page’ edition"],
    ["BRL", "Braille edition"],
    ["BUD", "Budget edition"],
    ["CMB", "Combined volume"],
    ["CRI", "Critical edition"],
    ["CSP", "Coursepack"],
    ["DGO", "Digital original"],
    ["ENH", "Enhanced edition"],
    ["ENL", "Enlarged edition"],
    ["ETR", "Easy-to-read edition"],
    ["EXP", "Expurgated edition"],
    ["FAC", "Facsimile edition"],
    ["FST", "Festschrift"],
    ["HRE", "High readability edition"],
    ["ILL", "Illustrated edition"],
    ["INT", "International edition"],
    ["LTE", "Large type / large print edition"],
    ["MCP", "Microprint edition"],
    ["MDT", "Media tie-in"],
    ["MLL", "Multilingual edition"],
    ["NED", "New edition"],
    ["NUM", "Edition with numbered copies"],
    ["PBO", "Paperback original"],
    ["PRB", "Prebound edition"],
    ["REV", "Revised edition"],
    ["SCH", "School edition"],
    ["SIG", "Signed edition"],
    ["SMP", "Simplified language edition"],
    ["SPE", "Special edition"],
    ["STU", "Student edition"],
    ["TCH", "Teacher’s edition"],
    ["UBR", "Unabridged edition"],
    ["ULP", "Ultra large print edition"],
    ["UNN", "Edition with unnumbered copies"],
    ["UXP", "Unexpurgated edition"],
    ["VAR", "Variorum edition"],
    ["VOR", "Readaloud edition"],
  ]);

  // List 22 — Language role
  _lists[22] = new Map([
    ["01", "Language of text"],
    ["02", "Original language of a translated text"],
    ["03", "Language of abstracts"],
    ["06", "Original language in a multilingual edition"],
    ["07", "Translated language in a multilingual edition"],
    ["08", "Language of audio track"],
    ["09", "Language of subtitles"],
    ["10", "Language of original audio track"],
    ["11", "Original language audio track in a multilingual product"],
    ["12", "Language of notes"],
    ["13", "Language of introduction / end matter"],
    ["14", "Target language of teaching / learning"],
    ["15", "Additional vocabulary / text in this language"],
  ]);

  // List 23 — Extent type
  _lists[23] = new Map([
    ["00", "Main content page count"],
    ["02", "Total text length"],
    ["03", "Front matter page count"],
    ["04", "Back matter page count"],
    ["05", "Total numbered pages"],
    ["06", "Production page count"],
    ["07", "Absolute page count"],
    ["08", "Number of pages in print counterpart"],
    ["09", "Duration"],
    ["10", "Notional number of pages in digital product"],
    ["11", "Content page count"],
    ["12", "Total unnumbered insert page count"],
    ["13", "Duration of introductory matter"],
    ["14", "Duration of main content"],
    ["15", "Duration of back matter"],
    ["16", "Production duration"],
    ["17", "Number of cards"],
    ["18", "Number of write-in pages"],
    ["22", "Filesize"],
    ["23", "Storage filesize"],
  ]);

  // List 24 — Extent unit
  _lists[24] = new Map([
    ["00", "Physical pieces"],
    ["01", "Characters"],
    ["02", "Words"],
    ["03", "Pages"],
    ["04", "Hours (integer and decimals)"],
    ["05", "Minutes (integer and decimals)"],
    ["06", "Seconds (integer only)"],
    ["11", "Tracks"],
    ["12", "Discs"],
    ["14", "Hours HHH"],
    ["15", "Hours and minutes HHHMM"],
    ["16", "Hours minutes seconds HHHMMSS"],
    ["17", "Bytes"],
    ["18", "Kbytes"],
    ["19", "Mbytes"],
    ["31", "Chapters"],
  ]);

  // List 25 — Illustration and other content type
  _lists[25] = new Map([
    ["00", "Unspecified, see description"],
    ["01", "Illustrations, black and white"],
    ["02", "Illustrations, color"],
    ["03", "Halftones, black and white"],
    ["04", "Halftones, color"],
    ["05", "Line drawings, black and white"],
    ["06", "Line drawings, color"],
    ["07", "Tables, black and white"],
    ["08", "Tables, color"],
    ["09", "Illustrations, unspecified"],
    ["10", "Halftones, unspecified"],
    ["11", "Tables, unspecified"],
    ["12", "Line drawings, unspecified"],
    ["13", "Halftones, duotone"],
    ["14", "Maps"],
    ["15", "Frontispiece"],
    ["16", "Diagrams"],
    ["17", "Figures"],
    ["18", "Charts"],
    ["19", "Recorded music items"],
    ["20", "Printed music items"],
    ["21", "Graphs"],
    ["22", "Plates, unspecified"],
    ["23", "Plates, black and white"],
    ["24", "Plates, color"],
    ["25", "Index"],
    ["26", "Bibliography"],
    ["27", "Inset maps"],
    ["28", "GPS grids"],
    ["29", "Glossary"],
    ["30", "Table of contents"],
    ["31", "Gazetteer"],
  ]);

  // List 27 — Subject scheme identifier
  _lists[27] = new Map([
    ["01", "Dewey"],
    ["02", "Abridged Dewey"],
    ["03", "LC classification"],
    ["04", "LC subject heading"],
    ["05", "NLM classification"],
    ["06", "MeSH heading"],
    ["07", "NAL subject heading"],
    ["08", "AAT"],
    ["09", "UDC"],
    ["10", "BISAC Subject Heading"],
    ["11", "BISAC Regional theme"],
    ["12", "BIC subject category"],
    ["13", "BIC geographical qualifier"],
    ["14", "BIC language qualifier (language as subject)"],
    ["15", "BIC time period qualifier"],
    ["16", "BIC educational purpose qualifier"],
    ["17", "BIC reading level and special interest qualifier"],
    ["18", "DDC-Sachgruppen der Deutschen Nationalbibliografie"],
    ["19", "LC fiction genre heading"],
    ["20", "Keywords"],
    ["21", "BIC children’s book marketing category"],
    ["22", "BISAC Merchandising Theme"],
    ["23", "Publisher’s own category code"],
    ["24", "Proprietary subject scheme"],
    ["25", "Tabla de materias ISBN"],
    ["26", "Warengruppen-Systematik des deutschen Buchhandels"],
    ["27", "SWD"],
    ["28", "Thèmes Electre"],
    ["29", "CLIL Classification thématique"],
    ["30", "DNB-Sachgruppen"],
    ["31", "NUGI"],
    ["32", "NUR"],
    ["33", "ECPA Christian Book Category"],
    ["34", "SISO"],
    ["35", "Korean Decimal Classification (KDC)"],
    ["36", "DDC Deutsch 22"],
    ["37", "Bokgrupper"],
    ["38", "Varegrupper"],
    ["39", "Læreplaner-KL06"],
    ["40", "Nippon Decimal Classification"],
    ["41", "BSQ"],
    ["42", "ANELE Materias"],
    ["43", "GREP Utdanningsprogram"],
    ["44", "GREP Programområde"],
    ["45", "Undervisningsmateriell"],
    ["46", "Norsk DDK"],
    ["47", "Varugrupper"],
    ["48", "SAB"],
    ["49", "Läromedelstyp"],
    ["50", "Förhandsbeskrivning"],
    ["51", "Spanish ISBN UDC subset"],
    ["52", "ECI subject categories"],
    ["53", "Soggetto CCE"],
    ["54", "Qualificatore geografico CCE"],
    ["55", "Qualificatore di lingua CCE"],
    ["56", "Qualificatore di periodo storico CCE"],
    ["57", "Qualificatore di livello scolastico CCE"],
    ["58", "Qualificatore di età di lettura CCE"],
    ["59", "VdS Bildungsmedien Fächer"],
    ["60", "GREP Fagkoder"],
    ["61", "JEL classification"],
    ["62", "CSH"],
    ["63", "RVM"],
    ["64", "YSA"],
    ["65", "Allärs"],
    ["66", "YKL"],
    ["67", "MUSA"],
    ["68", "CILLA"],
    ["69", "Kaunokki"],
    ["70", "Bella"],
    ["71", "YSO"],
    ["72", "PTO"],
    ["73", "Suomalainen kirja-alan luokitus"],
    ["74", "Sears"],
    ["75", "BIC E4L"],
    ["76", "CSR"],
    ["77", "Suomalainen oppiaineluokitus"],
    ["78", "Japanese book trade C-Code"],
    ["79", "Japanese book trade Genre Code"],
    ["80", "Fiktiivisen aineiston lisäluokitus"],
    ["81", "Arabic Subject heading scheme"],
    ["82", "Arabized BIC subject category"],
    ["83", "Arabized LC subject headings"],
    ["84", "Bibliotheca Alexandrina Subject Headings"],
    ["85", "Postal code"],
    ["86", "GeoNames ID"],
    ["87", "NewBooks Subject Classification"],
    ["88", "Chinese Library Classification"],
    ["89", "NTCPDSAC Classification"],
    ["90", "Season and Event Indicator"],
    ["91", "GND"],
    ["92", "BIC UKSLC"],
    ["93", "Thema subject category"],
    ["94", "Thema place qualifier"],
    ["95", "Thema language qualifier"],
    ["96", "Thema time period qualifier"],
    ["97", "Thema educational purpose qualifier"],
    ["98", "Thema interest age / special interest qualifier"],
    ["99", "Thema style qualifier"],
    ["A2", "Ämnesord"],
    ["A3", "Statystyka Książek Papierowych, Mówionych I Elektronicznych"],
    ["A4", "CCSS"],
    ["A5", "Rameau"],
    ["A6", "Nomenclature discipline scolaire"],
    ["A7", "ISIC"],
    ["A8", "LC Children’s Subject Headings"],
    ["A9", "Ny Läromedel"],
    ["B0", "EuroVoc"],
    ["B1", "BISG Educational Taxonomy"],
    ["B2", "Keywords (not for display)"],
    ["B3", "Nomenclature Diplôme"],
    ["B4", "Key character names"],
    ["B5", "Key place names"],
    ["B6", "FAST"],
    ["B7", "NGSS"],
    ["B8", "MVB-Lesemotive"],
    ["B9", "LOPS21 Subject module"],
    ["C0", "Læreplaner-LK20"],
    ["C1", "Kompetansemål-LK20"],
    ["C2", "Kompetansemålsett-LK20"],
    ["C3", "Tverrfaglige temaer-LK20"],
    ["C4", "CLIL – Type d’article scolaire"],
    ["C5", "GAR – Type pédagogique"],
    ["C6", "ISCED-F"],
    ["C7", "Klassifikationen von Spielen, Puzzles und Spielwaren"],
    ["C8", "NBVok NTSF"],
    ["C9", "JPRO Genre"],
    ["D0", "KAUNO"],
    ["D1", "SLM"],
    ["D2", "YSO-places"],
    ["D3", "Norske emneord"],
    ["D4", "Austlang"],
    ["D5", "German Tropes list"],
  ]);

  // List 28 — Audience type
  _lists[28] = new Map([
    ["01", "General / adult"],
    ["02", "Children"],
    ["03", "Teenage"],
    ["04", "Primary and secondary education"],
    ["11", "Pre-primary education"],
    ["12", "Primary education"],
    ["13", "Lower secondary education"],
    ["14", "Upper secondary education"],
    ["05", "Tertiary education"],
    ["06", "Professional and scholarly"],
    ["08", "Adult education"],
    ["07", "EFL / TEFL / TESOL"],
    ["09", "Second / additional language teaching"],
  ]);

  // List 29 — Audience code type
  _lists[29] = new Map([
    ["01", "ONIX audience codes"],
    ["02", "Proprietary audience scheme"],
    ["03", "MPAA rating"],
    ["04", "BBFC rating"],
    ["05", "FSK rating"],
    ["06", "BTLF audience code"],
    ["07", "Electre audience code"],
    ["08", "ANELE Tipo"],
    ["09", "AVI"],
    ["10", "USK rating"],
    ["11", "AWS"],
    ["12", "Schulform"],
    ["13", "Bundesland"],
    ["14", "Ausbildungsberuf"],
    ["15", "Suomalainen kouluasteluokitus"],
    ["16", "CBG age guidance"],
    ["17", "BookData audience code"],
    ["18", "AVI (revised)"],
    ["19", "Lexile measure"],
    ["20", "Fry Readability score"],
    ["21", "Japanese Children’s audience code"],
    ["22", "ONIX Adult audience rating"],
    ["23", "Common European Framework of Reference for Language Learning (CEFR)"],
    ["24", "Korean Publication Ethics Commission rating"],
    ["25", "IoE Book Band"],
    ["26", "FSK Lehr-/Infoprogramm"],
    ["27", "Intended audience language"],
    ["28", "PEGI rating"],
    ["29", "Gymnasieprogram"],
    ["30", "ISCED 2011"],
  ]);

  // List 30 — Audience range qualifier
  _lists[30] = new Map([
    ["11", "US school grade range"],
    ["12", "UK school grade"],
    ["15", "Reading speed, words per minute"],
    ["16", "Interest age, months"],
    ["17", "Interest age, years"],
    ["18", "Reading age, years"],
    ["19", "Spanish school grade"],
    ["20", "Skoletrinn"],
    ["21", "Nivå"],
    ["22", "Italian school grade"],
    ["23", "Schulform"],
    ["24", "Bundesland"],
    ["25", "Ausbildungsberuf"],
    ["26", "Canadian school grade range"],
    ["27", "Finnish school grade range"],
    ["28", "Finnish Upper secondary school course"],
    ["29", "Chinese School Grade range"],
    ["30", "French school cycles / classes"],
    ["31", "Brazil Education level"],
    ["32", "French educational levels"],
    ["33", "Finnish Upper secondary school course (2021+)"],
    ["34", "Detailed French educational levels"],
    ["35", "GREP Aarstrinn"],
  ]);

  // List 31 — Audience range precision
  _lists[31] = new Map([
    ["01", "Exact"],
    ["03", "From"],
    ["04", "To"],
  ]);

  // List 32 — Complexity scheme identifier
  _lists[32] = new Map([
    ["01", "Lexile code"],
    ["02", "Lexile number"],
    ["03", "Fry Readability score"],
    ["04", "IoE Book Band"],
    ["05", "Fountas & Pinnell Text Level Gradient"],
    ["06", "Lexile measure"],
    ["07", "ATOS for Books"],
    ["08", "Flesch-Kincaid Grade Level"],
    ["09", "Guided Reading Level"],
    ["10", "Reading Recovery Level"],
    ["11", "LIX"],
    ["12", "Lexile Audio measure"],
    ["13", "Lexile measure (Spanish)"],
  ]);

  // List 34 — Text format
  _lists[34] = new Map([
    ["02", "HTML"],
    ["03", "XML"],
    ["05", "XHTML"],
    ["06", "Default text format"],
    ["07", "Basic ASCII text"],
  ]);

  // List 41 — Prize or award achievement
  _lists[41] = new Map([
    ["01", "Winner"],
    ["02", "Runner-up"],
    ["03", "Commended"],
    ["04", "Short-listed"],
    ["05", "Long-listed"],
    ["06", "Joint winner"],
    ["07", "Nominated"],
  ]);

  // List 42 — Text item type
  _lists[42] = new Map([
    ["01", "Textual work"],
    ["02", "Front matter"],
    ["03", "Body matter"],
    ["04", "Back matter"],
  ]);

  // List 43 — Text item identifier type
  _lists[43] = new Map([
    ["01", "Proprietary text item ID scheme"],
    ["03", "GTIN-13"],
    ["06", "DOI"],
    ["09", "PII"],
    ["10", "SICI"],
    ["11", "ISTC"],
    ["15", "ISBN-13"],
    ["39", "ISCC"],
  ]);

  // List 44 — Name identifier type
  _lists[44] = new Map([
    ["01", "Proprietary name ID scheme"],
    ["02", "Proprietary"],
    ["03", "DNB publisher identifier"],
    ["04", "Börsenverein Verkehrsnummer"],
    ["05", "German ISBN Agency publisher identifier"],
    ["06", "GLN"],
    ["07", "SAN"],
    ["08", "MARC organization code"],
    ["10", "Centraal Boekhuis Relatie ID"],
    ["12", "Distributeurscode Boekenbank"],
    ["13", "Fondscode Boekenbank"],
    ["15", "Y-tunnus"],
    ["16", "ISNI"],
    ["17", "PND"],
    ["18", "NACO"],
    ["19", "Japanese Publisher identifier"],
    ["20", "GKD"],
    ["21", "ORCID"],
    ["22", "GAPP Publisher Identifier"],
    ["23", "VAT Identity Number"],
    ["24", "JP Distribution Identifier"],
    ["25", "GND"],
    ["26", "DUNS"],
    ["27", "Ringgold ID"],
    ["28", "Identifiant Editeur Electre"],
    ["29", "EIDR Party ID"],
    ["30", "Identifiant Marque Electre"],
    ["31", "VIAF ID"],
    ["32", "FundRef DOI"],
    ["33", "BNE CN"],
    ["34", "BNF Control Number"],
    ["35", "ARK"],
    ["36", "Nasjonalt autoritetsregister"],
    ["37", "GRID"],
    ["38", "IDRef"],
    ["39", "IPI"],
    ["40", "ROR"],
    ["41", "EORI"],
    ["42", "LEI"],
    ["43", "SIREN"],
    ["44", "SIRET"],
    ["45", "Chinese Participant identifier"],
  ]);

  // List 45 — Publishing role
  _lists[45] = new Map([
    ["01", "Publisher"],
    ["02", "Co-publisher"],
    ["03", "Sponsor"],
    ["04", "Publisher of original-language version"],
    ["05", "Host / distributor of electronic content"],
    ["06", "Published for / on behalf of"],
    ["07", "Published in association with"],
    ["09", "New or acquiring publisher"],
    ["10", "Publishing group"],
    ["11", "Publisher of facsimile original"],
    ["12", "Repackager of prebound edition"],
    ["13", "Former publisher"],
    ["14", "Publication funder"],
    ["15", "Research funder"],
    ["16", "Funding body"],
    ["17", "Printer"],
    ["18", "Binder"],
    ["19", "Manufacturer"],
    ["21", "Previous publisher"],
  ]);

  // List 46 — Sales rights type
  _lists[46] = new Map([
    ["00", "Sales rights unknown or unstated for any reason"],
    ["01", "For sale, based on publisher’s exclusive publishing rights in the specified territory"],
    ["02", "For sale, based on publisher’s non-exclusive publishing rights in the specified territory"],
    ["03", "Not for sale in the specified territory (reason unspecified)"],
    ["04", "Not for sale in the specified territory (but publisher holds exclusive publishing rights in that territory)"],
    ["05", "Not for sale in the specified territory (but publisher holds non-exclusive publishing rights in that territory)"],
    ["06", "Not for sale in the specified territory (because publisher does not hold publishing rights in that territory)"],
    ["07", "For sale with exclusive rights in the specified countries or territories (sales restriction applies)"],
    ["08", "For sale with non-exclusive rights in the specified countries or territories (sales restriction applies)"],
  ]);

  // List 48 — Measure type
  _lists[48] = new Map([
    ["01", "Height"],
    ["02", "Width"],
    ["03", "Thickness"],
    ["04", "Page trim height"],
    ["05", "Page trim width"],
    ["06", "Unit volume"],
    ["07", "Unit capacity"],
    ["08", "Unit weight"],
    ["09", "Diameter (sphere)"],
    ["10", "Unfolded / unrolled sheet height"],
    ["11", "Unfolded / unrolled sheet width"],
    ["12", "Diameter (tube or cylinder)"],
    ["13", "Rolled sheet package side measure"],
    ["14", "Unpackaged height"],
    ["15", "Unpackaged width"],
    ["16", "Unpackaged thickness"],
    ["17", "Total battery weight"],
    ["18", "Total weight of Lithium"],
    ["19", "Assembled length"],
    ["20", "Assembled width"],
    ["21", "Assembled height"],
    ["22", "Unpackaged unit weight"],
    ["23", "Carton length"],
    ["24", "Carton width"],
    ["25", "Carton height"],
    ["26", "Carton weight"],
    ["27", "Pallet length"],
    ["28", "Pallet width"],
    ["29", "Pallet height"],
    ["30", "Pallet weight"],
  ]);

  // List 49 — Region – based on ISO 3166-2
  _lists[49] = new Map([
    ["AU-CT", "Australian Capital Territory"],
    ["AU-NS", "New South Wales"],
    ["AU-NT", "Northern Territory"],
    ["AU-QL", "Queensland"],
    ["AU-SA", "South Australia"],
    ["AU-TS", "Tasmania"],
    ["AU-VI", "Victoria"],
    ["AU-WA", "Western Australia"],
    ["BE-BRU", "Brussels-Capital Region"],
    ["BE-VLG", "Flemish Region"],
    ["BE-WAL", "Walloon Region"],
    ["CA-AB", "Alberta"],
    ["CA-BC", "British Columbia"],
    ["CA-MB", "Manitoba"],
    ["CA-NB", "New Brunswick"],
    ["CA-NL", "Newfoundland and Labrador"],
    ["CA-NS", "Nova Scotia"],
    ["CA-NT", "Northwest Territories"],
    ["CA-NU", "Nunavut"],
    ["CA-ON", "Ontario"],
    ["CA-PE", "Prince Edward Island"],
    ["CA-QC", "Quebec"],
    ["CA-SK", "Saskatchewan"],
    ["CA-YT", "Yukon Territory"],
    ["CN-BJ", "Beijing Municipality"],
    ["CN-TJ", "Tianjin Municipality"],
    ["CN-HE", "Hebei Province"],
    ["CN-SX", "Shanxi Province"],
    ["CN-NM", "Nei Mongol Autonomous Region"],
    ["CN-LN", "Liaoning Province"],
    ["CN-JL", "Jilin Province"],
    ["CN-HL", "Heilongjiang Province"],
    ["CN-SH", "Shanghai Municipality"],
    ["CN-JS", "Jiangsu Province"],
    ["CN-ZJ", "Zhejiang Province"],
    ["CN-AH", "Anhui Province"],
    ["CN-FJ", "Fujian Province"],
    ["CN-JX", "Jiangxi Province"],
    ["CN-SD", "Shandong Province"],
    ["CN-HA", "Henan Province"],
    ["CN-HB", "Hubei Province"],
    ["CN-HN", "Hunan Province"],
    ["CN-GD", "Guangdong Province"],
    ["CN-GX", "Guangxi Zhuangzu Autonomous Region"],
    ["CN-HI", "Hainan Province"],
    ["CN-CQ", "Chongqing Municipality"],
    ["CN-SC", "Sichuan Province"],
    ["CN-GZ", "Guizhou Province"],
    ["CN-YN", "Yunnan Province"],
    ["CN-XZ", "Tibet Autonomous Region"],
    ["CN-SN", "Shaanxi Province"],
    ["CN-GS", "Gansu Province"],
    ["CN-QH", "Qinghai Province"],
    ["CN-NX", "Ningxia Huizu Autonomous Region"],
    ["CN-XJ", "Xinjiang Uygur Autonomous Region"],
    ["CN-TW", "Taiwan Province"],
    ["CN-HK", "Hong Kong Special Administrative Region"],
    ["CN-MO", "Macau Special Administrative Region"],
    ["CN-11", "Beijing Municipality"],
    ["CN-12", "Tianjin Municipality"],
    ["CN-13", "Hebei Province"],
    ["CN-14", "Shanxi Province"],
    ["CN-15", "Inner Mongolia Autonomous Region"],
    ["CN-21", "Liaoning Province"],
    ["CN-22", "Jilin Province"],
    ["CN-23", "Heilongjiang Province"],
    ["CN-31", "Shanghai Municipality"],
    ["CN-32", "Jiangsu Province"],
    ["CN-33", "Zhejiang Province"],
    ["CN-34", "Anhui Province"],
    ["CN-35", "Fujian Province"],
    ["CN-36", "Jiangxi Province"],
    ["CN-37", "Shandong Province"],
    ["CN-41", "Henan Province"],
    ["CN-42", "Hubei Province"],
    ["CN-43", "Hunan Province"],
    ["CN-44", "Guangdong Province"],
    ["CN-45", "Guangxi Zhuang Autonomous Region"],
    ["CN-46", "Hainan Province"],
    ["CN-50", "Chongqing Municipality"],
    ["CN-51", "Sichuan Province"],
    ["CN-52", "Guizhou Province"],
    ["CN-53", "Yunnan Province"],
    ["CN-54", "Tibet Autonomous Region"],
    ["CN-61", "Shaanxi Province"],
    ["CN-62", "Gansu Province"],
    ["CN-63", "Qinghai Province"],
    ["CN-64", "Ningxia Hui Autonomous Region"],
    ["CN-65", "Xinjiang Uyghur Autonomous Region"],
    ["CN-71", "Taiwan Province"],
    ["CN-91", "Hong Kong Special Administrative Region"],
    ["CN-92", "Macau Special Administrative Region"],
    ["ES-CN", "Canary Islands"],
    ["FR-H", "Corsica"],
    ["GB-AIR", "UK airside"],
    ["GB-APS", "UK airports"],
    ["GB-CHA", "Channel Islands"],
    ["GB-ENG", "England"],
    ["GB-EWS", "England, Wales, Scotland"],
    ["GB-IOM", "Isle of Man"],
    ["GB-NIR", "Northern Ireland"],
    ["GB-SCT", "Scotland"],
    ["GB-WLS", "Wales"],
    ["IE-AIR", "Ireland airside"],
    ["IT-AG", "Agrigento"],
    ["IT-AL", "Alessandria"],
    ["IT-AN", "Ancona"],
    ["IT-AO", "Aosta"],
    ["IT-AR", "Arezzo"],
    ["IT-AP", "Ascoli Piceno"],
    ["IT-AT", "Asti"],
    ["IT-AV", "Avellino"],
    ["IT-BA", "Bari"],
    ["IT-BT", "Barletta-Andria-Trani"],
    ["IT-BL", "Belluno"],
    ["IT-BN", "Benevento"],
    ["IT-BG", "Bergamo"],
    ["IT-BI", "Biella"],
    ["IT-BO", "Bologna"],
    ["IT-BZ", "Bolzano"],
    ["IT-BS", "Brescia"],
    ["IT-BR", "Brindisi"],
    ["IT-CA", "Cagliari"],
    ["IT-CL", "Caltanissetta"],
    ["IT-CB", "Campobasso"],
    ["IT-CI", "Sulcis Iglesiente"],
    ["IT-CE", "Caserta"],
    ["IT-CT", "Catania"],
    ["IT-CZ", "Catanzaro"],
    ["IT-CH", "Chieti"],
    ["IT-CO", "Como"],
    ["IT-CS", "Cosenza"],
    ["IT-CR", "Cremona"],
    ["IT-KR", "Crotone"],
    ["IT-CN", "Cuneo"],
    ["IT-EN", "Enna"],
    ["IT-FM", "Fermo"],
    ["IT-FE", "Ferrara"],
    ["IT-FI", "Firenze"],
    ["IT-FG", "Foggia"],
    ["IT-FC", "Forlì-Cesena"],
    ["IT-FR", "Frosinone"],
    ["IT-GE", "Genova"],
    ["IT-GO", "Gorizia"],
    ["IT-GR", "Grosseto"],
    ["IT-IM", "Imperia"],
    ["IT-IS", "Isernia"],
    ["IT-SP", "La Spezia"],
    ["IT-AQ", "L’Aquila"],
    ["IT-LT", "Latina"],
    ["IT-LE", "Lecce"],
    ["IT-LC", "Lecco"],
    ["IT-LI", "Livorno"],
    ["IT-LO", "Lodi"],
    ["IT-LU", "Lucca"],
    ["IT-MC", "Macerata"],
    ["IT-MN", "Mantova"],
    ["IT-MS", "Massa-Carrara"],
    ["IT-MT", "Matera"],
    ["IT-VS", "Medio Campidano"],
    ["IT-ME", "Messina"],
    ["IT-MI", "Milano"],
    ["IT-MO", "Modena"],
    ["IT-MB", "Monza e Brianza"],
    ["IT-NA", "Napoli"],
    ["IT-NO", "Novara"],
    ["IT-NU", "Nuoro"],
    ["IT-OG", "Ogliastra"],
    ["IT-OT", "Gallura Nord-Est Sardegna"],
    ["IT-OR", "Oristano"],
    ["IT-PD", "Padova"],
    ["IT-PA", "Palermo"],
    ["IT-PR", "Parma"],
    ["IT-PV", "Pavia"],
    ["IT-PG", "Perugia"],
    ["IT-PU", "Pesaro e Urbino"],
    ["IT-PE", "Pescara"],
    ["IT-PC", "Piacenza"],
    ["IT-PI", "Pisa"],
    ["IT-PT", "Pistoia"],
    ["IT-PN", "Pordenone"],
    ["IT-PZ", "Potenza"],
    ["IT-PO", "Prato"],
    ["IT-RG", "Ragusa"],
    ["IT-RA", "Ravenna"],
    ["IT-RC", "Reggio Calabria"],
    ["IT-RE", "Reggio Emilia"],
    ["IT-RI", "Rieti"],
    ["IT-RN", "Rimini"],
    ["IT-RM", "Roma"],
    ["IT-RO", "Rovigo"],
    ["IT-SA", "Salerno"],
    ["IT-SS", "Sassari"],
    ["IT-SV", "Savona"],
    ["IT-SI", "Siena"],
    ["IT-SR", "Siracusa"],
    ["IT-SO", "Sondrio"],
    ["IT-TA", "Taranto"],
    ["IT-TE", "Teramo"],
    ["IT-TR", "Terni"],
    ["IT-TO", "Torino"],
    ["IT-TP", "Trapani"],
    ["IT-TN", "Trento"],
    ["IT-TV", "Treviso"],
    ["IT-TS", "Trieste"],
    ["IT-UD", "Udine"],
    ["IT-VA", "Varese"],
    ["IT-VE", "Venezia"],
    ["IT-VB", "Verbano-Cusio-Ossola"],
    ["IT-VC", "Vercelli"],
    ["IT-VR", "Verona"],
    ["IT-VV", "Vibo Valentia"],
    ["IT-VI", "Vicenza"],
    ["IT-VT", "Viterbo"],
    ["RS-KM", "Kosovo-Metohija"],
    ["RS-VO", "Vojvodina"],
    ["RU-AD", "Republic of Adygeya"],
    ["RU-AL", "Republic of Altay"],
    ["RU-BA", "Republic of Bashkortostan"],
    ["RU-BU", "Republic of Buryatiya"],
    ["RU-CE", "Chechenskaya Republic"],
    ["RU-CU", "Chuvashskaya Republic"],
    ["RU-DA", "Republic of Dagestan"],
    ["RU-IN", "Republic of Ingushetiya"],
    ["RU-KB", "Kabardino-Balkarskaya Republic"],
    ["RU-KL", "Republic of Kalmykiya"],
    ["RU-KC", "Karachayevo-Cherkesskaya Republic"],
    ["RU-KR", "Republic of Kareliya"],
    ["RU-KK", "Republic of Khakasiya"],
    ["RU-KO", "Republic of Komi"],
    ["RU-ME", "Republic of Mariy El"],
    ["RU-MO", "Republic of Mordoviya"],
    ["RU-SA", "Republic of Sakha (Yakutiya)"],
    ["RU-SE", "Republic of Severnaya Osetiya-Alaniya"],
    ["RU-TA", "Republic of Tatarstan"],
    ["RU-TY", "Republic of Tyva (Tuva)"],
    ["RU-UD", "Udmurtskaya Republic"],
    ["RU-ALT", "Altayskiy Administrative Territory"],
    ["RU-KAM", "Kamchatskiy Administrative Territory"],
    ["RU-KHA", "Khabarovskiy Administrative Territory"],
    ["RU-KDA", "Krasnodarskiy Administrative Territory"],
    ["RU-KYA", "Krasnoyarskiy Administrative Territory"],
    ["RU-PER", "Permskiy Administrative Territory"],
    ["RU-PRI", "Primorskiy Administrative Territory"],
    ["RU-STA", "Stavropol’skiy Administrative Territory"],
    ["RU-ZAB", "Zabaykal’skiy Administrative Territory"],
    ["RU-AMU", "Amurskaya Administrative Region"],
    ["RU-ARK", "Arkhangel’skaya Administrative Region"],
    ["RU-AST", "Astrakhanskaya Administrative Region"],
    ["RU-BEL", "Belgorodskaya Administrative Region"],
    ["RU-BRY", "Bryanskaya Administrative Region"],
    ["RU-CHE", "Chelyabinskaya Administrative Region"],
    ["RU-IRK", "Irkutskaya Administrative Region"],
    ["RU-IVA", "Ivanovskaya Administrative Region"],
    ["RU-KGD", "Kaliningradskaya Administrative Region"],
    ["RU-KLU", "Kaluzhskaya Administrative Region"],
    ["RU-KEM", "Kemerovskaya Administrative Region"],
    ["RU-KIR", "Kirovskaya Administrative Region"],
    ["RU-KOS", "Kostromskaya Administrative Region"],
    ["RU-KGN", "Kurganskaya Administrative Region"],
    ["RU-KRS", "Kurskaya Administrative Region"],
    ["RU-LEN", "Leningradskaya Administrative Region"],
    ["RU-LIP", "Lipetskaya Administrative Region"],
    ["RU-MAG", "Magadanskaya Administrative Region"],
    ["RU-MOS", "Moskovskaya Administrative Region"],
    ["RU-MUR", "Murmanskaya Administrative Region"],
    ["RU-NIZ", "Nizhegorodskaya Administrative Region"],
    ["RU-NGR", "Novgorodskaya Administrative Region"],
    ["RU-NVS", "Novosibirskaya Administrative Region"],
    ["RU-OMS", "Omskaya Administrative Region"],
    ["RU-ORE", "Orenburgskaya Administrative Region"],
    ["RU-ORL", "Orlovskaya Administrative Region"],
    ["RU-PNZ", "Penzenskaya Administrative Region"],
    ["RU-PSK", "Pskovskaya Administrative Region"],
    ["RU-ROS", "Rostovskaya Administrative Region"],
    ["RU-RYA", "Ryazanskaya Administrative Region"],
    ["RU-SAK", "Sakhalinskaya Administrative Region"],
    ["RU-SAM", "Samarskaya Administrative Region"],
    ["RU-SAR", "Saratovskaya Administrative Region"],
    ["RU-SMO", "Smolenskaya Administrative Region"],
    ["RU-SVE", "Sverdlovskaya Administrative Region"],
    ["RU-TAM", "Tambovskaya Administrative Region"],
    ["RU-TOM", "Tomskaya Administrative Region"],
    ["RU-TUL", "Tul’skaya Administrative Region"],
    ["RU-TVE", "Tverskaya Administrative Region"],
    ["RU-TYU", "Tyumenskaya Administrative Region"],
    ["RU-ULY", "Ul’yanovskaya Administrative Region"],
    ["RU-VLA", "Vladimirskaya Administrative Region"],
    ["RU-VGG", "Volgogradskaya Administrative Region"],
    ["RU-VLG", "Vologodskaya Administrative Region"],
    ["RU-VOR", "Voronezhskaya Administrative Region"],
    ["RU-YAR", "Yaroslavskaya Administrative Region"],
    ["RU-MOW", "Moskva City"],
    ["RU-SPE", "Sankt-Peterburg City"],
    ["RU-YEV", "Yevreyskaya Autonomous Administrative Region"],
    ["RU-CHU", "Chukotskiy Autonomous District"],
    ["RU-KHM", "Khanty-Mansiyskiy Autonomous District"],
    ["RU-NEN", "Nenetskiy Autonomous District"],
    ["RU-YAN", "Yamalo-Nenetskiy Autonomous District"],
    ["US-AK", "Alaska"],
    ["US-AL", "Alabama"],
    ["US-AR", "Arkansas"],
    ["US-AZ", "Arizona"],
    ["US-CA", "California"],
    ["US-CO", "Colorado"],
    ["US-CT", "Connecticut"],
    ["US-DC", "District of Columbia"],
    ["US-DE", "Delaware"],
    ["US-FL", "Florida"],
    ["US-GA", "Georgia"],
    ["US-HI", "Hawaii"],
    ["US-IA", "Iowa"],
    ["US-ID", "Idaho"],
    ["US-IL", "Illinois"],
    ["US-IN", "Indiana"],
    ["US-KS", "Kansas"],
    ["US-KY", "Kentucky"],
    ["US-LA", "Louisiana"],
    ["US-MA", "Massachusetts"],
    ["US-MD", "Maryland"],
    ["US-ME", "Maine"],
    ["US-MI", "Michigan"],
    ["US-MN", "Minnesota"],
    ["US-MO", "Missouri"],
    ["US-MS", "Mississippi"],
    ["US-MT", "Montana"],
    ["US-NC", "North Carolina"],
    ["US-ND", "North Dakota"],
    ["US-NE", "Nebraska"],
    ["US-NH", "New Hampshire"],
    ["US-NJ", "New Jersey"],
    ["US-NM", "New Mexico"],
    ["US-NV", "Nevada"],
    ["US-NY", "New York"],
    ["US-OH", "Ohio"],
    ["US-OK", "Oklahoma"],
    ["US-OR", "Oregon"],
    ["US-PA", "Pennsylvania"],
    ["US-RI", "Rhode Island"],
    ["US-SC", "South Carolina"],
    ["US-SD", "South Dakota"],
    ["US-TN", "Tennessee"],
    ["US-TX", "Texas"],
    ["US-UT", "Utah"],
    ["US-VA", "Virginia"],
    ["US-VT", "Vermont"],
    ["US-WA", "Washington"],
    ["US-WI", "Wisconsin"],
    ["US-WV", "West Virginia"],
    ["US-WY", "Wyoming"],
    ["ECZ", "Eurozone"],
    ["WORLD", "World"],
  ]);

  // List 50 — Measure unit
  _lists[50] = new Map([
    ["cm", "Centimeters"],
    ["gr", "Grams"],
    ["in", "Inches (US)"],
    ["kg", "Kilograms"],
    ["lb", "Pounds (US)"],
    ["mm", "Millimeters"],
    ["oz", "Ounces (US)"],
    ["px", "Pixels"],
  ]);

  // List 51 — Product relation
  _lists[51] = new Map([
    ["00", "Unspecified"],
    ["01", "Includes"],
    ["02", "Is part of"],
    ["03", "Replaces"],
    ["04", "Has companion product"],
    ["05", "Replaced by"],
    ["06", "Alternative format"],
    ["07", "Has ancillary product"],
    ["08", "Is ancillary to"],
    ["09", "Is remaindered as"],
    ["10", "Is remainder of"],
    ["11", "Is other-language version of"],
    ["12", "Publisher’s suggested alternative"],
    ["13", "Epublication based on (print product)"],
    ["16", "POD replacement for"],
    ["17", "Replaced by POD"],
    ["18", "Is special edition of"],
    ["19", "Has special edition"],
    ["20", "Is prebound edition of"],
    ["21", "Is original of prebound edition"],
    ["22", "Product by same author"],
    ["23", "Similar product"],
    ["24", "Is facsimile of"],
    ["25", "Is original of facsimile"],
    ["26", "Is license for"],
    ["27", "Electronic version available as"],
    ["28", "Enhanced version available as"],
    ["29", "Basic version available as"],
    ["30", "Product in same collection"],
    ["31", "Has alternative in a different market sector"],
    ["32", "Has equivalent intended for a different market"],
    ["33", "Has alternative intended for different market"],
    ["34", "Cites"],
    ["35", "Is cited by"],
    ["37", "Is signed version of"],
    ["38", "Has signed version"],
    ["39", "Has related student material"],
    ["40", "Has related teacher material"],
    ["41", "Some content shared with"],
    ["42", "Is later edition of first edition"],
    ["43", "Adapted from"],
    ["44", "Adapted as"],
    ["45", "Has linked product offer"],
    ["46", "May be substituted by"],
    ["47", "May be substituted for"],
    ["48", "Is linked product offer"],
  ]);

  // List 53 — Returns conditions code type
  _lists[53] = new Map([
    ["00", "Proprietary returns coding scheme"],
    ["01", "French book trade returns conditions code"],
    ["02", "BISAC Returnable Indicator code"],
    ["03", "UK book trade returns conditions code"],
    ["04", "ONIX Returns conditions code"],
  ]);

  // List 55 — Date format
  _lists[55] = new Map([
    ["00", "YYYYMMDD"],
    ["01", "YYYYMM"],
    ["02", "YYYYWW"],
    ["03", "YYYYQ"],
    ["04", "YYYYS"],
    ["05", "YYYY"],
    ["06", "YYYYMMDDYYYYMMDD"],
    ["07", "YYYYMMYYYYMM"],
    ["08", "YYYYWWYYYYWW"],
    ["09", "YYYYQYYYYQ"],
    ["10", "YYYYSYYYYS"],
    ["11", "YYYYYYYY"],
    ["12", "Text string"],
    ["13", "YYYYMMDDThhmm"],
    ["14", "YYYYMMDDThhmmss"],
    ["20", "YYYYMMDD (H)"],
    ["21", "YYYYMM (H)"],
    ["25", "YYYY (H)"],
    ["32", "Text string (H)"],
  ]);

  // List 57 — Unpriced item type
  _lists[57] = new Map([
    ["01", "Free of charge"],
    ["02", "Price to be announced"],
    ["03", "Not sold separately"],
    ["04", "Contact supplier"],
    ["05", "Not sold as set"],
    ["06", "Revenue share"],
    ["07", "Calculated from contents"],
    ["08", "Supplier does not supply"],
  ]);

  // List 58 — Price type
  _lists[58] = new Map([
    ["01", "RRP excluding tax"],
    ["02", "RRP including tax"],
    ["03", "FRP excluding tax"],
    ["04", "FRP including tax"],
    ["05", "Supplier’s Net price excluding tax"],
    ["06", "Supplier’s Net price excluding tax: rental goods"],
    ["07", "Supplier’s Net price including tax"],
    ["08", "Supplier’s alternative Net price excluding tax"],
    ["09", "Supplier’s alternative net price including tax"],
    ["11", "Special sale RRP excluding tax"],
    ["12", "Special sale RRP including tax"],
    ["13", "Special sale fixed retail price excluding tax"],
    ["14", "Special sale fixed retail price including tax"],
    ["15", "Supplier’s net price for special sale excluding tax"],
    ["17", "Supplier’s net price for special sale including tax"],
    ["21", "Pre-publication RRP excluding tax"],
    ["22", "Pre-publication RRP including tax"],
    ["23", "Pre-publication fixed retail price excluding tax"],
    ["24", "Pre-publication fixed retail price including tax"],
    ["25", "Supplier’s pre-publication net price excluding tax"],
    ["27", "Supplier’s pre-publication net price including tax"],
    ["31", "Freight-pass-through RRP excluding tax"],
    ["32", "Freight-pass-through billing price excluding tax"],
    ["33", "Importer’s Fixed retail price excluding tax"],
    ["34", "Importer’s Fixed retail price including tax"],
    ["35", "Nominal gratis copy value for customs purposes, excluding tax"],
    ["36", "Nominal value for claims purposes, excluding tax"],
    ["37", "Nominal value for customs purposes, excluding tax"],
    ["41", "Publishers retail price excluding tax"],
    ["42", "Publishers retail price including tax"],
  ]);

  // List 59 — Price type qualifier
  _lists[59] = new Map([
    ["00", "Unqualified price"],
    ["01", "Member / subscriber price"],
    ["02", "Export price"],
    ["03", "Reduced price applicable when the item is purchased as part of a set (or series, or collection)"],
    ["04", "Voucher price"],
    ["05", "Consumer price"],
    ["06", "Corporate / Library / Education price"],
    ["07", "Reservation order price"],
    ["08", "Promotional offer price"],
    ["09", "Linked price"],
    ["10", "Library price"],
    ["11", "Education price"],
    ["12", "Corporate price"],
    ["13", "Subscription service price"],
    ["14", "School library price"],
    ["15", "Academic library price"],
    ["16", "Public library price"],
    ["17", "Introductory price"],
    ["18", "Consortial price"],
    ["19", "Education price for alternative provision"],
  ]);

  // List 60 — Unit of pricing
  _lists[60] = new Map([
    ["00", "Per copy of whole product"],
    ["01", "Per page for printed loose-leaf content only"],
  ]);

  // List 61 — Price status
  _lists[61] = new Map([
    ["00", "Unspecified"],
    ["01", "Provisional"],
    ["02", "Confirmed"],
  ]);

  // List 62 — Tax rate type
  _lists[62] = new Map([
    ["H", "Higher rate"],
    ["P", "Tax paid at source (Italy)"],
    ["R", "Lower rate"],
    ["S", "Standard rate"],
    ["T", "Super-low rate"],
    ["Z", "Zero-rated"],
  ]);

  // List 64 — Publishing status
  _lists[64] = new Map([
    ["00", "Unspecified"],
    ["01", "Cancelled"],
    ["02", "Forthcoming"],
    ["03", "Postponed indefinitely"],
    ["04", "Active"],
    ["05", "No longer our product"],
    ["06", "Out of stock indefinitely"],
    ["07", "Out of print"],
    ["08", "Inactive"],
    ["09", "Unknown"],
    ["10", "Remaindered"],
    ["11", "Withdrawn from sale"],
    ["12", "Recalled"],
    ["13", "Active, but not sold separately"],
    ["15", "Recalled"],
    ["16", "Temporarily withdrawn from sale"],
    ["17", "Permanently withdrawn from sale"],
    ["18", "Active, but not sold as set"],
  ]);

  // List 65 — Product availability
  _lists[65] = new Map([
    ["01", "Cancelled"],
    ["09", "Not yet available, postponed indefinitely"],
    ["10", "Not yet available"],
    ["11", "Awaiting stock"],
    ["12", "Not yet available, will be POD"],
    ["20", "Available"],
    ["21", "In stock"],
    ["22", "To order"],
    ["23", "POD"],
    ["30", "Temporarily unavailable"],
    ["31", "Out of stock"],
    ["32", "Reprinting"],
    ["33", "Awaiting reissue"],
    ["34", "Temporarily withdrawn from sale"],
    ["40", "Not available (reason unspecified)"],
    ["41", "Not available, replaced by new product"],
    ["42", "Not available, other format available"],
    ["43", "No longer supplied by the supplier"],
    ["44", "Apply direct"],
    ["45", "Not sold separately"],
    ["46", "Withdrawn from sale"],
    ["47", "Remaindered"],
    ["48", "Not available, replaced by POD"],
    ["49", "Recalled"],
    ["50", "Not sold as set"],
    ["51", "Not available, publisher indicates OP"],
    ["52", "Not available, publisher no longer sells product in this market"],
    ["97", "No recent update received"],
    ["98", "No longer receiving updates"],
    ["99", "Contact supplier"],
  ]);

  // List 66 — BISAC returnable indicator
  _lists[66] = new Map([
    ["N", "No, not returnable"],
    ["Y", "Yes, returnable, full copies only"],
    ["S", "Yes, returnable, stripped cover"],
    ["C", "Conditional"],
  ]);

  // List 68 — Market publishing status
  _lists[68] = new Map([
    ["00", "Unspecified"],
    ["01", "Cancelled"],
    ["02", "Forthcoming"],
    ["03", "Postponed indefinitely"],
    ["04", "Active"],
    ["05", "No longer our product"],
    ["06", "Out of stock indefinitely"],
    ["07", "Out of print"],
    ["08", "Inactive"],
    ["09", "Unknown"],
    ["10", "Remaindered"],
    ["11", "Withdrawn from sale"],
    ["12", "Not available in this market"],
    ["13", "Active, but not sold separately"],
    ["14", "Active, with market restrictions"],
    ["15", "Recalled"],
    ["16", "Temporarily withdrawn from sale"],
    ["17", "Permanently withdrawn from sale"],
    ["18", "Active, but not sold as set"],
  ]);

  // List 69 — Agent role
  _lists[69] = new Map([
    ["01", "Publisher acts as own sales agent"],
    ["05", "Exclusive sales agent"],
    ["06", "Non-exclusive sales agent"],
    ["07", "Local publisher"],
    ["08", "Sales agent"],
  ]);

  // List 70 — Stock quantity code type
  _lists[70] = new Map([
    ["01", "Proprietary stock quantity coding scheme"],
    ["02", "APA stock quantity code"],
  ]);

  // List 71 — Sales restriction type
  _lists[71] = new Map([
    ["00", "Unspecified – see text"],
    ["01", "Retailer exclusive / own brand"],
    ["02", "Through office supplies outlets only"],
    ["03", "Internal publisher use only: do not list"],
    ["04", "Retailer exclusive"],
    ["05", "Retailer own brand"],
    ["06", "To libraries only"],
    ["07", "To schools only"],
    ["08", "Indiziert"],
    ["09", "Except to libraries"],
    ["10", "Through news outlets only"],
    ["11", "Retailer exception"],
    ["12", "Except to subscription services"],
    ["13", "To subscription services only"],
    ["14", "Except through online retail"],
    ["15", "Through online retail only"],
    ["16", "Except to schools"],
    ["17", "Through Inventoryless POD"],
    ["18", "Through Stock Protection POD"],
    ["19", "Except through POD"],
    ["20", "Except to some subscription services"],
    ["21", "Subscription service exclusive"],
    ["22", "To education only"],
    ["23", "Except to education"],
    ["99", "No restrictions on sales"],
  ]);

  // List 72 — Thesis type
  _lists[72] = new Map([
    ["01", "Habilitationsschrift"],
    ["02", "Dissertationsschrift"],
    ["03", "Staatsexamensarbeit"],
    ["04", "Magisterarbeit"],
    ["05", "Diplomarbeit"],
    ["06", "Bachelorarbeit"],
    ["07", "Masterarbeit"],
  ]);

  // List 73 — Website role
  _lists[73] = new Map([
    ["00", "Unspecified, see website description"],
    ["01", "Publisher’s corporate website"],
    ["02", "Publisher’s website for a specified work"],
    ["03", "Online hosting service home page"],
    ["04", "Journal home page"],
    ["05", "Online resource ‘available content’ page"],
    ["06", "Contributor’s own website"],
    ["07", "Publisher’s website relating to specified contributor"],
    ["08", "Other publisher’s website relating to specified contributor"],
    ["09", "Third-party website relating to specified contributor"],
    ["10", "Contributor’s own website for specified work"],
    ["11", "Other publisher’s website relating to specified work"],
    ["12", "Third-party website relating to specified work"],
    ["13", "Contributor’s own website for group or series of works"],
    ["14", "Publisher’s website relating to group or series of works"],
    ["15", "Other publisher’s website relating to group or series of works"],
    ["16", "Third-party website relating to group or series of works (eg a fan site)"],
    ["17", "Publisher’s B2B website"],
    ["18", "Publisher’s B2C website"],
    ["23", "Author blog"],
    ["24", "Web page for author presentation / commentary"],
    ["25", "Web page for author interview"],
    ["26", "Web page for author reading"],
    ["27", "Web page for cover material"],
    ["28", "Web page for sample content"],
    ["29", "Web page for full content"],
    ["30", "Web page for other commentary / discussion"],
    ["31", "Transfer-URL"],
    ["32", "DOI Website Link"],
    ["33", "Supplier’s corporate website"],
    ["34", "Supplier’s B2B website"],
    ["35", "Supplier’s B2C website"],
    ["36", "Supplier’s website for a specified work"],
    ["37", "Supplier’s B2B website for a specified work"],
    ["38", "Supplier’s B2C website for a specified work"],
    ["39", "Supplier’s website for a group or series of works"],
    ["40", "URL of full metadata description"],
    ["41", "Social networking URL for specific work or product"],
    ["42", "Author’s social networking URL"],
    ["43", "Publisher’s social networking URL"],
    ["44", "Social networking URL for specific article, chapter or content item"],
    ["45", "Publisher’s or third party website for permissions requests"],
    ["46", "Publisher’s or third party website for privacy statement"],
    ["47", "Publisher’s website for digital preservation"],
    ["48", "Third-party website for digital preservation"],
    ["49", "Product website for environmental responsibility statement"],
    ["50", "Organization’s website for environmental responsibility statement"],
    ["51", "Legal deposit website for digital preservation"],
    ["52", "Publisher’s or third party contact form"],
    ["53", "Organization’s website for corporate social responsibility"],
    ["54", "Website for Indigenous statement or resource"],
    ["55", "Link to third-party certification of absence of AI-generated content"],
    ["56", "Link to first-party declaration of absence of AI-generated content"],
  ]);

  // List 74 — Language – based on ISO 639-2/B
  _lists[74] = new Map([
    ["abk", "Abkhazian"],
    ["ace", "Achinese"],
    ["ach", "Acoli"],
    ["ada", "Adangme"],
    ["ady", "Adygei; Adyghe"],
    ["aar", "Afar"],
    ["afh", "Afrihili"],
    ["afr", "Afrikaans"],
    ["afa", "Afro-Asiatic languages"],
    ["ain", "Ainu"],
    ["aka", "Akan"],
    ["akk", "Akkadian"],
    ["alb", "Albanian"],
    ["ale", "Aleut"],
    ["alg", "Algonquian languages"],
    ["alq", "Algonquin"],
    ["tut", "Altaic languages"],
    ["amh", "Amharic"],
    ["anp", "Angika"],
    ["apa", "Apache languages"],
    ["ara", "Arabic"],
    ["arg", "Aragonese"],
    ["qar", "Aranés"],
    ["arp", "Arapaho"],
    ["arw", "Arawak"],
    ["arm", "Armenian"],
    ["rup", "Aromanian; Arumanian; Macedo-Romanian"],
    ["art", "Artificial languages"],
    ["asm", "Assamese"],
    ["ast", "Asturian; Bable; Leonese; Asturleonese"],
    ["ath", "Athapascan languages"],
    ["atj", "Atikamekw"],
    ["aus", "Australian languages"],
    ["aav", "Austro-Asiatic languages"],
    ["map", "Austronesian languages"],
    ["ava", "Avaric"],
    ["ave", "Avestan"],
    ["awa", "Awadhi"],
    ["aym", "Aymara"],
    ["aze", "Azerbaijani"],
    ["ban", "Balinese"],
    ["bat", "Baltic languages"],
    ["bal", "Baluchi"],
    ["bam", "Bambara"],
    ["bai", "Bamileke languages"],
    ["bad", "Banda languages"],
    ["bnt", "Bantu languages"],
    ["bas", "Basa"],
    ["bak", "Bashkir"],
    ["baq", "Basque"],
    ["btk", "Batak languages"],
    ["bej", "Beja; Bedawiyet"],
    ["bel", "Belarusian"],
    ["bem", "Bemba"],
    ["ben", "Bengali"],
    ["ber", "Berber languages"],
    ["bho", "Bhojpuri"],
    ["bih", "Bihari languages"],
    ["bik", "Bikol"],
    ["bin", "Bini; Edo"],
    ["bis", "Bislama"],
    ["byn", "Blin; Bilin"],
    ["zbl", "Blissymbols; Blissymbolics; Bliss"],
    ["brx", "Bodo (India)"],
    ["bos", "Bosnian"],
    ["bra", "Braj"],
    ["bre", "Breton"],
    ["bug", "Buginese"],
    ["bul", "Bulgarian"],
    ["bum", "Bulu (Cameroon)"],
    ["bua", "Buriat"],
    ["bur", "Burmese"],
    ["cad", "Caddo"],
    ["sro", "Campidanese"],
    ["yue", "Cantonese"],
    ["cat", "Catalan"],
    ["cau", "Caucasian languages"],
    ["cay", "Cayuga"],
    ["ceb", "Cebuano"],
    ["cel", "Celtic languages"],
    ["cai", "Central American Indian languages"],
    ["tzm", "Central Atlas Tamazight"],
    ["khm", "Central Khmer"],
    ["ckb", "Central Kurdish (Sorani)"],
    ["chg", "Chagatai"],
    ["cmc", "Chamic languages"],
    ["cha", "Chamorro"],
    ["che", "Chechen"],
    ["chr", "Cherokee"],
    ["chy", "Cheyenne"],
    ["chb", "Chibcha"],
    ["nya", "Chichewa; Chewa; Nyanja"],
    ["cic", "Chickasaw"],
    ["chi", "Chinese"],
    ["chn", "Chinook jargon"],
    ["chp", "Chipewyan; Dene Suline"],
    ["cho", "Choctaw"],
    ["chu", "Church Slavic; Old Slavonic; Church Slavonic; Old Bulgarian; Old Church Slavonic"],
    ["chk", "Chuukese (Truk)"],
    ["chv", "Chuvash"],
    ["nwc", "Classical Newari; Old Newari; Classical Nepal Bhasa"],
    ["syc", "Classical Syriac"],
    ["cop", "Coptic"],
    ["cor", "Cornish"],
    ["cos", "Corsican"],
    ["cre", "Cree"],
    ["mus", "Creek"],
    ["crp", "Creoles and pidgins"],
    ["cpe", "Creoles and pidgins, English-based"],
    ["cpf", "Creoles and pidgins, French-based"],
    ["cpp", "Creoles and pidgins, Portuguese-based"],
    ["crh", "Crimean Turkish; Crimean Tatar"],
    ["hrv", "Croatian"],
    ["scr", "Croatian"],
    ["cus", "Cushitic languages"],
    ["cze", "Czech"],
    ["dak", "Dakota"],
    ["dan", "Danish"],
    ["dar", "Dargwa"],
    ["prs", "Dari; Afghan Persian"],
    ["del", "Delaware"],
    ["din", "Dinka"],
    ["div", "Divehi; Dhivehi; Maldivian"],
    ["doi", "Dogri"],
    ["dgr", "Dogrib"],
    ["dra", "Dravidian languages"],
    ["dua", "Duala"],
    ["dum", "Dutch, Middle (ca. 1050-1350)"],
    ["dut", "Dutch; Flemish"],
    ["dyu", "Dyula"],
    ["dzo", "Dzongkha"],
    ["ike", "Eastern Canadian Inuktitut"],
    ["frs", "Eastern Frisian"],
    ["efi", "Efik"],
    ["egy", "Egyptian (Ancient)"],
    ["eka", "Ekajuk"],
    ["elx", "Elamite"],
    ["egl", "Emilian"],
    ["eng", "English"],
    ["enm", "English, Middle (1100-1500)"],
    ["ang", "English, Old (ca. 450-1100)"],
    ["myv", "Erzya"],
    ["esx", "Eskaleut languages"],
    ["epo", "Esperanto"],
    ["est", "Estonian"],
    ["gez", "Ethiopic (Ge’ez)"],
    ["ewe", "Ewe"],
    ["ewo", "Ewondo"],
    ["fan", "Fang"],
    ["fat", "Fanti"],
    ["fao", "Faroese"],
    ["fij", "Fijian"],
    ["fil", "Filipino; Pilipino"],
    ["fin", "Finnish"],
    ["fiu", "Finno-Ugrian languages"],
    ["fon", "Fon"],
    ["fre", "French"],
    ["frm", "French, Middle (ca. 1400-1600)"],
    ["fro", "French, Old (ca. 842-1400)"],
    ["fur", "Friulian"],
    ["ful", "Fulah"],
    ["gaa", "Gã"],
    ["car", "Galibi Carib"],
    ["glg", "Galician"],
    ["qgl", "Gallo"],
    ["sdn", "Gallurese"],
    ["lug", "Ganda"],
    ["grt", "Garo"],
    ["gay", "Gayo"],
    ["gba", "Gbaya"],
    ["geo", "Georgian"],
    ["ger", "German"],
    ["gmh", "German, Middle High (ca. 1050-1500)"],
    ["goh", "German, Old High (ca. 750-1050)"],
    ["gem", "Germanic languages"],
    ["gil", "Gilbertese"],
    ["git", "Gitxsan"],
    ["gon", "Gondi"],
    ["gor", "Gorontalo"],
    ["got", "Gothic"],
    ["grb", "Grebo"],
    ["grc", "Greek, Ancient (to 1453)"],
    ["gre", "Greek, Modern (1453-)"],
    ["grn", "Guarani"],
    ["nrf", "Guernésiais, Jèrriais"],
    ["guj", "Gujarati"],
    ["gju", "Gujari"],
    ["gwi", "Gwich’in"],
    ["hai", "Haida"],
    ["hat", "Haitian; Haitian Creole"],
    ["hau", "Hausa"],
    ["haw", "Hawaiian"],
    ["heb", "Hebrew"],
    ["her", "Herero"],
    ["hil", "Hiligaynon"],
    ["him", "Himachali languages; Western Pahari languages"],
    ["hin", "Hindi"],
    ["hmo", "Hiri Motu"],
    ["hit", "Hittite"],
    ["hmx", "Hmong-Mien languages"],
    ["hmn", "Hmong; Mong"],
    ["hop", "Hopi"],
    ["hun", "Hungarian"],
    ["hup", "Hupa"],
    ["iba", "Iban"],
    ["ice", "Icelandic"],
    ["ido", "Ido"],
    ["ibo", "Igbo"],
    ["ijo", "Ijo languages"],
    ["ilo", "Iloko"],
    ["smn", "Inari Sami"],
    ["inc", "Indic languages"],
    ["ine", "Indo-European languages"],
    ["ind", "Indonesian"],
    ["inh", "Ingush"],
    ["moe", "Innu, Montagnais"],
    ["ina", "Interlingua (International Auxiliary Language Association)"],
    ["ile", "Interlingue; Occidental"],
    ["ikt", "Inuinnaqtun"],
    ["iku", "Inuktitut"],
    ["ipk", "Inupiaq"],
    ["qiv", "Inuvialuktun"],
    ["ira", "Iranian languages"],
    ["pes", "Iranian Persian; Parsi"],
    ["gle", "Irish"],
    ["mga", "Irish, Middle (ca. 1100-1550)"],
    ["sga", "Irish, Old (to 1100)"],
    ["iro", "Iroquoian languages"],
    ["ita", "Italian"],
    ["jpn", "Japanese"],
    ["jpx", "Japanese languages"],
    ["jav", "Javanese"],
    ["jow", "Jowulu"],
    ["jrb", "Judeo-Arabic"],
    ["jpr", "Judeo-Persian"],
    ["kbd", "Kabardian"],
    ["kbp", "Kabiyè"],
    ["kab", "Kabyle"],
    ["kac", "Kachin; Jingpho"],
    ["kal", "Kalâtdlisut; Greenlandic"],
    ["xal", "Kalmyk; Oirat"],
    ["kam", "Kamba"],
    ["kan", "Kannada"],
    ["kau", "Kanuri"],
    ["kaa", "Kara-Kalpak"],
    ["krc", "Karachay-Balkar"],
    ["kdr", "Karaim"],
    ["krl", "Karelian"],
    ["kar", "Karen languages"],
    ["kas", "Kashmiri"],
    ["csb", "Kashubian"],
    ["kaw", "Kawi"],
    ["kaz", "Kazakh"],
    ["kha", "Khasi"],
    ["khi", "Khoisan languages"],
    ["kho", "Khotanese; Sakan"],
    ["xuu", "Khwedam, Kxoe"],
    ["kik", "Kikuyu; Gikuyu"],
    ["kmb", "Kimbundu"],
    ["kin", "Kinyarwanda"],
    ["kir", "Kirghiz; Kyrgyz"],
    ["kqs", "Kissi, Northern"],
    ["kss", "Kissi, Southern"],
    ["tlh", "Klingon; tlhIngan-Hol"],
    ["kom", "Komi"],
    ["kon", "Kongo"],
    ["kok", "Konkani"],
    ["kor", "Korean"],
    ["kpe", "Kpelle"],
    ["kro", "Kru languages"],
    ["kua", "Kuanyama; Kwanyama"],
    ["kum", "Kumyk"],
    ["kur", "Kurdish"],
    ["kru", "Kurukh"],
    ["kos", "Kusaiean (Caroline Islands)"],
    ["kut", "Kutenai"],
    ["fkv", "Kvensk"],
    ["kwk", "Kwakiutl"],
    ["lad", "Ladino"],
    ["lah", "Lahnda"],
    ["lkt", "Lakota"],
    ["lam", "Lamba"],
    ["day", "Land Dayak languages"],
    ["lao", "Lao"],
    ["lat", "Latin"],
    ["lav", "Latvian"],
    ["qlk", "Lemko"],
    ["lez", "Lezgian"],
    ["lij", "Ligurian"],
    ["lim", "Limbergan; Limburger; Limburgish"],
    ["lin", "Lingala"],
    ["lit", "Lithuanian"],
    ["jbo", "Lojban"],
    ["lom", "Loma (Liberia)"],
    ["lmo", "Lombard"],
    ["nds", "Low German; Low Saxon"],
    ["dsb", "Lower Sorbian"],
    ["loz", "Lozi"],
    ["lub", "Luba-Katanga"],
    ["lua", "Luba-Lulua"],
    ["lui", "Luiseño"],
    ["smj", "Lule Sami"],
    ["lun", "Lunda"],
    ["luo", "Luo (Kenya and Tanzania)"],
    ["lus", "Lushai"],
    ["ltz", "Luxembourgish; Letzeburgesch"],
    ["mac", "Macedonian"],
    ["mad", "Madurese"],
    ["mag", "Magahi"],
    ["mai", "Maithili"],
    ["mak", "Makasar"],
    ["mlg", "Malagasy"],
    ["may", "Malay"],
    ["mal", "Malayalam"],
    ["poz", "Malayo-Polynesian languages"],
    ["mlt", "Maltese"],
    ["mnc", "Manchu"],
    ["mdr", "Mandar"],
    ["cmn", "Mandarin"],
    ["man", "Mandingo"],
    ["mni", "Manipuri"],
    ["mno", "Manobo languages"],
    ["glv", "Manx"],
    ["swb", "Maore Comorian"],
    ["mao", "Maori"],
    ["arn", "Mapudungun; Mapuche"],
    ["mar", "Marathi"],
    ["chm", "Mari"],
    ["mah", "Marshallese"],
    ["mwr", "Marwari"],
    ["mas", "Masai"],
    ["myn", "Mayan languages"],
    ["fit", "Meänkieli / Tornedalen Finnish"],
    ["men", "Mende"],
    ["mic", "Mi’kmaq; Micmac; Mi’gmaw"],
    ["crg", "Michif"],
    ["gml", "Middle Low German"],
    ["min", "Minangkabau"],
    ["mwl", "Mirandese"],
    ["moh", "Mohawk"],
    ["mdf", "Moksha"],
    ["mol", "Moldavian; Moldovan"],
    ["mkh", "Mon-Khmer languages"],
    ["lol", "Mongo-Nkundu"],
    ["mon", "Mongolian"],
    ["cnr", "Montenegrin"],
    ["mos", "Mooré; Mossi"],
    ["crm", "Moose Cree"],
    ["mun", "Munda languages"],
    ["mwf", "Murrinh-Patha"],
    ["nqo", "N’Ko"],
    ["nah", "Nahuatl languages"],
    ["nsk", "Naskapi"],
    ["nau", "Nauruan"],
    ["nav", "Navajo; Navaho"],
    ["nde", "Ndebele, North"],
    ["nbl", "Ndebele, South"],
    ["ndo", "Ndonga"],
    ["nap", "Neapolitan"],
    ["nep", "Nepali"],
    ["qls", "Neutral Latin American Spanish"],
    ["new", "Newari; Nepal Bhasa"],
    ["nia", "Nias"],
    ["nic", "Niger-Kordofanian languages"],
    ["ssa", "Nilo-Saharan languages"],
    ["niu", "Niuean"],
    ["nog", "Nogai"],
    ["qnf", "Norman French"],
    ["nai", "North American Indian languages"],
    ["crl", "Northern East Cree"],
    ["frr", "Northern Frisian"],
    ["sme", "Northern Sami"],
    ["nor", "Norwegian"],
    ["nob", "Norwegian Bokmål"],
    ["nno", "Norwegian Nynorsk"],
    ["nub", "Nubian languages"],
    ["hux", "Nüpode Huitoto (Nipóde)"],
    ["nym", "Nyamwezi"],
    ["nyn", "Nyankole"],
    ["nyo", "Nyoro"],
    ["nzi", "Nzima"],
    ["oci", "Occitan (post-1500)"],
    ["arc", "Official Aramaic; Imperial Aramaic (700-300 BCE)"],
    ["oji", "Ojibwa"],
    ["oka", "Okanagan; Nsyilxcn / Nsyilxcən"],
    ["odt", "Old Dutch / Old Low Franconian (ca. 400–1050)"],
    ["non", "Old Norse"],
    ["peo", "Old Persian (ca. 600-400 B.C.)"],
    ["ood", "O’odham"],
    ["ori", "Oriya"],
    ["orm", "Oromo"],
    ["osa", "Osage"],
    ["oss", "Ossetian; Ossetic"],
    ["omq", "Oto-Manguean languages"],
    ["oto", "Otomian languages"],
    ["pal", "Pahlavi"],
    ["pri", "Paicî"],
    ["pau", "Palauan"],
    ["pli", "Pali"],
    ["pam", "Pampanga; Kapampangan"],
    ["pag", "Pangasinan"],
    ["pan", "Panjabi; Punjabi"],
    ["pap", "Papiamento"],
    ["paa", "Papuan languages"],
    ["nso", "Pedi; Sepedi; Northern Sotho"],
    ["per", "Persian"],
    ["phi", "Philippine languages"],
    ["phn", "Phoenician"],
    ["pcd", "Picard"],
    ["qsp", "Picture Communication Symbols (PCS)"],
    ["pms", "Piedmontese"],
    ["crk", "Plains Cree"],
    ["pol", "Polish"],
    ["pon", "Ponapeian"],
    ["por", "Portuguese"],
    ["pra", "Prakrit languages"],
    ["pro", "Provençal, Old (to 1500); Occitan, Old (to 1500)"],
    ["fuf", "Pular"],
    ["pus", "Pushto; Pashto"],
    ["que", "Quechua"],
    ["raj", "Rajasthani"],
    ["rjs", "Rajbanshi"],
    ["rkt", "Rangpuri"],
    ["rap", "Rapanui"],
    ["rar", "Rarotongan; Cook Islands Maori"],
    ["rcf", "Réunion Creole French"],
    ["rgn", "Romagnol"],
    ["roa", "Romance languages"],
    ["rum", "Romanian"],
    ["roh", "Romansh"],
    ["rom", "Romany"],
    ["run", "Rundi"],
    ["rus", "Russian"],
    ["sal", "Salishan languages"],
    ["sam", "Samaritan Aramaic"],
    ["smi", "Sami languages"],
    ["smo", "Samoan"],
    ["sad", "Sandawe"],
    ["sag", "Sango"],
    ["san", "Sanskrit"],
    ["sat", "Santali"],
    ["srm", "Saramaccan"],
    ["srd", "Sardinian"],
    ["sas", "Sasak"],
    ["sdc", "Sassarese"],
    ["sco", "Scots"],
    ["gla", "Scottish Gaelic"],
    ["sel", "Selkup"],
    ["sem", "Semitic languages"],
    ["scc", "Serbian"],
    ["srp", "Serbian"],
    ["srr", "Serer"],
    ["ojs", "Severn Ojibwa"],
    ["shn", "Shan"],
    ["sna", "Shona"],
    ["shs", "Shuswap"],
    ["iii", "Sichuan Yi; Nuosu"],
    ["scn", "Sicilian"],
    ["sid", "Sidamo"],
    ["sgn", "Sign languages"],
    ["bla", "Siksika"],
    ["szl", "Silesian"],
    ["snd", "Sindhi"],
    ["sin", "Sinhala; Sinhalese"],
    ["zhx", "Sinitic languages"],
    ["sit", "Sino-Tibetan languages"],
    ["sio", "Siouan languages"],
    ["sms", "Skolt Sami"],
    ["den", "Slave (Athapascan)"],
    ["sla", "Slavic languages"],
    ["slo", "Slovak"],
    ["slv", "Slovenian"],
    ["sog", "Sogdian"],
    ["som", "Somali"],
    ["son", "Songhai languages"],
    ["snk", "Soninke"],
    ["wen", "Sorbian languages"],
    ["sot", "Sotho; Sesotho"],
    ["sai", "South American Indian languages"],
    ["alt", "Southern Altai"],
    ["crj", "Southern East Cree"],
    ["sma", "Southern Sami"],
    ["spa", "Spanish"],
    ["srn", "Sranan Tongo"],
    ["zgh", "Standard Moroccan Tamazight"],
    ["sto", "Stoney"],
    ["suk", "Sukuma"],
    ["sux", "Sumerian"],
    ["sun", "Sundanese"],
    ["sus", "Susu"],
    ["swa", "Swahili"],
    ["csw", "Swampy Cree"],
    ["ssw", "Swazi; Swati"],
    ["swe", "Swedish"],
    ["gsw", "Swiss German; Alemannic; Alsatian"],
    ["syr", "Syriac"],
    ["tgl", "Tagalog"],
    ["tah", "Tahitian"],
    ["tai", "Tai languages"],
    ["tgk", "Tajik; Tajiki Persian"],
    ["tmh", "Tamashek"],
    ["tam", "Tamil"],
    ["shi", "Tashelhit; Shilha"],
    ["tat", "Tatar"],
    ["tel", "Telugu"],
    ["tem", "Temne; Timne"],
    ["ter", "Terena"],
    ["tet", "Tetum"],
    ["tha", "Thai"],
    ["thp", "Thompson"],
    ["tib", "Tibetan"],
    ["tig", "Tigré"],
    ["tir", "Tigrinya"],
    ["tiv", "Tiv"],
    ["tli", "Tlingit"],
    ["tpi", "Tok Pisin"],
    ["tkl", "Tokelauan"],
    ["tod", "Toma"],
    ["tog", "Tonga (Nyasa)"],
    ["ton", "Tongan"],
    ["rmg", "Traveler Norwegian"],
    ["tsd", "Tsakonian"],
    ["tsi", "Tsimshian"],
    ["tso", "Tsonga"],
    ["tsn", "Tswana"],
    ["tum", "Tumbuka"],
    ["tup", "Tupi languages"],
    ["tur", "Turkish"],
    ["ota", "Turkish, Ottoman"],
    ["tuk", "Turkmen"],
    ["tvl", "Tuvaluan"],
    ["tyv", "Tuvinian"],
    ["twi", "Twi"],
    ["tzo", "Tzotzil"],
    ["udm", "Udmurt"],
    ["uga", "Ugaritic"],
    ["uig", "Uighur; Uyghur"],
    ["ukr", "Ukrainian"],
    ["umb", "Umbundu"],
    ["hsb", "Upper Sorbian"],
    ["urj", "Uralic languages"],
    ["urd", "Urdu"],
    ["uzb", "Uzbek"],
    ["vai", "Vai"],
    ["qav", "Valencian"],
    ["ven", "Venda"],
    ["vec", "Venetian / Venetan"],
    ["vie", "Vietnamese"],
    ["vol", "Volapük"],
    ["vot", "Votic"],
    ["wak", "Wakashan languages"],
    ["wls", "Wallisian"],
    ["wln", "Walloon"],
    ["cou", "Wamey"],
    ["war", "Waray"],
    ["was", "Washo"],
    ["guc", "Wayuu"],
    ["wel", "Welsh"],
    ["fry", "Western Frisian"],
    ["qsw", "Widgit symbols"],
    ["wal", "Wolaitta; Wolaytta"],
    ["wol", "Wolof"],
    ["cwd", "Woods Cree"],
    ["wym", "Wymysorys"],
    ["xho", "Xhosa"],
    ["sah", "Yakut"],
    ["yao", "Yao"],
    ["yap", "Yapese"],
    ["yid", "Yiddish"],
    ["yor", "Yoruba"],
    ["ypk", "Yupik languages"],
    ["znd", "Zande languages"],
    ["zap", "Zapotec"],
    ["zza", "Zaza; Dimili; Dimli; Kirdki; Kirmanjki; Zazaki"],
    ["zen", "Zenaga"],
    ["zha", "Zhuang; Chuang"],
    ["zul", "Zulu"],
    ["zun", "Zuni"],
    ["mis", "Uncoded language"],
    ["und", "Undetermined language"],
    ["mul", "Multiple languages"],
    ["zxx", "No linguistic content"],
  ]);

  // List 76 — DVD region
  _lists[76] = new Map([
    ["0", "All regions"],
    ["1", "DVD region 1"],
    ["2", "DVD region 2"],
    ["3", "DVD region 3"],
    ["4", "DVD region 4"],
    ["5", "DVD region 5"],
    ["6", "DVD region 6"],
    ["7", "DVD region 7"],
    ["8", "DVD region 8"],
    ["A", "Blu-Ray region A"],
    ["B", "Blu-Ray region B"],
    ["C", "Blu-Ray region C"],
  ]);

  // List 77 — US and Canada (excluding Québec) school or college grade
  _lists[77] = new Map([
    ["P", "Preschool"],
    ["K", "Kindergarten"],
    ["1", "First Grade"],
    ["2", "Second Grade"],
    ["3", "Third Grade"],
    ["4", "Fourth Grade"],
    ["5", "Fifth Grade"],
    ["6", "Sixth Grade"],
    ["7", "Seventh Grade"],
    ["8", "Eighth Grade"],
    ["9", "Ninth Grade"],
    ["10", "Tenth Grade"],
    ["11", "Eleventh Grade"],
    ["12", "Twelfth Grade"],
    ["13", "College Freshman"],
    ["14", "College Sophomore"],
    ["15", "College Junior"],
    ["16", "College Senior"],
    ["17", "College Graduate Student"],
  ]);

  // List 79 — Product form feature type
  _lists[79] = new Map([
    ["01", "Color of cover"],
    ["26", "Color of spine"],
    ["27", "Color of foil (on cover)"],
    ["59", "Color of foil (on jacket)"],
    ["02", "Color of page edges"],
    ["55", "Color of endpapers"],
    ["57", "Color of ribbon marker(s)"],
    ["58", "Color of head and tail bands"],
    ["03", "Text font"],
    ["04", "Special cover material"],
    ["05", "DVD region"],
    ["06", "Operating system requirements"],
    ["07", "Other system requirements"],
    ["08", "‘Point and listen’ device compatibility"],
    ["09", "E-publication accessibility detail"],
    ["10", "E-publication format version"],
    ["12", "US CPSIA or other international hazard warning"],
    ["13", "EU General Product Safety Regulation and Toy Safety Directive hazard warning"],
    ["14", "IATA Dangerous Goods warning"],
    ["15", "E-publication format version code"],
    ["16", "E-publication format validator version"],
    ["17", "‘Point and watch’ device / app compatibility"],
    ["18", "E-publication authentication and access control"],
    ["19", "Battery type and safety"],
    ["20", "Battery capacity"],
    ["21", "Dangerous goods"],
    ["22", "Game pieces"],
    ["23", "Game players"],
    ["24", "Game play time"],
    ["25", "Personal data requirements"],
    ["30", "Not FSC or PEFC certified"],
    ["31", "FSC certified – pure"],
    ["32", "FSC certified – mixed sources"],
    ["33", "FSC certified – recycled"],
    ["34", "PEFC certified"],
    ["35", "PEFC recycled"],
    ["36", "FSC or PEFC certified Pre- and Post-Consumer Waste (PCW) percentage"],
    ["37", "Claimed Pre- and Post-Consumer Waste (PCW) percentage"],
    ["38", "‘Green’ inks"],
    ["39", "‘Green’ adhesives"],
    ["40", "Paper produced by ‘green’ technology"],
    ["41", "Carbon / GHG emission certification scheme"],
    ["42", "Carbon / GHG emission certification / license number"],
    ["43", "Carbon / GHG emission certification URL"],
    ["44", "Carbon / GHG Scope 3 certified Carbon dioxide equivalent emission"],
    ["45", "Carbon / GHG Scope 2 certified Carbon dioxide equivalent emission"],
    ["46", "Carbon / GHG Scope 1 certified Carbon dioxide equivalent emission"],
    ["47", "EUDR raw material location (interior or full product)"],
    ["48", "EUDR raw material location (binding)"],
    ["49", "EUDR raw material location (other)"],
    ["50", "EUDR due diligence reference"],
    ["51", "EUDR associated due diligence references"],
    ["52", "EUDR compliant with national law attestation"],
    ["53", "EUDR deforestation-free attestation"],
    ["54", "EUDR stock present attestation"],
    ["56", "EUDR beyond scope attestation"],
    ["60", "EUDR raw material location (interior or full product) URL"],
    ["61", "EUDR raw material location (binding) URL"],
    ["62", "EUDR raw material location (other) URL"],
    ["63", "EUDR raw material location (interior or full product) Postal address"],
    ["64", "EUDR raw material location (binding) Postal address"],
    ["65", "EUDR raw material location (other) Postal address"],
    ["66", "Map projection"],
  ]);

  // List 80 — Product packaging type
  _lists[80] = new Map([
    ["00", "No outer packaging"],
    ["01", "Slip-sleeve"],
    ["02", "Clamshell"],
    ["03", "Keep case"],
    ["05", "Jewel case"],
    ["06", "Digipak"],
    ["08", "Shrink-wrapped (biodegradable)"],
    ["09", "In box (with lid)"],
    ["10", "Slip-cased"],
    ["11", "Slip-cased set"],
    ["12", "Tube"],
    ["13", "Binder"],
    ["14", "In wallet or folder"],
    ["15", "Long triangular package"],
    ["16", "Long square package"],
    ["17", "Softbox (for DVD)"],
    ["18", "Pouch"],
    ["19", "Rigid plastic case"],
    ["20", "Cardboard case"],
    ["21", "Shrink-wrapped"],
    ["22", "Blister pack"],
    ["23", "Carry case"],
    ["24", "In tin"],
    ["25", "With browse-prevention tape"],
  ]);

  // List 81 — Product content type
  _lists[81] = new Map([
    ["10", "Text"],
    ["15", "Extensive links between internal content"],
    ["14", "Extensive links to external content"],
    ["51", "Links to external interactive content"],
    ["16", "Additional text not part of main content"],
    ["45", "Text within images"],
    ["41", "Additional eye-readable links to external content"],
    ["17", "Promotional text for other book product"],
    ["11", "Musical notation"],
    ["07", "Still images / graphics"],
    ["18", "Photographs"],
    ["19", "Figures, diagrams, charts, graphs"],
    ["20", "Additional images / graphics not part of main work"],
    ["12", "Maps and / or other cartographic content"],
    ["47", "Chemical content"],
    ["48", "Mathematical content"],
    ["46", "Decorative images or graphics"],
    ["42", "Assessment material"],
    ["01", "Audiobook"],
    ["52", "Audiobook – dual"],
    ["53", "Audiobook – duet"],
    ["02", "Performance – spoken word"],
    ["13", "Other speech content"],
    ["03", "Music recording"],
    ["04", "Other audio"],
    ["49", "Images of text"],
    ["21", "Partial performance – spoken word"],
    ["22", "Additional audio content not part of main content"],
    ["23", "Promotional audio for other book product"],
    ["06", "Video"],
    ["26", "Video recording of a reading"],
    ["50", "Video content without audio"],
    ["27", "Performance – visual"],
    ["24", "Animated / interactive illustrations"],
    ["25", "Narrative animation"],
    ["28", "Other video"],
    ["29", "Partial performance – video"],
    ["30", "Additional video content not part of main work"],
    ["31", "Promotional video for other book product"],
    ["05", "Game / Puzzle"],
    ["32", "Contest"],
    ["08", "Software"],
    ["09", "Data"],
    ["33", "Data set plus software"],
    ["34", "Blank pages or spaces"],
    ["35", "Advertising content"],
    ["37", "Advertising – first party"],
    ["36", "Advertising – coupons"],
    ["38", "Advertising – third party display"],
    ["39", "Advertising – third party textual"],
    ["40", "Scripting"],
    ["43", "Scripted pop-ups"],
    ["44", "Sequential art"],
  ]);

  // List 82 — Bible contents
  _lists[82] = new Map([
    ["AP", "Apocrypha (Catholic canon)"],
    ["AQ", "Apocrypha (canon unspecified)"],
    ["AX", "Additional Apocryphal texts: Greek Orthodox canon"],
    ["AY", "Additional Apocryphal texts: Slavonic Orthodox canon"],
    ["AZ", "Additional Apocryphal texts"],
    ["GA", "General canon with Apocrypha (Catholic canon)"],
    ["GC", "General canon with Apocryphal texts (canon unspecified)"],
    ["GE", "General canon"],
    ["GS", "Gospels"],
    ["OT", "Old Testament"],
    ["NT", "New Testament"],
    ["NP", "New Testament with Psalms and Proverbs"],
    ["PE", "Paul’s Epistles"],
    ["PP", "Psalms and Proverbs"],
    ["PS", "Psalms"],
    ["PT", "Pentateuch"],
    ["ZZ", "Other portions"],
  ]);

  // List 83 — Bible version
  _lists[83] = new Map([
    ["ALV", "Alberto Vaccari"],
    ["AMP", "Amplified"],
    ["ANM", "Antonio Martini"],
    ["ASV", "American Standard"],
    ["BLA", "Biblia de las Americas"],
    ["BLB", "Nueva Biblia de las Americas"],
    ["BDA", "Biblia de América"],
    ["CEB", "Common English Bible"],
    ["CEI", "Conferenza Episcopale Italiana"],
    ["CEN", "Conferenza Episcopale Italiana 2008"],
    ["CEV", "Contemporary English"],
    ["CNC", "Concordata"],
    ["DDI", "Diodati"],
    ["DDN", "Nuova Diodati"],
    ["DOU", "Douay–Rheims"],
    ["EIN", "Einheitsübersetzung"],
    ["ESV", "English Standard"],
    ["FBB", "Biblia (1776)"],
    ["FRA", "Raamattu (1933/1938)"],
    ["FRK", "Raamattu kansalle"],
    ["FRM", "Raamattu (1992)"],
    ["GDW", "God’s Word"],
    ["GEN", "Geneva"],
    ["GNB", "Good News"],
    ["GPR", "Galbiati, Penna, Rossano – UTET"],
    ["GRK", "Original Greek"],
    ["GRM", "Garofano, Rinaldi – Marietti"],
    ["HBR", "Original Hebrew"],
    ["HCS", "Christian Standard"],
    ["ICB", "International Children’s"],
    ["ILC", "Traduzione Interconfessionale in Lingua Corrente"],
    ["JER", "Jerusalem"],
    ["KJV", "King James"],
    ["KJT", "21st Century King James"],
    ["KJC", "King James Clássica"],
    ["LVB", "Living Bible"],
    ["LZZ", "Luzzi"],
    ["MSG", "Message Bible"],
    ["NAB", "New American"],
    ["NAR", "New American Revised"],
    ["NAS", "New American Standard"],
    ["NAU", "New American Standard, Updated"],
    ["NCB", "CTS New Catholic Bible"],
    ["NCJ", "St. Joseph New Catholic Bible"],
    ["NBA", "Bibelen 1895"],
    ["NBB", "Bibelen 1930"],
    ["NBC", "Bibelen 1938"],
    ["NBD", "Bibelen 1978-85"],
    ["NBE", "Bibelen 1978"],
    ["NBF", "Bibelen 1985"],
    ["NBG", "Norsk Bibel 88"],
    ["NBH", "Bibelen 1978-85 / rev. 2005"],
    ["NBI", "Bibelen 2011"],
    ["NBJ", "Norsk Bibel 88 / rev. 2007"],
    ["NBK", "Fauskanger 2015"],
    ["NBL", "Bibelen 2011 / rev. 2024"],
    ["NBP", "Pollestad 2023"],
    ["NCV", "New Century"],
    ["NEB", "New English"],
    ["NGO", "Bibelen Guds ord"],
    ["NIV", "New International"],
    ["NIR", "New International Reader’s"],
    ["NJB", "New Jerusalem"],
    ["NJR", "New Jerusalem Revised"],
    ["NKJ", "New King James"],
    ["NNK", "Bibelen, nynorsk"],
    ["NLV", "New Living"],
    ["NRS", "New Revised Standard"],
    ["NTV", "Nueva Traducción Viviente"],
    ["NVB", "Novissima Versione della Bibbia"],
    ["NVD", "Nueva Biblia al Dia"],
    ["NVI", "Nueva Version Internacional"],
    ["NWT", "New World Translation"],
    ["PHP", "New Testament in Modern English (Phillips)"],
    ["REB", "Revised English"],
    ["REV", "Revised Version"],
    ["RSV", "Revised Standard"],
    ["RSC", "Revised Standard Catholic"],
    ["RVL", "Reina–Valera"],
    ["SBB", "Bibel 2000"],
    ["SMK", "Bibelen, samisk"],
    ["TEV", "Today’s English"],
    ["TNI", "Today’s New International"],
    ["VUN", "Nova Vulgata"],
    ["ERV", "Easy to read version"],
    ["JEL", "Biblia de Jerusalén Latinoamericana"],
    ["BDP", "Bíblia do Peregrino"],
    ["JFA", "João Ferreira de Almeida (Primeira Versão)"],
    ["JFB", "João Ferreira de Almeida (Segunda Versão)"],
    ["JFC", "Almeida Revista e Emendada"],
    ["JFD", "Almeida Revista e Reformada"],
    ["JFE", "Almeida Revista e Correcta"],
    ["JFF", "Almeida Revista e Corrigida 1898"],
    ["JFG", "Almeida Revista e Corrigida na Grafia Simplificada"],
    ["JFH", "Almeida Revista e Corrigida 1949"],
    ["JFI", "Almeida Revista e Atualizada"],
    ["JFJ", "Almeida Revisada de Acordo com os Melhores Textos em Hebraico e Grego"],
    ["JFK", "Almeida Revista e Corrigida 1969"],
    ["JFL", "Almeida Revista e Atualizada, Segunda Edição"],
    ["JFM", "Almeida Revista e Corrigida, Nova Edição"],
    ["JFN", "Almeida Edição Contemporânea"],
    ["JFO", "Almeida Século 21"],
    ["JFP", "Almeida Corrigida e Fiel 1994"],
    ["JFQ", "Almeida Corrigida e Fiel 2011"],
    ["JFR", "Nova Almeida Atualizada"],
    ["TBB", "Tradução Brasileira"],
    ["TBC", "Tradução na Linguagem de Hoje"],
    ["TBD", "Nova Tradução na Linguagem de Hoje"],
    ["NVJ", "Nova Versão Internacional"],
    ["NVT", "Nova Versão Transformadora"],
    ["KJA", "King James Atualizada"],
    ["BCN", "Bíblia Sagrada Tradução Oficial da CNBB"],
    ["BMD", "Bíblia Mensagem de Deus"],
    ["TEB", "Tradução Ecumênica da Bíblia"],
    ["PAU", "A Bíblia Paulinas"],
    ["BAM", "Bible de Maredsous (Bíblia Ave-Maria)"],
    ["TOB", "Traduction Oecuménique de la Bible"],
    ["ZZZ", "Other"],
  ]);

  // List 84 — Study Bible type
  _lists[84] = new Map([
    ["CAM", "Cambridge Annotated"],
    ["LIF", "Life Application"],
    ["MAC", "Macarthur"],
    ["OXF", "Oxford Annotated"],
    ["NNT", "Studiebibel, Det Nye testamentet"],
    ["NOX", "New Oxford Annotated"],
    ["NSB", "Norsk studiebibel"],
    ["RYR", "Ryrie"],
    ["SCO", "Scofield"],
    ["SPR", "Spirit Filled"],
  ]);

  // List 85 — Bible purpose
  _lists[85] = new Map([
    ["AW", "Award"],
    ["BB", "Baby"],
    ["BR", "Bride"],
    ["CH", "Children’s"],
    ["CT", "Christening / Baptism gift"],
    ["CM", "Compact"],
    ["CF", "Confirmation"],
    ["CR", "Cross-reference"],
    ["DR", "Daily readings"],
    ["DV", "Devotional"],
    ["FM", "Family"],
    ["FC", "First communion"],
    ["GT", "General / Text"],
    ["GF", "Gift"],
    ["JN", "Journaling / notetaking"],
    ["LP", "Lectern / Pulpit"],
    ["MN", "Men’s"],
    ["OT", "Outreach"],
    ["PA", "Pastoral"],
    ["PW", "Pew"],
    ["PR", "Preaching"],
    ["PS", "Primary school"],
    ["RD", "Reader’s"],
    ["SC", "Scholarly"],
    ["SL", "Slimline"],
    ["ST", "Student"],
    ["SU", "Study"],
    ["WG", "Wedding gift"],
    ["WM", "Women’s"],
    ["YT", "Youth / Teen"],
  ]);

  // List 86 — Bible text organization
  _lists[86] = new Map([
    ["CHR", "Chronological"],
    ["CHA", "Chain reference"],
    ["INT", "Interlinear"],
    ["PAR", "Parallel"],
    ["STN", "Standard"],
  ]);

  // List 87 — Bible reference location
  _lists[87] = new Map([
    ["CCL", "Center column"],
    ["PGE", "Page end"],
    ["SID", "Side column"],
    ["VER", "Verse end"],
    ["UNK", "Unknown"],
    ["ZZZ", "Other"],
  ]);

  // List 89 — Religious text feature type
  _lists[89] = new Map([
    ["01", "Church season or activity"],
  ]);

  // List 90 — Religious text feature
  _lists[90] = new Map([
    ["01", "Academic year"],
    ["02", "Catechistic year"],
    ["03", "Liturgical year"],
    ["04", "Advent and Christmas"],
    ["05", "Blessings"],
    ["06", "Scholastic cycles"],
    ["07", "Confirmation and Holy Communion"],
    ["08", "Summer activities"],
    ["09", "Easter"],
    ["10", "Lent"],
    ["11", "Marian themes"],
  ]);

  // List 91 — Country – based on ISO 3166-1
  _lists[91] = new Map([
    ["AD", "Andorra"],
    ["AE", "United Arab Emirates"],
    ["AF", "Afghanistan"],
    ["AG", "Antigua and Barbuda"],
    ["AI", "Anguilla"],
    ["AL", "Albania"],
    ["AM", "Armenia"],
    ["AN", "Netherlands Antilles"],
    ["AO", "Angola"],
    ["AQ", "Antarctica"],
    ["AR", "Argentina"],
    ["AS", "American Samoa"],
    ["AT", "Austria"],
    ["AU", "Australia"],
    ["AW", "Aruba"],
    ["AX", "Åland Islands"],
    ["AZ", "Azerbaijan"],
    ["BA", "Bosnia and Herzegovina"],
    ["BB", "Barbados"],
    ["BD", "Bangladesh"],
    ["BE", "Belgium"],
    ["BF", "Burkina Faso"],
    ["BG", "Bulgaria"],
    ["BH", "Bahrain"],
    ["BI", "Burundi"],
    ["BJ", "Benin"],
    ["BL", "Saint Barthélemy"],
    ["BM", "Bermuda"],
    ["BN", "Brunei Darussalam"],
    ["BO", "Bolivia, Plurinational State of"],
    ["BQ", "Bonaire, Sint Eustatius and Saba"],
    ["BR", "Brazil"],
    ["BS", "Bahamas"],
    ["BT", "Bhutan"],
    ["BV", "Bouvet Island"],
    ["BW", "Botswana"],
    ["BY", "Belarus"],
    ["BZ", "Belize"],
    ["CA", "Canada"],
    ["CC", "Cocos (Keeling) Islands"],
    ["CD", "Congo, Democratic Republic of the"],
    ["CF", "Central African Republic"],
    ["CG", "Congo"],
    ["CH", "Switzerland"],
    ["CI", "Cote d’Ivoire"],
    ["CK", "Cook Islands"],
    ["CL", "Chile"],
    ["CM", "Cameroon"],
    ["CN", "China"],
    ["CO", "Colombia"],
    ["CR", "Costa Rica"],
    ["CS", "Serbia and Montenegro"],
    ["CU", "Cuba"],
    ["CV", "Cabo Verde"],
    ["CW", "Curaçao"],
    ["CX", "Christmas Island"],
    ["CY", "Cyprus"],
    ["CZ", "Czechia"],
    ["DE", "Germany"],
    ["DJ", "Djibouti"],
    ["DK", "Denmark"],
    ["DM", "Dominica"],
    ["DO", "Dominican Republic"],
    ["DZ", "Algeria"],
    ["EC", "Ecuador"],
    ["EE", "Estonia"],
    ["EG", "Egypt"],
    ["EH", "Western Sahara"],
    ["ER", "Eritrea"],
    ["ES", "Spain"],
    ["ET", "Ethiopia"],
    ["FI", "Finland"],
    ["FJ", "Fiji"],
    ["FK", "Falkland Islands (Malvinas)"],
    ["FM", "Micronesia, Federated States of"],
    ["FO", "Faroe Islands"],
    ["FR", "France"],
    ["GA", "Gabon"],
    ["GB", "United Kingdom"],
    ["GD", "Grenada"],
    ["GE", "Georgia"],
    ["GF", "French Guiana"],
    ["GG", "Guernsey"],
    ["GH", "Ghana"],
    ["GI", "Gibraltar"],
    ["GL", "Greenland"],
    ["GM", "Gambia"],
    ["GN", "Guinea"],
    ["GP", "Guadeloupe"],
    ["GQ", "Equatorial Guinea"],
    ["GR", "Greece"],
    ["GS", "South Georgia and the South Sandwich Islands"],
    ["GT", "Guatemala"],
    ["GU", "Guam"],
    ["GW", "Guinea-Bissau"],
    ["GY", "Guyana"],
    ["HK", "Hong Kong"],
    ["HM", "Heard Island and McDonald Islands"],
    ["HN", "Honduras"],
    ["HR", "Croatia"],
    ["HT", "Haiti"],
    ["HU", "Hungary"],
    ["ID", "Indonesia"],
    ["IE", "Ireland"],
    ["IL", "Israel"],
    ["IM", "Isle of Man"],
    ["IN", "India"],
    ["IO", "British Indian Ocean Territory"],
    ["IQ", "Iraq"],
    ["IR", "Iran, Islamic Republic of"],
    ["IS", "Iceland"],
    ["IT", "Italy"],
    ["JE", "Jersey"],
    ["JM", "Jamaica"],
    ["JO", "Jordan"],
    ["JP", "Japan"],
    ["KE", "Kenya"],
    ["KG", "Kyrgyzstan"],
    ["KH", "Cambodia"],
    ["KI", "Kiribati"],
    ["KM", "Comoros"],
    ["KN", "Saint Kitts and Nevis"],
    ["KP", "Korea, Democratic People’s Republic of"],
    ["KR", "Korea, Republic of"],
    ["KW", "Kuwait"],
    ["KY", "Cayman Islands"],
    ["KZ", "Kazakhstan"],
    ["LA", "Lao People’s Democratic Republic"],
    ["LB", "Lebanon"],
    ["LC", "Saint Lucia"],
    ["LI", "Liechtenstein"],
    ["LK", "Sri Lanka"],
    ["LR", "Liberia"],
    ["LS", "Lesotho"],
    ["LT", "Lithuania"],
    ["LU", "Luxembourg"],
    ["LV", "Latvia"],
    ["LY", "Libya"],
    ["MA", "Morocco"],
    ["MC", "Monaco"],
    ["MD", "Moldova, Republic of"],
    ["ME", "Montenegro"],
    ["MF", "Saint Martin (French part)"],
    ["MG", "Madagascar"],
    ["MH", "Marshall Islands"],
    ["MK", "North Macedonia"],
    ["ML", "Mali"],
    ["MM", "Myanmar"],
    ["MN", "Mongolia"],
    ["MO", "Macao"],
    ["MP", "Northern Mariana Islands"],
    ["MQ", "Martinique"],
    ["MR", "Mauritania"],
    ["MS", "Montserrat"],
    ["MT", "Malta"],
    ["MU", "Mauritius"],
    ["MV", "Maldives"],
    ["MW", "Malawi"],
    ["MX", "Mexico"],
    ["MY", "Malaysia"],
    ["MZ", "Mozambique"],
    ["NA", "Namibia"],
    ["NC", "New Caledonia"],
    ["NE", "Niger"],
    ["NF", "Norfolk Island"],
    ["NG", "Nigeria"],
    ["NI", "Nicaragua"],
    ["NL", "Netherlands"],
    ["NO", "Norway"],
    ["NP", "Nepal"],
    ["NR", "Nauru"],
    ["NU", "Niue"],
    ["NZ", "New Zealand"],
    ["OM", "Oman"],
    ["PA", "Panama"],
    ["PE", "Peru"],
    ["PF", "French Polynesia"],
    ["PG", "Papua New Guinea"],
    ["PH", "Philippines"],
    ["PK", "Pakistan"],
    ["PL", "Poland"],
    ["PM", "Saint Pierre and Miquelon"],
    ["PN", "Pitcairn"],
    ["PR", "Puerto Rico"],
    ["PS", "Palestine, State of"],
    ["PT", "Portugal"],
    ["PW", "Palau"],
    ["PY", "Paraguay"],
    ["QA", "Qatar"],
    ["RE", "Réunion"],
    ["RO", "Romania"],
    ["RS", "Serbia"],
    ["RU", "Russian Federation"],
    ["RW", "Rwanda"],
    ["SA", "Saudi Arabia"],
    ["SB", "Solomon Islands"],
    ["SC", "Seychelles"],
    ["SD", "Sudan"],
    ["SE", "Sweden"],
    ["SG", "Singapore"],
    ["SH", "Saint Helena, Ascension and Tristan da Cunha"],
    ["SI", "Slovenia"],
    ["SJ", "Svalbard and Jan Mayen"],
    ["SK", "Slovakia"],
    ["SL", "Sierra Leone"],
    ["SM", "San Marino"],
    ["SN", "Senegal"],
    ["SO", "Somalia"],
    ["SR", "Suriname"],
    ["SS", "South Sudan"],
    ["ST", "Sao Tome and Principe"],
    ["SV", "El Salvador"],
    ["SX", "Sint Maarten (Dutch part)"],
    ["SY", "Syrian Arab Republic"],
    ["SZ", "Eswatini"],
    ["TC", "Turks and Caicos Islands"],
    ["TD", "Chad"],
    ["TF", "French Southern Territories"],
    ["TG", "Togo"],
    ["TH", "Thailand"],
    ["TJ", "Tajikistan"],
    ["TK", "Tokelau"],
    ["TL", "Timor-Leste"],
    ["TM", "Turkmenistan"],
    ["TN", "Tunisia"],
    ["TO", "Tonga"],
    ["TR", "Türkiye"],
    ["TT", "Trinidad and Tobago"],
    ["TV", "Tuvalu"],
    ["TW", "Taiwan, Province of China"],
    ["TZ", "Tanzania, United Republic of"],
    ["UA", "Ukraine"],
    ["UG", "Uganda"],
    ["UM", "United States Minor Outlying Islands"],
    ["US", "United States"],
    ["UY", "Uruguay"],
    ["UZ", "Uzbekistan"],
    ["VA", "Holy See (Vatican City State)"],
    ["VC", "Saint Vincent and the Grenadines"],
    ["VE", "Venezuela, Bolivarian Republic of"],
    ["VG", "Virgin Islands, British"],
    ["VI", "Virgin Islands, US"],
    ["VN", "Viet Nam"],
    ["VU", "Vanuatu"],
    ["WF", "Wallis and Futuna"],
    ["WS", "Samoa"],
    ["YE", "Yemen"],
    ["YT", "Mayotte"],
    ["YU", "Yugoslavia"],
    ["ZA", "South Africa"],
    ["ZM", "Zambia"],
    ["ZW", "Zimbabwe"],
  ]);

  // List 92 — Supplier identifier type
  _lists[92] = new Map([
    ["01", "Proprietary name ID scheme"],
    ["02", "Proprietary"],
    ["04", "Börsenverein Verkehrsnummer"],
    ["05", "German ISBN Agency publisher identifier"],
    ["06", "GLN"],
    ["07", "SAN"],
    ["12", "Distributeurscode Boekenbank"],
    ["13", "Fondscode Boekenbank"],
    ["16", "ISNI"],
    ["23", "VAT Identity Number"],
    ["41", "EORI"],
    ["45", "Chinese participant identifier"],
  ]);

  // List 93 — Supplier role
  _lists[93] = new Map([
    ["00", "Unspecified"],
    ["01", "Publisher to resellers"],
    ["02", "Publisher’s exclusive distributor to resellers"],
    ["03", "Publisher’s non-exclusive distributor to resellers"],
    ["04", "Wholesaler to retailers"],
    ["05", "Sales agent"],
    ["06", "Publisher’s distributor to retailers"],
    ["07", "POD supplier"],
    ["08", "Retailer"],
    ["09", "Publisher to end-customers"],
    ["10", "Exclusive distributor to end-customers"],
    ["11", "Non-exclusive distributor to end-customers"],
    ["12", "Distributor to end-customers"],
    ["13", "Exclusive distributor to resellers and end-customers"],
    ["14", "Non-exclusive distributor to resellers and end-customers"],
    ["15", "Distributor to resellers and end-customers"],
  ]);

  // List 96 — Currency code – based on ISO 4217
  _lists[96] = new Map([
    ["AED", "UAE Dirham"],
    ["AFA", "Afghani"],
    ["AFN", "Afghani"],
    ["ALL", "Lek"],
    ["AMD", "Armenian Dram"],
    ["ANG", "Netherlands Antillean Guilder"],
    ["AOA", "Kwanza"],
    ["ARS", "Argentine Peso"],
    ["ATS", "Schilling"],
    ["AUD", "Australian Dollar"],
    ["AWG", "Aruban Florin"],
    ["AZN", "Azerbaijan Manat"],
    ["BAM", "Convertible Marks"],
    ["BBD", "Barbados Dollar"],
    ["BDT", "Taka"],
    ["BEF", "Belgian Franc"],
    ["BGL", "Bulgarian Lev"],
    ["BGN", "Bulgarian Lev"],
    ["BHD", "Bahraini Dinar"],
    ["BIF", "Burundi Franc"],
    ["BMD", "Bermudian Dollar"],
    ["BND", "Brunei Dollar"],
    ["BOB", "Boliviano"],
    ["BRL", "Brazilian Real"],
    ["BSD", "Bahamian Dollar"],
    ["BTN", "Ngultrun"],
    ["BWP", "Pula"],
    ["BYR", "(Old) Belarussian Ruble"],
    ["BYN", "Belarussian Ruble"],
    ["BZD", "Belize Dollar"],
    ["CAD", "Canadian Dollar"],
    ["CDF", "Franc Congolais"],
    ["CHF", "Swiss Franc"],
    ["CLP", "Chilean Peso"],
    ["CNY", "Yuan Renminbi"],
    ["COP", "Colombian Peso"],
    ["CRC", "Costa Rican Colon"],
    ["CSD", "Serbian Dinar"],
    ["CUC", "Cuban Convertible Peso"],
    ["CUP", "Cuban Peso"],
    ["CVE", "Cabo Verde Escudo"],
    ["CYP", "Cyprus Pound"],
    ["CZK", "Czech Koruna"],
    ["DEM", "Mark"],
    ["DJF", "Djibouti Franc"],
    ["DKK", "Danish Krone"],
    ["DOP", "Dominican Peso"],
    ["DZD", "Algerian Dinar"],
    ["EEK", "Kroon"],
    ["EGP", "Egyptian Pound"],
    ["ERN", "Nakfa"],
    ["ESP", "Peseta"],
    ["ETB", "Ethiopian Birr"],
    ["EUR", "Euro"],
    ["FIM", "Markka"],
    ["FJD", "Fiji Dollar"],
    ["FKP", "Falkland Islands Pound"],
    ["FRF", "Franc"],
    ["GBP", "Pound Sterling"],
    ["GEL", "Lari"],
    ["GHC", "Ghana Cedi"],
    ["GHS", "Ghana Cedi"],
    ["GIP", "Gibraltar Pound"],
    ["GMD", "Dalasi"],
    ["GNF", "Guinean Franc"],
    ["GRD", "Drachma"],
    ["GTQ", "Quetzal"],
    ["GWP", "Guinea-Bissau Peso"],
    ["GYD", "Guyana Dollar"],
    ["HKD", "Hong Kong Dollar"],
    ["HNL", "Lempira"],
    ["HRK", "Kuna"],
    ["HTG", "Gourde"],
    ["HUF", "Forint"],
    ["IDR", "Rupiah"],
    ["IEP", "Punt"],
    ["ILS", "New Israeli Sheqel"],
    ["INR", "Indian Rupee"],
    ["IQD", "Iraqi Dinar"],
    ["IRR", "Iranian Rial"],
    ["ISK", "Iceland Krona"],
    ["ITL", "Lira"],
    ["JMD", "Jamaican Dollar"],
    ["JOD", "Jordanian Dinar"],
    ["JPY", "Yen"],
    ["KES", "Kenyan Shilling"],
    ["KGS", "Som"],
    ["KHR", "Riel"],
    ["KMF", "Comorian Franc"],
    ["KPW", "North Korean Won"],
    ["KRW", "Won"],
    ["KWD", "Kuwaiti Dinar"],
    ["KYD", "Cayman Islands Dollar"],
    ["KZT", "Tenge"],
    ["LAK", "Lao Kip"],
    ["LBP", "Lebanese Pound"],
    ["LKR", "Sri Lanka Rupee"],
    ["LRD", "Liberian Dollar"],
    ["LSL", "Loti"],
    ["LTL", "Litus"],
    ["LUF", "Luxembourg Franc"],
    ["LVL", "Latvian Lats"],
    ["LYD", "Libyan Dinar"],
    ["MAD", "Moroccan Dirham"],
    ["MDL", "Moldovan Leu"],
    ["MGA", "Malagasy Ariary"],
    ["MGF", "Malagasy Franc"],
    ["MKD", "Denar"],
    ["MMK", "Kyat"],
    ["MNT", "Tugrik"],
    ["MOP", "Pataca"],
    ["MRO", "(Old) Ouguiya"],
    ["MRU", "Ouguiya"],
    ["MTL", "Maltese Lira"],
    ["MUR", "Mauritius Rupee"],
    ["MVR", "Rufiyaa"],
    ["MWK", "Malawi Kwacha"],
    ["MXN", "Mexican Peso"],
    ["MYR", "Malaysian Ringgit"],
    ["MZN", "Mozambique Metical"],
    ["NAD", "Namibia Dollar"],
    ["NGN", "Naira"],
    ["NIO", "Cordoba Oro"],
    ["NLG", "Guilder"],
    ["NOK", "Norwegian Krone"],
    ["NPR", "Nepalese Rupee"],
    ["NZD", "New Zealand Dollar"],
    ["OMR", "Rial Omani"],
    ["PAB", "Balboa"],
    ["PEN", "Sol"],
    ["PGK", "Kina"],
    ["PHP", "Philippine Peso"],
    ["PKR", "Pakistan Rupee"],
    ["PLN", "Złoty"],
    ["PTE", "Escudo"],
    ["PYG", "Guarani"],
    ["QAR", "Qatari Rial"],
    ["ROL", "Romanian Old Leu"],
    ["RON", "Romanian Leu"],
    ["RSD", "Serbian Dinar"],
    ["RUB", "Russian Ruble"],
    ["RUR", "Russian Ruble"],
    ["RWF", "Rwanda Franc"],
    ["SAR", "Saudi Riyal"],
    ["SBD", "Solomon Islands Dollar"],
    ["SCR", "Seychelles Rupee"],
    ["SDD", "Sudanese Dinar"],
    ["SDG", "Sudanese Pound"],
    ["SEK", "Swedish Krona"],
    ["SGD", "Singapore Dollar"],
    ["SHP", "Saint Helena Pound"],
    ["SIT", "Tolar"],
    ["SKK", "Slovak Koruna"],
    ["SLE", "Leone"],
    ["SLL", "Leone"],
    ["SOS", "Somali Shilling"],
    ["SRD", "Surinam Dollar"],
    ["SRG", "Suriname Guilder"],
    ["STD", "(Old) Dobra"],
    ["STN", "Dobra"],
    ["SVC", "El Salvador Colon"],
    ["SYP", "Syrian Pound"],
    ["SZL", "Lilangeni"],
    ["THB", "Baht"],
    ["TJS", "Somoni"],
    ["TMM", "Turkmenistan Manat"],
    ["TMT", "Turkmenistan New Manat"],
    ["TND", "Tunisian Dinar"],
    ["TOP", "Pa’anga"],
    ["TPE", "Timor Escudo"],
    ["TRL", "Turkish Lira (old)"],
    ["TRY", "Turkish Lira"],
    ["TTD", "Trinidad and Tobago Dollar"],
    ["TWD", "New Taiwan Dollar"],
    ["TZS", "Tanzanian Shilling"],
    ["UAH", "Hryvnia"],
    ["UGX", "Uganda Shilling"],
    ["USD", "US Dollar"],
    ["UYU", "Peso Uruguayo"],
    ["UZS", "Uzbekistan Sum"],
    ["VEB", "Bolívar"],
    ["VEF", "Bolívar"],
    ["VES", "Bolívar Soberano"],
    ["VND", "Dong"],
    ["VUV", "Vatu"],
    ["WST", "Tala"],
    ["XAF", "CFA Franc BEAC"],
    ["XCD", "East Caribbean Dollar"],
    ["XCG", "Caribbean Guilder"],
    ["XOF", "CFA Franc BCEAO"],
    ["XPF", "CFP Franc"],
    ["YER", "Yemeni Rial"],
    ["YUM", "Yugoslavian Dinar"],
    ["ZAR", "Rand"],
    ["ZMK", "Kwacha"],
    ["ZMW", "Zambian Kwacha"],
    ["ZWD", "Zimbabwe Dollar"],
    ["ZWG", "Zimbabwe Gold"],
    ["ZWL", "Zimbabwe Dollar"],
  ]);

  // List 97 — Bible text feature
  _lists[97] = new Map([
    ["RL", "Red letter"],
  ]);

  // List 98 — Binding or page edge color
  _lists[98] = new Map([
    ["BLK", "Black"],
    ["BLU", "Blue"],
    ["BRN", "Brown"],
    ["BUR", "Burgundy / Maroon"],
    ["CEL", "Celadon / Pale green"],
    ["CPR", "Copper"],
    ["CRE", "Cream"],
    ["FCO", "Four-color"],
    ["FCS", "Four-color and spot-color"],
    ["GLD", "Gold"],
    ["GRN", "Green"],
    ["GRY", "Grey"],
    ["HOL", "Holographic"],
    ["MUL", "Multicolor"],
    ["NAV", "Navy / Dark blue"],
    ["ORG", "Orange"],
    ["PNK", "Pink"],
    ["PUR", "Purple"],
    ["RED", "Red"],
    ["SKY", "Sky / Pale blue"],
    ["SLV", "Silver"],
    ["TAN", "Tan / Light brown"],
    ["TEA", "Teal / Turquoise green"],
    ["WHI", "White"],
    ["YEL", "Yellow"],
    ["ZZZ", "Other"],
  ]);

  // List 99 — Special cover material
  _lists[99] = new Map([
    ["01", "Berkshire leather"],
    ["02", "Calfskin"],
    ["03", "French Morocco"],
    ["04", "Morocco"],
    ["05", "Bonded buffalo grain"],
    ["06", "Bonded calf grain"],
    ["07", "Bonded Cordova"],
    ["08", "Bonded eelskin"],
    ["09", "Bonded Ostraleg"],
    ["10", "Bonded ostrich"],
    ["11", "Bonded reptile grain"],
    ["12", "Bonded leather"],
    ["13", "Cowhide"],
    ["14", "Eelskin"],
    ["15", "Kivar"],
    ["16", "Leatherflex"],
    ["17", "Moleskin"],
    ["18", "Softhide leather"],
    ["19", "Metal"],
    ["20", "Velvet"],
    ["21", "Mother-of-pearl"],
    ["22", "Papyrus"],
    ["23", "Géltex / Wibalin"],
    ["24", "Guaflex / Skivertex"],
    ["25", "Imitation leather"],
    ["26", "Pigskin"],
    ["27", "Goatskin"],
    ["28", "Rubber"],
    ["29", "Padded or quilted fabric"],
  ]);

  // List 100 — Discount code type
  _lists[100] = new Map([
    ["01", "BIC discount group code"],
    ["02", "Proprietary discount code scheme"],
    ["03", "Boeksoort"],
    ["04", "German terms code"],
    ["05", "Proprietary commission code scheme"],
    ["06", "BIC commission group code"],
    ["07", "ISNI-based discount group code"],
  ]);

  // List 102 — Sales outlet identifier type
  _lists[102] = new Map([
    ["01", "Proprietary sales outlet ID scheme"],
    ["03", "ONIX retail sales outlet ID code"],
    ["04", "Retail sales outlet GLN"],
    ["05", "Retail sales outlet SAN"],
  ]);

  // List 121 — Text script – based on ISO 15924
  _lists[121] = new Map([
    ["Adlm", "Adlam"],
    ["Afak", "Afaka"],
    ["Aghb", "Caucasian Albanian"],
    ["Ahom", "Ahom, Tai Ahom"],
    ["Arab", "Arabic"],
    ["Aran", "Arabic (Nastaliq variant)"],
    ["Armi", "Imperial Aramaic"],
    ["Armn", "Armenian"],
    ["Avst", "Avestan"],
    ["Bali", "Balinese"],
    ["Bamu", "Bamun"],
    ["Bass", "Bassa Vah"],
    ["Batk", "Batak"],
    ["Beng", "Bengali (Bangla)"],
    ["Berf", "Beria Erfe"],
    ["Bhks", "Bhaiksuki"],
    ["Blis", "Blissymbols"],
    ["Bopo", "Bopomofo"],
    ["Brah", "Brahmi"],
    ["Brai", "Braille"],
    ["Bugi", "Buginese"],
    ["Buhd", "Buhid"],
    ["Cakm", "Chakma"],
    ["Cans", "Unified Canadian Aboriginal Syllabics"],
    ["Cari", "Carian"],
    ["Cham", "Cham"],
    ["Cher", "Cherokee"],
    ["Chis", "Chisoi"],
    ["Chrs", "Chorasmian"],
    ["Cirt", "Cirth"],
    ["Copt", "Coptic"],
    ["Cpmn", "Cypro-Minoan"],
    ["Cprt", "Cypriot"],
    ["Cyrl", "Cyrillic"],
    ["Cyrs", "Cyrillic (Old Church Slavonic variant)"],
    ["Deva", "Devanagari (Nagari)"],
    ["Diak", "Dives Akuru"],
    ["Dogr", "Dogra"],
    ["Dsrt", "Deseret (Mormon)"],
    ["Dupl", "Duployan shorthand, Duployan stenography"],
    ["Egyd", "Egyptian demotic"],
    ["Egyh", "Egyptian hieratic"],
    ["Egyp", "Egyptian hieroglyphs"],
    ["Elba", "Elbasan"],
    ["Elym", "Elymaic"],
    ["Ethi", "Ethiopic (Ge‘ez)"],
    ["Gara", "Garay"],
    ["Geok", "Khutsuri (Asomtavruli and Khutsuri)"],
    ["Geor", "Georgian (Mkhedruli and Mtavruli)"],
    ["Glag", "Glagolitic"],
    ["Gong", "Gunjala Gondi"],
    ["Gonm", "Masaram Gondi"],
    ["Goth", "Gothic"],
    ["Gran", "Grantha"],
    ["Grek", "Greek"],
    ["Gujr", "Gujarati"],
    ["Gukh", "Gurung Khema"],
    ["Guru", "Gurmukhi"],
    ["Hanb", "Han with Bopomofo"],
    ["Hang", "Hangul (Hangŭl, Hangeul)"],
    ["Hani", "Han (Hanzi, Kanji, Hanja)"],
    ["Hano", "Hanunoo (Hanunóo)"],
    ["Hans", "Han (Simplified variant)"],
    ["Hant", "Han (Traditional variant)"],
    ["Hatr", "Hatran"],
    ["Hebr", "Hebrew"],
    ["Hira", "Hiragana"],
    ["Hluw", "Anatolian Hieroglyphs (Luwian Hieroglyphs, Hittite Hieroglyphs)"],
    ["Hmng", "Pahawh Hmong"],
    ["Hmnp", "Nyiakeng Puachue Hmong"],
    ["Hntl", "Han (Traditional) with Latin (alias for Hant + Latn)"],
    ["Hrkt", "Japanese syllabaries (alias for Hiragana + Katakana)"],
    ["Hung", "Old Hungarian (Hungarian Runic)"],
    ["Inds", "Indus (Harappan)"],
    ["Ital", "Old Italic (Etruscan, Oscan, etc.)"],
    ["Jamo", "Jamo (alias for Jamo subset of Hangul)"],
    ["Java", "Javanese"],
    ["Jpan", "Japanese (alias for Han + Hiragana + Katakana)"],
    ["Jurc", "Jurchen"],
    ["Kali", "Kayah Li"],
    ["Kana", "Katakana"],
    ["Kawi", "Kawi"],
    ["Khar", "Kharoshthi"],
    ["Khmr", "Khmer"],
    ["Khoj", "Khojki"],
    ["Kitl", "Khitan large script"],
    ["Kits", "Khitan small script"],
    ["Knda", "Kannada"],
    ["Kore", "Korean (alias for Hangul + Han)"],
    ["Kpel", "Kpelle"],
    ["Krai", "Kirat Rai"],
    ["Kthi", "Kaithi"],
    ["Lana", "Tai Tham (Lanna)"],
    ["Laoo", "Lao"],
    ["Latf", "Latin (Fraktur variant)"],
    ["Latg", "Latin (Gaelic variant)"],
    ["Latn", "Latin"],
    ["Leke", "Leke"],
    ["Lepc", "Lepcha (Róng)"],
    ["Limb", "Limbu"],
    ["Lina", "Linear A"],
    ["Linb", "Linear B"],
    ["Lisu", "Lisu (Fraser)"],
    ["Loma", "Loma"],
    ["Lyci", "Lycian"],
    ["Lydi", "Lydian"],
    ["Mahj", "Mahajani"],
    ["Maka", "Makasar"],
    ["Mand", "Mandaic, Mandaean"],
    ["Mani", "Manichaean"],
    ["Marc", "Marchen"],
    ["Maya", "Mayan hieroglyphs"],
    ["Medf", "Medefaidrin (Oberi Okaime, Oberi Ɔkaimɛ)"],
    ["Mend", "Mende Kikakui"],
    ["Merc", "Meroitic Cursive"],
    ["Mero", "Meroitic Hieroglyphs"],
    ["Mlym", "Malayalam"],
    ["Modi", "Modi, Moḍī"],
    ["Mong", "Mongolian"],
    ["Moon", "Moon (Moon code, Moon script, Moon type)"],
    ["Mroo", "Mro, Mru"],
    ["Mtei", "Meitei Mayek (Meithei, Meetei)"],
    ["Mult", "Multani"],
    ["Mymr", "Myanmar (Burmese)"],
    ["Nagm", "Nag Mundari"],
    ["Nand", "Nandinagari"],
    ["Narb", "Old North Arabian (Ancient North Arabian)"],
    ["Nbat", "Nabatean"],
    ["Newa", "Newa, Newar, Newari, Nepāla lipi"],
    ["Nkdb", "Nakhi Tomba (Naxi Dongba)"],
    ["Nkgb", "Nakhi Geba (’Na-’Khi ²Ggŏ-¹baw, Naxi Geba)"],
    ["Nkoo", "N’Ko"],
    ["Nshu", "Nüshu"],
    ["Ogam", "Ogham"],
    ["Olck", "Ol Chiki (Ol Cemet’, Ol, Santali)"],
    ["Onao", "Ol Onal"],
    ["Orkh", "Old Turkic, Orkhon Runic"],
    ["Orya", "Oriya (Odia)"],
    ["Osge", "Osage"],
    ["Osma", "Osmanya"],
    ["Ougr", "Old Uyghur"],
    ["Palm", "Palmyrene"],
    ["Pauc", "Pau Cin Hau"],
    ["Pcun", "Proto-Cuneiform"],
    ["Pelm", "Proto-Elamite"],
    ["Perm", "Old Permic"],
    ["Phag", "Phags-pa"],
    ["Phli", "Inscriptional Pahlavi"],
    ["Phlp", "Psalter Pahlavi"],
    ["Phlv", "Book Pahlavi"],
    ["Phnx", "Phoenician"],
    ["Piqd", "Klingon (KLI plqaD)"],
    ["Plrd", "Miao (Pollard)"],
    ["Prti", "Inscriptional Parthian"],
    ["Psin", "Proto-Sinaitic"],
    ["Qaaa", "Reserved for private use (start)"],
    ["Qabp", "Picture Communication Symbols (PCS)"],
    ["Qabw", "Widgit symbols"],
    ["Qabx", "Reserved for private use (end)"],
    ["Ranj", "Ranjana"],
    ["Rjng", "Rejang (Redjang, Kaganga)"],
    ["Rohg", "Hanifi Rohingya"],
    ["Roro", "Rongorongo"],
    ["Runr", "Runic"],
    ["Samr", "Samaritan"],
    ["Sara", "Sarati"],
    ["Sarb", "Old South Arabian"],
    ["Saur", "Saurashtra"],
    ["Seal", "(Small) Seal"],
    ["Sgnw", "SignWriting"],
    ["Shaw", "Shavian (Shaw)"],
    ["Shrd", "Sharada, Śāradā"],
    ["Shui", "Shuishu"],
    ["Sidd", "Siddham, Siddhaṃ, Siddhamātṛkā"],
    ["Sidt", "Sidetic"],
    ["Sind", "Khudawadi, Sindhi"],
    ["Sinh", "Sinhala"],
    ["Sogd", "Sogdian"],
    ["Sogo", "Old Sogdian"],
    ["Sora", "Sora Sompeng"],
    ["Soyo", "Soyombo"],
    ["Sund", "Sundanese"],
    ["Sunu", "Sunuwar"],
    ["Sylo", "Syloti Nagri"],
    ["Syrc", "Syriac"],
    ["Syre", "Syriac (Estrangelo variant)"],
    ["Syrj", "Syriac (Western variant)"],
    ["Syrn", "Syriac (Eastern variant)"],
    ["Tagb", "Tagbanwa"],
    ["Takr", "Takri, Ṭākrī, Ṭāṅkrī"],
    ["Tale", "Tai Le"],
    ["Talu", "New Tai Lue"],
    ["Taml", "Tamil"],
    ["Tang", "Tangut"],
    ["Tavt", "Tai Viet"],
    ["Tayo", "Tai Yo"],
    ["Telu", "Telugu"],
    ["Teng", "Tengwar"],
    ["Tfng", "Tifinagh (Berber)"],
    ["Tglg", "Tagalog (Baybayin, Alibata)"],
    ["Thaa", "Thaana"],
    ["Thai", "Thai"],
    ["Tibt", "Tibetan"],
    ["Tirh", "Tirhuta"],
    ["Tnsa", "Tangsa"],
    ["Todr", "Todhri"],
    ["Tols", "Tolong Siki"],
    ["Toto", "Toto"],
    ["Tutg", "Tulu-Tigalari"],
    ["Ugar", "Ugaritic"],
    ["Vaii", "Vai"],
    ["Visp", "Visible Speech"],
    ["Vith", "Vithkuqi"],
    ["Wara", "Warang Citi (Varang Kshiti)"],
    ["Wcho", "Wancho"],
    ["Wole", "Woleai"],
    ["Xpeo", "Old Persian"],
    ["Xsux", "Cuneiform, Sumero-Akkadian"],
    ["Yezi", "Yezidi"],
    ["Yiii", "Yi"],
    ["Zanb", "Zanabazar Square (Zanabazarin Dörböljin Useg, Xewtee Dörböljin Bicig, Horizontal Square Script)"],
    ["Zmth", "Mathematical notation"],
    ["Zsye", "Symbols (Emoji variant)"],
    ["Zsym", "Symbols"],
    ["Zxxx", "Code for unwritten documents"],
    ["Zinh", "Code for inherited script"],
    ["Zyyy", "Code for undetermined script"],
    ["Zzzz", "Code for uncoded script"],
  ]);

  // List 139 — ONIX retail sales outlet IDs
  _lists[139] = new Map([
    ["SYM", "24Symbols"],
    ["ACM", "A C Moore"],
    ["AAP", "AandP"],
    ["ADL", "Adlibris"],
    ["AJA", "Ajassa"],
    ["ACB", "Akademibokhandeln"],
    ["AKA", "Akademika"],
    ["AKK", "Akateeminen Kirjakauppa"],
    ["ALB", "Albertson’s"],
    ["ALV", "Allvit"],
    ["ALT", "AlmaTalent"],
    ["AMZ", "Amazon"],
    ["ANR", "Angus and Robertson"],
    ["ANB", "Anobii"],
    ["APA", "Apabi"],
    ["APC", "Apple"],
    ["ARK", "Ark"],
    ["ASD", "Asda"],
    ["ASB", "Asia Books"],
    ["AUD", "Audible"],
    ["AUC", "Audiobooks.com"],
    ["ATK", "Audioteka"],
    ["ELU", "Axiell ELibU"],
    ["BDL", "B Dalton"],
    ["BNO", "Barnes and Noble"],
    ["BDZ", "BDBuzz"],
    ["BBB", "Bed Bath and Beyond"],
    ["BST", "Best Buy"],
    ["BOD", "Biblio-on-Demand"],
    ["MMM", "Bibliotheca"],
    ["BIL", "Bilbary"],
    ["BLG", "Billigbook"],
    ["BJW", "BJ’s Wholesale Club"],
    ["BLA", "Blackstone Audio"],
    ["BLK", "Blackwell’s"],
    ["GOS", "Blinkbox"],
    ["BGI", "Bog & idé"],
    ["BKL", "Bokkilden"],
    ["BKK", "Bokklubben"],
    ["BSK", "Boksnok"],
    ["BSY", "Boksy"],
    ["BKU", "Bokus"],
    ["BLD", "Bolinda"],
    ["BCA", "Book Club Associates"],
    ["BDO", "Book d’oreille"],
    ["BSL", "Book Solutions"],
    ["BOO", "Bookbeat"],
    ["BBI", "Bookbites"],
    ["BSH", "Bookish"],
    ["BMA", "Bookmate"],
    ["BKP", "Bookpeople"],
    ["BRB", "Books Etc"],
    ["BKM", "Books-A-Million"],
    ["BSO", "Bookshop.org"],
    ["BKS", "Bookshout"],
    ["BTP", "Booktopia"],
    ["BKY", "Booky.fi"],
    ["BRD", "Borders"],
    ["BRT", "British Bookshops"],
    ["BSU", "Lære"],
    ["CAI", "Cairn"],
    ["CDL", "Casa del Libro"],
    ["CDN", "CDON.com"],
    ["CEB", "Ceebo"],
    ["CFU", "CFU Danmark"],
    ["CHP", "Chapter"],
    ["CHD", "Christianbook.com"],
    ["CYM", "Citymarket"],
    ["CMX", "Comixology"],
    ["COP", "Copia"],
    ["CST", "Costco"],
    ["CRB", "Crate and Barrel"],
    ["CVS", "CVS Drug Stores"],
    ["CVM", "CVS Mediatheques"],
    ["CYB", "Cyberlibris"],
    ["DAU", "Daunt Books"],
    ["DEM", "De Marque"],
    ["DSG", "Dick’s Sporting Goods"],
    ["DIL", "Dilicom"],
    ["DYM", "Dymocks"],
    ["EKJ", "E-kirjasto"],
    ["EPA", "E-Pagine"],
    ["ERD", "E-Reads"],
    ["ELC", "Early Learning Centre"],
    ["ESN", "Eason"],
    ["EBK", "Ebok"],
    ["EBC", "Ebooks Corp"],
    ["ECH", "eChristian"],
    ["ECI", "El Corte Inglés"],
    ["ELE", "Electre"],
    ["ELI", "Elib Library"],
    ["ELS", "Elisa"],
    ["ELK", "Elisa Kirja Kuukausitilaus"],
    ["ELL", "Ellibs.com"],
    ["ELM", "Ellibs university libraries"],
    ["EMP", "Empik"],
    ["ENH", "English Heritage"],
    ["EPC", "Epic!"],
    ["ELN", "Ereolen.dk"],
    ["EVB", "Everybook"],
    ["FAB", "Fabel"],
    ["FAI", "FairyLoot"],
    ["FDB", "FeedBooks"],
    ["FIN", "Findaway"],
    ["FSP", "Fishpond"],
    ["FLP", "FlipKart"],
    ["FNC", "Fnac"],
    ["FOL", "Follett"],
    ["FRY", "Fry’s Electronics"],
    ["GMS", "Gamestop"],
    ["GAR", "Gardners"],
    ["GLO", "Glose"],
    ["GOO", "Google Books"],
    ["HST", "Hastings Entertainment"],
    ["HMV", "HMV"],
    ["HMD", "Home Depot"],
    ["HOO", "Hoopla / Midwest Tape"],
    ["HUG", "Hugendubel"],
    ["ILU", "Illumicrate"],
    ["IMM", "Immatériel.fr"],
    ["IND", "Indigo-Chapters"],
    ["INF", "Info.fi"],
    ["ILT", "Inläsningstjänst"],
    ["IBS", "Internet Bookshop Italia"],
    ["IZN", "Izneo"],
    ["JAM", "Jamera"],
    ["JBH", "JB Hifi"],
    ["JSM", "John Smith and Son"],
    ["KMT", "K-Mart"],
    ["KIP", "KIPA"],
    ["KRJ", "Kirja.fi"],
    ["KID", "Kirja&Idea"],
    ["KPV", "Kirjastopalvelu"],
    ["KNB", "KNFB / Blio"],
    ["KNO", "Kno Inc"],
    ["KBO", "Kobo"],
    ["KOO", "Koorong"],
    ["KOR", "Kortext"],
    ["KRG", "Kroger"],
    ["LEG", "Legible.com"],
    ["LTP", "Lehtipiste"],
    ["LEX", "Lexplore"],
    ["LIB", "Libro.fm"],
    ["LDL", "Lidl"],
    ["LMB", "LoadMyBook"],
    ["LWE", "Lowe’s"],
    ["MAC", "Mackin"],
    ["MAG", "Magzter"],
    ["MNO", "Maneno"],
    ["MKS", "Marks and Spencer"],
    ["MAT", "Matras"],
    ["MMS", "Media Markt / Saturn"],
    ["ADP", "Messageries ADP"],
    ["MCR", "Microcenter"],
    ["MSF", "Microsoft"],
    ["MOF", "Mofibo"],
    ["MRR", "Morrisons"],
    ["MTC", "Mothercare"],
    ["MYB", "MyBoox"],
    ["MYI", "MyiLibrary"],
    ["NTR", "National Trust"],
    ["NEX", "NEXT"],
    ["NXT", "Nextory"],
    ["NOL", "Norli"],
    ["NOV", "Novelist"],
    ["NUM", "Numilog"],
    ["ODL", "Odilo.us"],
    ["OFD", "Office Depot"],
    ["OFM", "Office Max"],
    ["OLF", "OLF"],
    ["OSS", "Osloskolen"],
    ["OVD", "OverDrive"],
    ["OWC", "OwlCrate"],
    ["OYS", "Oyster"],
    ["PST", "Past Times"],
    ["PER", "Perlego"],
    ["PTS", "Pet Smart"],
    ["PTC", "Petco"],
    ["PIC", "Piccoma"],
    ["PLY", "Play.com"],
    ["POD", "Podimo"],
    ["POY", "Podit Oy"],
    ["PDM", "Podme"],
    ["POK", "Pokkaritukku"],
    ["PTB", "Pottery Barn"],
    ["PRR", "PressReader"],
    ["PLO", "Publio"],
    ["ELB", "Publizon"],
    ["RDB", "Readbooks"],
    ["RCL", "ReadCloud"],
    ["RST", "Restoration Hardware"],
    ["RET", "Rethink"],
    ["RTZ", "Ritz Camera"],
    ["RMB", "RM Books"],
    ["RBD", "Rosebud"],
    ["SGR", "S Group"],
    ["SAC", "Sacrum"],
    ["SAF", "Safari"],
    ["SFW", "Safeway"],
    ["SNS", "Sainsbury’s"],
    ["SAX", "Saxo"],
    ["SCR", "Scribd"],
    ["SLF", "Selfridges"],
    ["SEQ", "Sequencity"],
    ["SES", "Sesamy"],
    ["SBT", "Shanghai Book Traders"],
    ["SKL", "Skolon"],
    ["SKB", "Skoobe"],
    ["SMW", "SmashWords"],
    ["SNY", "Sony"],
    ["SPO", "Spotify"],
    ["STP", "Staples"],
    ["STY", "Staytuned.io"],
    ["STH", "Storyhouse"],
    ["STT", "Storytel"],
    ["SKK", "Suomalainen Kirjakauppa"],
    ["SPL", "Suomalainen Plus"],
    ["SUP", "Supla+"],
    ["SSK", "Suuri Suomalainen Kirjakerho"],
    ["THA", "Thalia"],
    ["TRG", "Target"],
    ["TES", "Tesco"],
    ["TIK", "TikTok Shop"],
    ["TOK", "Tokmanni"],
    ["TOL", "Tolino"],
    ["TSR", "Toys ’R’ Us"],
    ["TSO", "TSO (The Stationery Office)"],
    ["TKK", "Turun kansallinen kirjakauppa"],
    ["TXR", "Txtr"],
    ["UBH", "Ugglan Bokhandel"],
    ["VRG", "Virgin Megastores"],
    ["TEA", "Vivlio"],
    ["WHS", "W H Smith"],
    ["WTR", "Waitrose"],
    ["WLM", "Wal-Mart"],
    ["WST", "Waterstone’s"],
    ["WHT", "Whitcoul’s"],
    ["WLS", "Williams Sonoma"],
    ["WLW", "Woolworths"],
    ["XIG", "XigXag"],
    ["YOU", "Youboox"],
    ["YSC", "Youscribe"],
    ["ZVV", "Zavvi"],
    ["PUB", "Publisher’s own direct sales"],
    ["ZZZ", "Other"],
  ]);

  // List 141 — Barcode indicator
  _lists[141] = new Map([
    ["00", "Not barcoded"],
    ["01", "Barcoded, scheme unspecified"],
    ["02", "GTIN-13"],
    ["03", "GTIN-13+5 (US dollar price encoded)"],
    ["04", "GTIN-13+5 (CAN dollar price encoded)"],
    ["05", "GTIN-13+5 (no price encoded)"],
    ["06", "UPC-12 (item-specific)"],
    ["07", "UPC-12+5 (item-specific)"],
    ["08", "UPC-12 (price-point)"],
    ["09", "UPC-12+5 (price-point)"],
    ["10", "GTIN-13+5 (UK Pound Sterling price encoded)"],
    ["11", "GTIN-13+5 (other price encoded)"],
    ["12", "GTIN-13+2"],
    ["13", "GTIN-13+5"],
  ]);

  // List 142 — Position on product
  _lists[142] = new Map([
    ["00", "Unknown / unspecified"],
    ["01", "Cover 4"],
    ["02", "Cover 3"],
    ["03", "Cover 2"],
    ["04", "Cover 1"],
    ["05", "On spine"],
    ["06", "On box"],
    ["07", "On tag"],
    ["08", "On bottom"],
    ["09", "On back"],
    ["10", "On outer sleeve / back"],
    ["11", "On removable wrapping"],
  ]);

  // List 143 — US CPSIA or other international hazard warning type
  _lists[143] = new Map([
    ["00", "No known hazards or warnings"],
    ["01", "WARNING: CHOKING HAZARD – Small parts | Not for children under 3 yrs."],
    ["02", "WARNING: CHOKING HAZARD – Children under 8 yrs. can choke or suffocate on uninflated or broken balloons. Adult supervision required | Keep uninflated balloons from children. Discard broken balloons at once."],
    ["03", "WARNING: CHOKING HAZARD – This toy is a small ball | Not for children under 3 yrs."],
    ["04", "WARNING: CHOKING HAZARD – Toy contains a small ball | Not for children under 3 yrs."],
    ["05", "WARNING: CHOKING HAZARD – This toy is a marble | Not for children under 3 yrs."],
    ["06", "WARNING: CHOKING HAZARD – Toy contains a marble | Not for children under 3 yrs."],
    ["07", "No choking hazard warning necessary"],
    ["08", "WARNING: STRANGULATION HAZARD – Long cord | Not for children under 3 yrs."],
    ["09", "No strangulation hazard warning necessary"],
    ["10", "US CPSC Certificate of Conformity available"],
    ["11", "WARNING: MAGNET HAZARD – Product contains (a) small magnet(s)"],
    ["12", "No magnet hazard warning necessary"],
    ["13", "WARNING – Flashing hazard"],
    ["14", "No flashing hazard warning necessary"],
    ["15", "WARNING – Sound hazard"],
    ["16", "No sound hazard warning necessary"],
    ["17", "WARNING – Motion simulation hazard"],
    ["18", "No motion simulation hazard warning necessary"],
    ["21", "WARNING – California Proposition 65 carcinogenic, teratogenic or other reproductively harmful chemical hazard"],
    ["22", "No California Proposition 65 hazard warning necessary"],
    ["23", "California Proposition 65 chemical name"],
    ["24", "Flashing risk unknown"],
    ["25", "Sound risk unknown"],
    ["26", "Motion simulation risk unknown"],
  ]);

  // List 144 — E-publication technical protection
  _lists[144] = new Map([
    ["00", "None"],
    ["01", "DRM"],
    ["02", "Digital watermarking"],
    ["03", "Adobe DRM"],
    ["04", "Apple DRM"],
    ["05", "OMA DRM"],
    ["06", "Readium LCP DRM"],
    ["07", "Sony DRM"],
  ]);

  // List 145 — Usage type
  _lists[145] = new Map([
    ["00", "No constraints"],
    ["01", "Preview"],
    ["02", "Print"],
    ["03", "Copy / paste"],
    ["04", "Share"],
    ["05", "Text to speech"],
    ["06", "Lend"],
    ["07", "Time-limited license"],
    ["08", "Library loan renewal"],
    ["09", "Multi-user license"],
    ["10", "Preview on premises"],
    ["11", "Text and data mining"],
    ["13", "TDM for AI to enhance the private purchase experience"],
    ["14", "TDM for AI to enhance the private reading experience"],
    ["16", "Library loan"],
  ]);

  // List 146 — Usage status
  _lists[146] = new Map([
    ["01", "Permitted unlimited"],
    ["02", "Permitted subject to limit"],
    ["03", "Prohibited"],
  ]);

  // List 147 — Unit of usage
  _lists[147] = new Map([
    ["01", "Copies"],
    ["02", "Characters"],
    ["03", "Words"],
    ["04", "Pages"],
    ["05", "Percentage"],
    ["06", "Devices"],
    ["07", "Concurrent users"],
    ["15", "Users"],
    ["19", "Concurrent classes"],
    ["20", "Classes"],
    ["31", "Institutions"],
    ["08", "Percentage per time period"],
    ["09", "Days"],
    ["13", "Weeks"],
    ["14", "Months"],
    ["16", "Hours minutes and seconds"],
    ["27", "Days (fixed start)"],
    ["28", "Weeks (fixed start)"],
    ["29", "Months (fixed start)"],
    ["10", "Times"],
    ["22", "Times per day"],
    ["23", "Times per month"],
    ["24", "Times per year"],
    ["21", "Dots per inch"],
    ["26", "Dots per cm"],
    ["11", "Allowed usage start page"],
    ["12", "Allowed usage end page"],
    ["17", "Allowed usage start time"],
    ["18", "Allowed usage end time"],
    ["98", "Valid from"],
    ["99", "Valid to"],
  ]);

  // List 148 — Collection type
  _lists[148] = new Map([
    ["00", "Unspecified (default)"],
    ["10", "Publisher collection"],
    ["11", "Collection éditoriale"],
    ["20", "Ascribed collection"],
  ]);

  // List 149 — Title element level
  _lists[149] = new Map([
    ["01", "Product"],
    ["02", "Collection level"],
    ["03", "Subcollection"],
    ["04", "Content item"],
    ["05", "Master brand"],
    ["06", "Sub-subcollection"],
    ["07", "Universe"],
  ]);

  // List 150 — Product form
  _lists[150] = new Map([
    ["00", "Undefined"],
    ["AA", "Audio"],
    ["AB", "Audio cassette"],
    ["AC", "CD-Audio"],
    ["AD", "DAT"],
    ["AE", "Audio disc"],
    ["AF", "Audio tape"],
    ["AG", "MiniDisc"],
    ["AH", "CD-Extra"],
    ["AI", "DVD Audio"],
    ["AJ", "Downloadable audio file"],
    ["AK", "Pre-recorded digital audio player"],
    ["AL", "Pre-recorded SD card"],
    ["AM", "LP"],
    ["AN", "Downloadable and online / streamed audio file"],
    ["AO", "Online / streamed audio file"],
    ["AZ", "Other audio format"],
    ["BA", "Book"],
    ["BB", "Hardback"],
    ["BC", "Paperback / softback"],
    ["BD", "Loose-leaf"],
    ["BE", "Spiral bound"],
    ["BF", "Pamphlet"],
    ["BG", "Leather / fine binding"],
    ["BH", "Board book"],
    ["BI", "Rag book"],
    ["BJ", "Bath book"],
    ["BK", "Novelty book"],
    ["BL", "Slide bound"],
    ["BM", "Big book"],
    ["BN", "Part-work (fascículo)"],
    ["BO", "Fold-out book or chart"],
    ["BP", "Foam book"],
    ["BZ", "Other book format"],
    ["CA", "Sheet map"],
    ["CB", "Sheet map, folded"],
    ["CC", "Sheet map, flat"],
    ["CD", "Sheet map, rolled"],
    ["CE", "Globe"],
    ["CZ", "Other cartographic"],
    ["DA", "Digital (on physical carrier)"],
    ["DB", "CD-ROM"],
    ["DC", "CD-I"],
    ["DE", "Game cartridge"],
    ["DF", "Diskette"],
    ["DI", "DVD-ROM"],
    ["DJ", "Secure Digital (SD) Memory Card"],
    ["DK", "Compact Flash Memory Card"],
    ["DL", "Memory Stick Memory Card"],
    ["DM", "USB Flash Drive"],
    ["DN", "Double-sided CD / DVD"],
    ["DO", "BR-ROM"],
    ["DZ", "Other digital carrier"],
    ["EA", "Digital (delivered electronically)"],
    ["EB", "Digital download and online / streamed"],
    ["EC", "Digital online / streamed"],
    ["ED", "Digital download"],
    ["FA", "Film or transparency"],
    ["FC", "Slides"],
    ["FD", "OHP transparencies"],
    ["FE", "Filmstrip"],
    ["FF", "Film"],
    ["FZ", "Other film or transparency format"],
    ["LA", "Digital product license"],
    ["LB", "Digital product license key"],
    ["LC", "Digital product license code"],
    ["MA", "Microform"],
    ["MB", "Microfiche"],
    ["MC", "Microfilm"],
    ["MZ", "Other microform"],
    ["PA", "Miscellaneous print"],
    ["PB", "Address book"],
    ["PC", "Calendar"],
    ["PD", "Cards"],
    ["PE", "Copymasters"],
    ["PF", "Diary or journal"],
    ["PG", "Frieze"],
    ["PH", "Kit"],
    ["PI", "Sheet music"],
    ["PJ", "Postcard book or pack"],
    ["PK", "Poster"],
    ["PL", "Record book"],
    ["PM", "Wallet or folder"],
    ["PN", "Pictures or photographs"],
    ["PO", "Wallchart"],
    ["PP", "Stickers"],
    ["PQ", "Plate (lámina)"],
    ["PR", "Notebook / blank book"],
    ["PS", "Organizer"],
    ["PT", "Bookmark"],
    ["PU", "Leaflet"],
    ["PV", "Book plates"],
    ["PZ", "Other printed item"],
    ["SA", "Multiple-component retail product"],
    ["SB", "Multiple-component retail product, boxed"],
    ["SC", "Multiple-component retail product, slip-cased"],
    ["SD", "Multiple-component retail product, shrink-wrapped"],
    ["SE", "Multiple-component retail product, loose"],
    ["SF", "Multiple-component retail product, part(s) enclosed"],
    ["SG", "Multiple-component retail product, entirely digital"],
    ["SH", "Multiple-component retail product, partly digital"],
    ["VA", "Video"],
    ["VF", "Videodisc"],
    ["VI", "DVD video"],
    ["VJ", "VHS video"],
    ["VK", "Betamax video"],
    ["VL", "VCD"],
    ["VM", "SVCD"],
    ["VN", "HD DVD"],
    ["VO", "Blu-ray"],
    ["VP", "UMD Video"],
    ["VQ", "CBHD"],
    ["VZ", "Other video format"],
    ["XA", "Trade-only material"],
    ["XB", "Dumpbin – empty"],
    ["XC", "Dumpbin – filled"],
    ["XD", "Counterpack – empty"],
    ["XE", "Counterpack – filled"],
    ["XF", "Poster, promotional"],
    ["XG", "Shelf strip"],
    ["XH", "Window piece"],
    ["XI", "Streamer"],
    ["XJ", "Spinner – empty"],
    ["XK", "Large book display"],
    ["XL", "Shrink-wrapped pack"],
    ["XM", "Boxed pack"],
    ["XN", "Pack (outer packaging unspecified)"],
    ["XO", "Spinner – filled"],
    ["XY", "Other point of sale – including retail product"],
    ["XZ", "Other point of sale"],
    ["ZA", "General merchandise"],
    ["ZB", "Doll or figure"],
    ["ZC", "Soft toy"],
    ["ZD", "Toy"],
    ["ZE", "Game"],
    ["ZF", "T-shirt"],
    ["ZG", "E-book reader"],
    ["ZH", "Tablet computer"],
    ["ZI", "Audiobook player"],
    ["ZJ", "Jigsaw"],
    ["ZK", "Mug"],
    ["ZL", "Tote bag"],
    ["ZM", "Tableware"],
    ["ZN", "Umbrella"],
    ["ZO", "Paints, crayons, pencils"],
    ["ZP", "Handicraft kit"],
    ["ZX", "Other toy / game accessories"],
    ["ZY", "Other apparel"],
    ["ZZ", "Other merchandise"],
  ]);

  // List 151 — Contributor place relator
  _lists[151] = new Map([
    ["00", "Associated with"],
    ["01", "Born in"],
    ["02", "Died in"],
    ["03", "Formerly resided in"],
    ["04", "Currently resides in"],
    ["05", "Educated in"],
    ["06", "Worked in"],
    ["07", "Flourished in"],
    ["08", "Citizen of"],
    ["09", "Registered in"],
    ["10", "Operating from"],
    ["11", "Eligible for geographical marketing programs"],
    ["12", "Indigenous to (Indigenous geographies or territorialities)"],
  ]);

  // List 152 — Illustrated / not illustrated
  _lists[152] = new Map([
    ["01", "No"],
    ["02", "Yes"],
  ]);

  // List 153 — Text type
  _lists[153] = new Map([
    ["01", "Sender-defined text"],
    ["02", "Short description / annotation"],
    ["03", "Description"],
    ["04", "Table of contents"],
    ["05", "Primary cover copy"],
    ["06", "Review quote"],
    ["07", "Review quote: previous edition"],
    ["08", "Review quote: previous work"],
    ["09", "Endorsement"],
    ["10", "Promotional headline"],
    ["11", "Feature"],
    ["12", "Biographical note"],
    ["13", "Publisher’s notice"],
    ["14", "Excerpt"],
    ["15", "Index"],
    ["16", "Short description / annotation for collection"],
    ["17", "Description for collection"],
    ["18", "New feature"],
    ["19", "Version history"],
    ["20", "Open access statement"],
    ["21", "Digital exclusivity statement"],
    ["22", "Official recommendation"],
    ["23", "JBPA description"],
    ["24", "schema.org snippet"],
    ["25", "Errata"],
    ["26", "Introduction"],
    ["27", "Secondary cover copy"],
    ["28", "Full cast and credit list"],
    ["29", "Bibliography"],
    ["30", "Abstract"],
    ["31", "Rules or instructions"],
    ["32", "List of contents"],
    ["33", "Short description / annotation for imprint"],
    ["34", "Description for imprint"],
    ["35", "Short description / annotation for publisher"],
    ["36", "Description for publisher"],
    ["37", "Cover line"],
    ["38", "Special cover statement"],
    ["39", "List of bonus contents"],
  ]);

  // List 154 — Content audience
  _lists[154] = new Map([
    ["00", "Unrestricted"],
    ["01", "Restricted"],
    ["02", "Booktrade"],
    ["03", "End customers"],
    ["04", "Librarians"],
    ["05", "Teachers"],
    ["06", "Students"],
    ["07", "Press"],
    ["08", "Shopping comparison service"],
    ["09", "Search engine index"],
    ["10", "Social media"],
    ["11", "Children"],
    ["12", "Teens"],
  ]);

  // List 155 — Content date role
  _lists[155] = new Map([
    ["01", "Publication date"],
    ["04", "Broadcast date"],
    ["14", "From date"],
    ["15", "Until date"],
    ["17", "Last updated"],
    ["24", "From… until date"],
    ["27", "Available from"],
    ["28", "Available until"],
    ["31", "Associated start date"],
    ["32", "Associated end date"],
  ]);

  // List 156 — Cited content type
  _lists[156] = new Map([
    ["01", "Review"],
    ["02", "Bestseller list"],
    ["03", "Media mention"],
    ["04", "‘One locality, one book’ program"],
    ["05", "Curated list"],
    ["06", "Commentary / discussion"],
    ["07", "Interview"],
    ["08", "Soundtrack"],
  ]);

  // List 157 — Content source type
  _lists[157] = new Map([
    ["01", "Printed media"],
    ["02", "Website"],
    ["03", "Radio"],
    ["04", "TV"],
    ["05", "Podcast"],
    ["06", "Social media"],
    ["07", "Playlist"],
  ]);

  // List 158 — Resource content type
  _lists[158] = new Map([
    ["01", "Front cover"],
    ["02", "Back cover"],
    ["56", "Spine image"],
    ["57", "Spine panorama image"],
    ["03", "Cover / pack"],
    ["04", "Contributor picture"],
    ["05", "Collection image / artwork"],
    ["06", "Collection logo"],
    ["07", "Product image / artwork"],
    ["08", "Product logo"],
    ["09", "Publisher logo"],
    ["10", "Imprint logo"],
    ["11", "Contributor interview"],
    ["12", "Contributor presentation"],
    ["13", "Contributor reading"],
    ["14", "Contributor event schedule"],
    ["15", "Sample content"],
    ["16", "Widget"],
    ["17", "Review"],
    ["18", "Commentary / discussion"],
    ["19", "Reading group guide"],
    ["20", "Teacher’s guide"],
    ["21", "Feature article"],
    ["22", "Character ‘interview’"],
    ["23", "Wallpaper / screensaver"],
    ["24", "Press release"],
    ["25", "Table of contents"],
    ["26", "Trailer"],
    ["27", "Cover thumbnail"],
    ["28", "Full content"],
    ["29", "Full cover"],
    ["30", "Master brand logo"],
    ["31", "Description"],
    ["32", "Index"],
    ["33", "Student’s guide"],
    ["34", "Publisher’s catalogue"],
    ["35", "Online advertisement panel"],
    ["36", "Online advertisement page"],
    ["37", "Promotional event material"],
    ["38", "Digital review copy"],
    ["39", "Instructional material"],
    ["40", "Errata"],
    ["41", "Introduction"],
    ["42", "Collection description"],
    ["43", "Bibliography"],
    ["44", "Abstract"],
    ["45", "Cover holding image"],
    ["46", "Rules or instructions"],
    ["47", "Transcript"],
    ["48", "Full cast and credit list"],
    ["49", "Image for social media"],
    ["50", "Supplementary learning resources"],
    ["51", "Cover flap image"],
    ["52", "Warning label"],
    ["53", "Product safety contacts"],
    ["54", "Page edge deco image"],
    ["55", "Endpaper deco image"],
    ["99", "License"],
  ]);

  // List 159 — Resource mode
  _lists[159] = new Map([
    ["01", "Application"],
    ["02", "Audio"],
    ["03", "Image"],
    ["04", "Text"],
    ["05", "Video"],
    ["06", "Multi-mode"],
  ]);

  // List 160 — Resource feature type
  _lists[160] = new Map([
    ["01", "Required credit"],
    ["02", "Caption"],
    ["03", "Copyright holder"],
    ["04", "Length in minutes"],
    ["05", "ISNI of resource contributor"],
    ["06", "Proprietary ID of resource contributor"],
    ["07", "Resource alternative text"],
    ["08", "Background color of image resource"],
    ["09", "Attribute of product image resource"],
    ["10", "Background color of page"],
    ["11", "ORCID of resource contributor"],
  ]);

  // List 161 — Resource form
  _lists[161] = new Map([
    ["01", "Linkable resource"],
    ["02", "Downloadable file"],
    ["03", "Embeddable application"],
  ]);

  // List 162 — Resource version feature type
  _lists[162] = new Map([
    ["01", "File format"],
    ["02", "Image height in pixels"],
    ["03", "Image width in pixels"],
    ["04", "Filename"],
    ["05", "Approximate download file size in megabytes"],
    ["06", "MD5 hash value"],
    ["07", "Exact download file size in bytes"],
    ["08", "SHA-256 hash value"],
    ["09", "ISCC"],
    ["10", "Previous filename"],
  ]);

  // List 163 — Publishing date role
  _lists[163] = new Map([
    ["01", "Publication date"],
    ["02", "Sales embargo date"],
    ["09", "Public announcement date"],
    ["10", "Trade announcement date"],
    ["11", "Date of first publication"],
    ["12", "Latest reprint date"],
    ["13", "Out-of-print / permanently withdrawn date"],
    ["16", "Latest reissue date"],
    ["19", "Publication date of print counterpart"],
    ["20", "Date of first publication in original language"],
    ["21", "Forthcoming reissue date"],
    ["22", "Expected availability date after temporary withdrawal"],
    ["23", "Review embargo date"],
    ["25", "Publisher’s reservation order deadline"],
    ["26", "Forthcoming reprint date"],
    ["27", "Preorder embargo date"],
    ["28", "Transfer date"],
    ["29", "Date of production"],
    ["30", "Streaming embargo date"],
    ["31", "Subscription embargo date"],
    ["32", "Download embargo date"],
    ["33", "Purchase embargo date"],
    ["35", "CIP date"],
    ["36", "Date of first unabridged publication"],
    ["37", "Year of publication"],
    ["38", "Preview embargo date"],
    ["39", "Library embargo date"],
  ]);

  // List 164 — Work relation
  _lists[164] = new Map([
    ["01", "Manifestation of"],
    ["02", "Derived from"],
    ["03", "Related work is derived from this"],
    ["04", "Other work in same (bibliographic) collection"],
    ["05", "Other work by same contributor"],
    ["06", "Manifestation of original work"],
    ["21", "Derived from by abridgement"],
    ["22", "Derived from by annotation"],
    ["23", "Derived from by compilation"],
    ["24", "Derived from by criticism"],
    ["25", "Derived from by excerption"],
    ["26", "Derived from by expurgation"],
    ["27", "Derived from by addition (of non-text material)"],
    ["28", "Derived from by revision"],
    ["29", "Derived from via translation"],
    ["30", "Derived from via adaptation"],
    ["31", "Derived from by subtraction (of non-text material)"],
    ["41", "Related work is derived from this by abridgement"],
    ["42", "Related work is derived from this by annotation"],
    ["43", "Related work is derived from this by compilation"],
    ["44", "Related work is derived from this by criticism"],
    ["45", "Related work is derived from this by excerption"],
    ["46", "Related work is derived from this by expurgation"],
    ["47", "Related work is derived from this by addition (of non-text material)"],
    ["48", "Related work is derived from this by revision"],
    ["49", "Related work is derived from this via translation"],
    ["50", "Related work is derived from this via adaptation"],
    ["51", "Derived from this by subtraction (of non-text material)"],
    ["98", "Manifestation of LRM work"],
    ["99", "Manifestation of LRM expression"],
  ]);

  // List 165 — Supplier own code type
  _lists[165] = new Map([
    ["01", "Supplier’s sales classification"],
    ["02", "Supplier’s bonus eligibility"],
    ["03", "Publisher’s sales classification"],
    ["04", "Supplier’s pricing restriction classification"],
    ["05", "Supplier’s sales expectation"],
    ["06", "Publisher’s sales expectation"],
    ["07", "Supplier’s order routing eligibility"],
  ]);

  // List 166 — Supply date role
  _lists[166] = new Map([
    ["02", "Sales embargo date"],
    ["08", "Expected availability date"],
    ["18", "Last date for returns"],
    ["25", "Reservation order deadline"],
    ["29", "Last redownload date"],
    ["30", "Last TPM date"],
    ["34", "Expected warehouse date"],
    ["50", "New supplier start date"],
    ["51", "Supplier end date"],
  ]);

  // List 167 — Price condition type
  _lists[167] = new Map([
    ["00", "No conditions"],
    ["01", "Includes updates"],
    ["02", "Must also purchase updates"],
    ["03", "Updates available"],
    ["04", "Linked subsequent purchase price"],
    ["05", "Linked prior purchase price"],
    ["06", "Linked price"],
    ["07", "Auto-renewing"],
    ["08", "Combined price"],
    ["10", "Rental duration"],
    ["11", "Rental to purchase"],
    ["12", "Rental extension"],
  ]);

  // List 168 — Price condition quantity type
  _lists[168] = new Map([
    ["01", "Time period"],
    ["02", "Number of updates"],
    ["03", "Number of linked products"],
    ["04", "Number of copies of this product"],
    ["05", "Minimum number of linked products"],
    ["06", "Maximum number of copies of this product"],
  ]);

  // List 169 — Quantity unit
  _lists[169] = new Map([
    ["00", "Units"],
    ["07", "Days"],
    ["08", "Weeks"],
    ["09", "Months"],
    ["10", "Years"],
    ["20", "Classes"],
  ]);

  // List 170 — Discount type
  _lists[170] = new Map([
    ["01", "Rising discount"],
    ["02", "Rising discount (cumulative)"],
    ["03", "Progressive discount"],
    ["04", "Progressive discount (cumulative)"],
  ]);

  // List 171 — Tax type
  _lists[171] = new Map([
    ["01", "Value-added tax"],
    ["02", "Sales tax"],
    ["03", "ECO"],
  ]);

  // List 172 — Currency zone
  _lists[172] = new Map([
    ["EUR", "Eurozone"],
  ]);

  // List 173 — Price date role
  _lists[173] = new Map([
    ["14", "From date"],
    ["15", "Until date"],
    ["24", "From… until date"],
  ]);

  // List 174 — Printed on product
  _lists[174] = new Map([
    ["01", "No"],
    ["02", "Yes"],
  ]);

  // List 175 — Product form detail
  _lists[175] = new Map([
    ["A101", "CD standard audio format"],
    ["A102", "SACD super audio format"],
    ["A103", "MP3 format"],
    ["A104", "WAV format"],
    ["A105", "Real Audio format"],
    ["A106", "WMA"],
    ["A107", "AAC"],
    ["A108", "Ogg/Vorbis"],
    ["A109", "Audible"],
    ["A110", "FLAC"],
    ["A111", "AIFF"],
    ["A112", "ALAC"],
    ["A113", "W3C Audiobook format"],
    ["A201", "DAISY 2: full audio with title only (no navigation)"],
    ["A202", "DAISY 2: full audio with navigation (no text)"],
    ["A203", "DAISY 2: full audio with navigation and partial text"],
    ["A204", "DAISY 2: full audio with navigation and full text"],
    ["A205", "DAISY 2: full text with navigation and partial audio"],
    ["A206", "DAISY 2: full text with navigation and no audio"],
    ["A207", "DAISY 3: full audio with title only (no navigation)"],
    ["A208", "DAISY 3: full audio with navigation (no text)"],
    ["A209", "DAISY 3: full audio with navigation and partial text"],
    ["A210", "DAISY 3: full audio with navigation and full text"],
    ["A211", "DAISY 3: full text with navigation and partial audio"],
    ["A212", "DAISY 3: full text with navigation and no audio"],
    ["A301", "Standalone audio"],
    ["A302", "Readalong audio"],
    ["A303", "Playalong audio"],
    ["A304", "Speakalong audio"],
    ["A305", "Synchronized audio"],
    ["A310", "Sound effects"],
    ["A311", "Background music"],
    ["A312", "Without background sounds"],
    ["A400", "64kbits/s"],
    ["A401", "128kbits/s"],
    ["A402", "192kbits/s"],
    ["A403", "256kbits/s"],
    ["A404", "320kbits/s"],
    ["A410", "Mono"],
    ["A420", "Stereo"],
    ["A421", "Stereo 2.1"],
    ["A441", "Surround 4.1"],
    ["A451", "Surround 5.1"],
    ["A471", "Dolby Atmos"],
    ["B101", "Mass market (rack) paperback"],
    ["B102", "Trade paperback (US)"],
    ["B103", "Digest format paperback"],
    ["B104", "A-format paperback"],
    ["B105", "B-format paperback"],
    ["B106", "Trade paperback (UK)"],
    ["B107", "Tall rack paperback (US)"],
    ["B108", "A5 size Tankobon"],
    ["B109", "JIS B5 size Tankobon"],
    ["B110", "JIS B6 size Tankobon"],
    ["B111", "A6 size Bunko"],
    ["B112", "B40-dori Shinsho"],
    ["B113", "Pocket (Sweden, Norway, France)"],
    ["B114", "Storpocket (Sweden)"],
    ["B115", "Kartonnage (Sweden)"],
    ["B116", "Flexband (Sweden)"],
    ["B117", "Mook / Bookazine"],
    ["B118", "Dwarsligger"],
    ["B119", "46 size"],
    ["B120", "46-Henkei size"],
    ["B121", "A4"],
    ["B122", "A4-Henkei size"],
    ["B123", "A5-Henkei size"],
    ["B124", "B5-Henkei size"],
    ["B125", "B6-Henkei size"],
    ["B126", "AB size"],
    ["B127", "JIS B7 size"],
    ["B128", "Kiku size"],
    ["B129", "Kiku-Henkei size"],
    ["B130", "JIS B4 size"],
    ["B131", "Paperback (DE)"],
    ["B132", "Libro de bolsillo"],
    ["B133", "Pocket-sized"],
    ["B134", "A5"],
    ["B135", "Mass market max paperback"],
    ["B139", "Comic book size (US)"],
    ["B140", "Comic album size (Euro)"],
    ["B141", "B4-Henkei size"],
    ["B201", "Coloring / join-the-dot book"],
    ["B202", "Lift-the-flap book"],
    ["B204", "Miniature book"],
    ["B205", "Moving picture / flicker book"],
    ["B206", "Pop-up book"],
    ["B207", "Scented / ‘smelly’ book"],
    ["B208", "Sound story / ‘noisy’ book"],
    ["B209", "Sticker book"],
    ["B210", "Touch-and-feel book"],
    ["B212", "Die-cut book"],
    ["B213", "Book-as-toy"],
    ["B214", "Soft-to-touch book"],
    ["B215", "Fuzzy-felt book"],
    ["B216", "Press-out pieces"],
    ["B221", "Picture book"],
    ["B222", "‘Carousel’ book"],
    ["B223", "Pull-the-tab book"],
    ["B224", "‘Wordless’ book"],
    ["B225", "Cut-out pieces"],
    ["B301", "Loose leaf or partwork – sheets / parts and binder / wallet"],
    ["B302", "Loose leaf or partwork – binder / wallet only"],
    ["B303", "Loose leaf or partwork – sheets / parts only"],
    ["B304", "Sewn"],
    ["B305", "Unsewn / adhesive bound"],
    ["B306", "Library binding"],
    ["B307", "Reinforced binding"],
    ["B308", "Half bound"],
    ["B309", "Quarter bound"],
    ["B310", "Saddle-sewn"],
    ["B311", "Comb bound"],
    ["B312", "Wire-O"],
    ["B313", "Concealed wire"],
    ["B314", "Coiled wire bound"],
    ["B315", "Trade binding"],
    ["B316", "Swiss binding"],
    ["B317", "Notched binding"],
    ["B318", "Lay-flat binding"],
    ["B319", "Flush cut binding"],
    ["B320", "Rounded spine"],
    ["B321", "Square spine"],
    ["B400", "Self-covered"],
    ["B401", "Cloth over boards"],
    ["B402", "Paper over boards"],
    ["B403", "Leather, real"],
    ["B404", "Leather, imitation"],
    ["B405", "Leather, bonded"],
    ["B406", "Vellum"],
    ["B407", "Head and tail bands"],
    ["B419", "Decorated page edges"],
    ["B408", "Decorated endpapers"],
    ["B409", "Cloth"],
    ["B410", "Imitation cloth"],
    ["B411", "Velvet"],
    ["B412", "Flexible plastic / vinyl cover"],
    ["B413", "Plastic-covered"],
    ["B414", "Vinyl-covered"],
    ["B415", "Laminated cover"],
    ["B416", "Card cover"],
    ["B417", "Duplex-printed cover"],
    ["B418", "Paper cover"],
    ["B420", "Delicate cover / jacket finish"],
    ["B421", "Embossed cover"],
    ["B422", "Foil (on cover)"],
    ["B423", "Foil (on jacket)"],
    ["B424", "Temporary stickers on cover or jacket"],
    ["B425", "Permanent stickers on cover or jacket"],
    ["B426", "Spine panorama"],
    ["B427", "Belly band"],
    ["B501", "With dust jacket"],
    ["B502", "With printed dust jacket"],
    ["B503", "With translucent dust cover"],
    ["B504", "With flaps"],
    ["B505", "With thumb index"],
    ["B506", "With ribbon marker(s)"],
    ["B507", "With zip fastener"],
    ["B508", "With button snap fastener"],
    ["B509", "With leather edge lining"],
    ["B510", "Rough front"],
    ["B511", "Foldout"],
    ["B512", "Wide margin"],
    ["B513", "With fastening strap"],
    ["B514", "With perforated pages"],
    ["B515", "Acid-free paper"],
    ["B516", "Archival paper"],
    ["B517", "With elasticated strap"],
    ["B518", "With serialized authenticity token"],
    ["B519", "With dust jacket poster"],
    ["B520", "Rounded corners"],
    ["B521", "Splashproof"],
    ["B522", "Mineral paper"],
    ["B523", "With accessibility claim ticket"],
    ["B524", "Plastic paper"],
    ["B525", "With wraparound foredge flap"],
    ["B601", "Turn-around book"],
    ["B602", "Unflipped manga format"],
    ["B603", "Back-to-back book"],
    ["B604", "Flipped manga format"],
    ["B605", "Variant turn-around book"],
    ["B606", "Page progression LTR"],
    ["B607", "Page progression RTL"],
    ["B608", "Page progression TTB"],
    ["B609", "Page progression other"],
    ["B610", "Syllabification"],
    ["B611", "Upper case only"],
    ["B701", "UK Uncontracted Braille"],
    ["B702", "UK Contracted Braille"],
    ["B703", "US Braille"],
    ["B704", "US Uncontracted Braille"],
    ["B705", "US Contracted Braille"],
    ["B706", "Unified English Braille"],
    ["B707", "Moon"],
    ["B708", "Unified English Uncontracted Braille"],
    ["B709", "Unified English Contracted Braille"],
    ["B750", "Tactile images"],
    ["B751", "Lenticular images"],
    ["B752", "Anaglyph images"],
    ["C750", "Raised 3D relief"],
    ["D101", "Real Video format"],
    ["D102", "Quicktime format"],
    ["D103", "AVI format"],
    ["D104", "Windows Media Video format"],
    ["D105", "MPEG-4"],
    ["D201", "MS-DOS"],
    ["D202", "Windows"],
    ["D203", "Macintosh"],
    ["D204", "UNIX / LINUX"],
    ["D205", "Other operating system(s)"],
    ["D206", "Palm OS"],
    ["D207", "Windows Mobile"],
    ["D301", "Microsoft XBox"],
    ["D302", "Nintendo Gameboy Color"],
    ["D303", "Nintendo Gameboy Advanced"],
    ["D304", "Nintendo Gameboy"],
    ["D305", "Nintendo Gamecube"],
    ["D306", "Nintendo 64"],
    ["D307", "Sega Dreamcast"],
    ["D308", "Sega Genesis / Megadrive"],
    ["D309", "Sega Saturn"],
    ["D310", "Sony PlayStation 1"],
    ["D311", "Sony PlayStation 2"],
    ["D312", "Nintendo Dual Screen"],
    ["D313", "Sony PlayStation 3"],
    ["D314", "Microsoft Xbox 360"],
    ["D315", "Nintendo Wii"],
    ["D316", "Sony PlayStation Portable (PSP)"],
    ["D317", "Sony PlayStation 3"],
    ["D318", "Sony PlayStation 4"],
    ["D319", "Sony PlayStation Vita"],
    ["D320", "Microsoft Xbox One"],
    ["D321", "Nintendo Switch"],
    ["D322", "Nintendo Wii U"],
    ["D323", "Sony PlayStation 5"],
    ["D324", "Microsoft Xbox Series X / S"],
    ["E100", "Other"],
    ["E101", "EPUB"],
    ["E102", "OEB"],
    ["E103", "DOC"],
    ["E104", "DOCX"],
    ["E105", "HTML"],
    ["E106", "ODF"],
    ["E107", "PDF"],
    ["E108", "PDF/A"],
    ["E109", "RTF"],
    ["E110", "SGML"],
    ["E111", "TCR"],
    ["E112", "TXT"],
    ["E113", "XHTML"],
    ["E114", "zTXT"],
    ["E115", "XPS"],
    ["E116", "Amazon Kindle"],
    ["E117", "BBeB"],
    ["E118", "DXReader"],
    ["E119", "EBL"],
    ["E120", "Ebrary"],
    ["E121", "eReader"],
    ["E122", "Exebook"],
    ["E123", "Franklin eBookman"],
    ["E124", "Gemstar Rocketbook"],
    ["E125", "iSilo"],
    ["E126", "Microsoft Reader"],
    ["E127", "Mobipocket"],
    ["E128", "MyiLibrary"],
    ["E129", "NetLibrary"],
    ["E130", "Plucker"],
    ["E131", "VitalBook"],
    ["E132", "Vook"],
    ["E133", "Google Edition"],
    ["E134", "Book ‘app’ for iOS"],
    ["E135", "Book ‘app’ for Android"],
    ["E136", "Book ‘app’ for other operating system"],
    ["E139", "CEB"],
    ["E140", "CEBX"],
    ["E141", "iBook"],
    ["E142", "ePIB"],
    ["E143", "SCORM"],
    ["E144", "EBP"],
    ["E145", "Page Perfect"],
    ["E146", "BRF"],
    ["E147", "Erudit"],
    ["E148", "Amazon Kindle Print Replica"],
    ["E149", "Comic Book Archive"],
    ["E150", "EPUB/A"],
    ["E151", "eBraille"],
    ["E200", "Reflowable"],
    ["E201", "Fixed format"],
    ["E202", "Readable offline"],
    ["E203", "Requires network connection"],
    ["E204", "Content removed"],
    ["E205", "Visible page numbering"],
    ["E206", "No preferred page progression"],
    ["E210", "Landscape"],
    ["E211", "Portrait"],
    ["E212", "Square"],
    ["E213", "Vertical scrolling"],
    ["E221", "5:4"],
    ["E222", "4:3"],
    ["E223", "3:2"],
    ["E224", "16:10"],
    ["E225", "16:9"],
    ["E226", "18:9"],
    ["E227", "21:9"],
    ["L101", "Laminated"],
    ["P091", "Calendar with write-in space"],
    ["P092", "Calendar without write-in space"],
    ["P096", "Multiple months per page"],
    ["P097", "One month per page"],
    ["P098", "One week per page"],
    ["P099", "One day per page"],
    ["P101", "Desk calendar or diary"],
    ["P102", "Mini calendar or pocket diary"],
    ["P103", "Engagement calendar or Appointment diary"],
    ["P104", "Day by day calendar"],
    ["P105", "Poster calendar"],
    ["P106", "Wall calendar"],
    ["P107", "Perpetual calendar or diary"],
    ["P108", "Advent calendar"],
    ["P109", "Bookmark calendar"],
    ["P110", "Student or Academic calendar or diary"],
    ["P111", "Project calendar"],
    ["P112", "Almanac calendar"],
    ["P113", "Other calendar, diary or organiser"],
    ["P114", "Other calendar or organiser product"],
    ["P115", "Family planner"],
    ["P116", "Postcard calendar"],
    ["P131", "Blank calendar"],
    ["P132", "Panoramic calendar"],
    ["P133", "Columnar calendar"],
    ["P134", "Square calendar"],
    ["P120", "Picture story cards"],
    ["P121", "Flash cards"],
    ["P122", "Reference cards"],
    ["P123", "Recreation cards"],
    ["P124", "Postcards"],
    ["P125", "Greeting cards"],
    ["P126", "Gift cards"],
    ["P127", "Certificate cards"],
    ["P201", "Hardback (stationery)"],
    ["P202", "Paperback / softback (stationery)"],
    ["P203", "Spiral bound (stationery)"],
    ["P204", "Leather / fine binding (stationery)"],
    ["P301", "With hanging strips"],
    ["P305", "Single-sided"],
    ["P306", "Double-sided"],
    ["V201", "PAL"],
    ["V202", "NTSC"],
    ["V203", "SECAM"],
    ["V205", "HD"],
    ["V206", "UHD"],
    ["V207", "3D video"],
    ["V210", "Closed captions"],
    ["V211", "Open captions"],
    ["V212", "Transcript"],
    ["V213", "Sign language interpretation"],
    ["V214", "Textual description of audio"],
    ["V215", "Audio description of video"],
    ["V220", "Home use"],
    ["V221", "Classroom use"],
    ["Z101", "Wooden"],
    ["Z102", "Plastic"],
    ["Z103", "Board"],
    ["Z111", "3D puzzle"],
    ["Z112", "Noisy kit / puzzle / toy"],
    ["Z113", "Puppet"],
    ["Z121", "Extra large pieces"],
  ]);

  // List 176 — Operating system
  _lists[176] = new Map([
    ["01", "Android"],
    ["02", "BlackBerry OS"],
    ["03", "iOS"],
    ["04", "Linux"],
    ["05", "Mac OS"],
    ["06", "Mac OS X"],
    ["07", "Palm OS"],
    ["08", "webOS"],
    ["09", "Symbian"],
    ["10", "Windows"],
    ["11", "Windows CE"],
    ["12", "Windows Mobile"],
    ["13", "Mac OS"],
    ["14", "Windows Phone 7"],
  ]);

  // List 177 — Person / organization date role
  _lists[177] = new Map([
    ["50", "Date of birth"],
    ["51", "Date of death"],
    ["56", "Flourished around"],
  ]);

  // List 178 — Supporting resource file format
  _lists[178] = new Map([
    ["A103", "MP3"],
    ["A104", "WAV"],
    ["A105", "Real Audio"],
    ["A106", "WMA"],
    ["A107", "AAC"],
    ["A108", "Ogg/Vorbis"],
    ["A111", "AIFF"],
    ["D101", "Real Video"],
    ["D102", "Quicktime"],
    ["D103", "AVI"],
    ["D104", "WMV"],
    ["D105", "MPEG-4"],
    ["D106", "FLV"],
    ["D107", "SWF"],
    ["D108", "3GP"],
    ["D109", "WebM"],
    ["D401", "PDF (single page)"],
    ["D501", "GIF"],
    ["D502", "JPEG"],
    ["D503", "PNG"],
    ["D504", "TIFF"],
    ["D505", "BMP"],
    ["D506", "JP2"],
    ["D507", "PSD"],
    ["D508", "EPS"],
    ["D509", "WebP"],
    ["D510", "SVG"],
    ["D511", "HEIF"],
    ["E101", "EPUB"],
    ["E105", "HTML"],
    ["E107", "PDF"],
    ["E112", "TXT"],
    ["E113", "XHTML"],
    ["E115", "XPS"],
    ["E116", "Amazon Kindle"],
    ["E139", "CEB"],
    ["E140", "CEBX"],
  ]);

  // List 179 — Price code type
  _lists[179] = new Map([
    ["01", "Proprietary price coding scheme"],
    ["02", "Finnish Pocket Book price code"],
    ["03", "Finnish Miki Book price code"],
  ]);

  // List 184 — EU General Product Safety Regulation and Toy Safety Directive hazard warning type
  _lists[184] = new Map([
    ["00", "No warning"],
    ["01", "Carries ‘CE’ logo"],
    ["02", "Carries minimum age warning"],
    ["03", "Carries EU Toy Safety Directive ‘Unsuitable for children ages 0–3’ warning logo"],
    ["04", "Carries EU Toy Safety Directive hazard warning"],
    ["05", "Carries other text associated with toy safety"],
    ["06", "Material Safety Data Sheet available"],
    ["07", "Declaration of Conformity available"],
    ["08", "Carries EN71 conformity statement"],
    ["09", "Carries ‘UKCA’ logo"],
  ]);

  // List 196 — E-publication Accessibility Details
  _lists[196] = new Map([
    ["00", "Accessibility summary"],
    ["01", "LIA Compliance Scheme"],
    ["02", "EPUB Accessibility Specification 1.0 A"],
    ["03", "EPUB Accessibility Specification 1.0 AA"],
    ["04", "EPUB Accessibility Specification 1.1"],
    ["05", "PDF/UA-1"],
    ["06", "PDF/UA-2"],
    ["08", "Unknown accessibility"],
    ["09", "Inaccessible, or known limited accessibility"],
    ["10", "No reading system accessibility options actively disabled (except)"],
    ["11", "Table of contents navigation"],
    ["12", "Index navigation"],
    ["13", "Single logical reading order"],
    ["14", "Short alternative textual descriptions"],
    ["15", "Full alternative textual descriptions"],
    ["16", "Visualized data also available as non-graphical data"],
    ["17", "Accessible math content as MathML"],
    ["18", "Accessible chemistry content as ChemML"],
    ["19", "Print-equivalent page numbering"],
    ["20", "Synchronized pre-recorded audio"],
    ["21", "Text-to-speech hinting provided"],
    ["22", "Language tagging provided"],
    ["24", "Dyslexia readability"],
    ["25", "Use of color is not sole means of conveying information"],
    ["26", "Use of high contrast between text and background color"],
    ["27", "Use of high contrast between foreground and background audio"],
    ["28", "Full alternative audio descriptions"],
    ["29", "Next / Previous structural navigation"],
    ["30", "ARIA roles provided"],
    ["31", "Accessible controls provided"],
    ["32", "Landmark navigation"],
    ["34", "Accessible chemistry content (as MathML)"],
    ["35", "Accessible math content (as LaTeX)"],
    ["36", "Appearance of all textual content can be modified"],
    ["37", "Use of ultra-high contrast between text foreground and background"],
    ["38", "Unusual words or abbreviations explained"],
    ["39", "Supplementary material to an audiobook is accessible"],
    ["40", "Link purposes clear"],
    ["41", "Page list navigation"],
    ["51", "All non-decorative content supports reading via pre-recorded audio"],
    ["52", "All non-decorative content supports reading without sight"],
    ["53", "Described math content"],
    ["54", "Accessible chemistry content (as LaTeX)"],
    ["75", "EAA exception 1 – Micro-enterprises"],
    ["76", "EAA exception 2 – Disproportionate burden"],
    ["77", "EAA exception 3 – Fundamental alteration"],
    ["80", "WCAG v2.0"],
    ["81", "WCAG v2.1"],
    ["82", "WCAG v2.2"],
    ["84", "WCAG level A"],
    ["85", "WCAG level AA"],
    ["86", "WCAG level AAA"],
    ["88", "Certifier credentialled by (name)"],
    ["89", "Certifier credentialled by (URL)"],
    ["90", "Compliance certification by (name)"],
    ["91", "Latest accessibility assessment date"],
    ["92", "Accessibility addendum"],
    ["93", "Compliance certification by (URL)"],
    ["94", "Compliance web page for detailed accessibility information"],
    ["95", "Trusted intermediary’s web page for detailed accessibility information"],
    ["96", "Publisher’s web page for detailed accessibility information"],
    ["97", "Compatibility tested"],
    ["98", "Trusted Intermediary contact"],
    ["99", "Publisher contact for further accessibility information"],
  ]);

  // List 197 — Collection sequence type
  _lists[197] = new Map([
    ["01", "Proprietary collection sequence type"],
    ["02", "Title order"],
    ["03", "Publication order"],
    ["04", "Temporal / narrative order"],
    ["05", "Original publication order"],
    ["06", "Suggested reading order"],
    ["07", "Suggested display order"],
  ]);

  // List 198 — Product contact role
  _lists[198] = new Map([
    ["00", "Metadata contact"],
    ["01", "Accessibility request contact"],
    ["02", "Promotional contact"],
    ["03", "Advertising contact"],
    ["04", "Review copy contact"],
    ["05", "Evaluation copy contact"],
    ["06", "Permissions contact"],
    ["07", "Return authorization contact"],
    ["08", "CIP / Legal deposit contact"],
    ["09", "Rights and licensing contact"],
    ["10", "Product safety contact"],
    ["11", "Product raw materials contact"],
    ["99", "Customer services contact"],
  ]);

  // List 203 — ONIX Adult Audience rating
  _lists[203] = new Map([
    ["00", "Unrated"],
    ["01", "Any adult audience"],
    ["02", "Content advice"],
    ["03", "Content advice (sex)"],
    ["04", "Content advice (violence)"],
    ["05", "Content advice (drugs)"],
    ["06", "Content advice (language)"],
    ["07", "Content advice (intolerance)"],
    ["08", "Content advice (abuse)"],
    ["09", "Content advice (self-harm)"],
    ["10", "Content advice (animal cruelty)"],
    ["11", "Content advice (illness)"],
    ["12", "Content advice (death and grief)"],
    ["13", "Content advice (suicide)"],
  ]);

  // List 204 — ONIX Returns conditions
  _lists[204] = new Map([
    ["00", "Unspecified"],
    ["01", "Consignment"],
    ["02", "Firm sale"],
    ["03", "Sale or return"],
    ["04", "Direct fulfillment"],
    ["05", "Sale or return of saleable copies"],
    ["06", "Sale or return of stripped covers"],
    ["07", "Sale or certified destruction"],
  ]);

  // List 215 — Proximity
  _lists[215] = new Map([
    ["01", "Less than"],
    ["02", "Not more than"],
    ["03", "Exactly"],
    ["04", "Approximately"],
    ["05", "About"],
    ["06", "Not less than"],
    ["07", "More than"],
  ]);

  // List 216 — Velocity metric
  _lists[216] = new Map([
    ["01", "Mean daily sale"],
    ["02", "Maximum daily sale"],
    ["03", "Minimum daily sale"],
    ["04", "Mean weekly sale"],
    ["05", "Maximum weekly sale"],
    ["06", "Minimum weekly sale"],
    ["07", "Mean monthly sale"],
    ["08", "Maximum monthly sale"],
    ["09", "Minimum monthly sale"],
  ]);

  // List 217 — Price identifier type
  _lists[217] = new Map([
    ["01", "Proprietary price identifier scheme"],
    ["02", "Proprietary price point identifier scheme"],
    ["03", "Proprietary price type identifier scheme"],
    ["04", "Proprietary price point and type identifier scheme"],
    ["05", "Proprietary unique price identifier scheme"],
    ["06", "Proprietary product price point identifier scheme"],
    ["07", "Proprietary product price type identifier scheme"],
  ]);

  // List 218 — License expression type
  _lists[218] = new Map([
    ["01", "Human readable"],
    ["02", "Professional readable"],
    ["03", "Human readable additional license"],
    ["04", "Professional readable additional license"],
    ["10", "ONIX-PL"],
    ["20", "ODRL"],
    ["21", "ODRL additional license"],
  ]);

  // List 219 — Rights type
  _lists[219] = new Map([
    ["C", "Copyright"],
    ["P", "Phonogram right"],
    ["D", "Database right"],
  ]);

  // List 220 — E-publication version number
  _lists[220] = new Map([
    ["101A", "EPUB 2.0.1"],
    ["101B", "EPUB 3.0"],
    ["101C", "EPUB 3.0.1"],
    ["101D", "EPUB 3.1"],
    ["101E", "EPUB 3.2"],
    ["101F", "EPUB 3.3"],
    ["107C", "PDF 1.3"],
    ["107D", "PDF 1.4"],
    ["107F", "PDF 1.6"],
    ["107G", "PDF 1.7"],
    ["107J", "PDF 2.0"],
    ["116A", "Kindle mobi 7"],
    ["116B", "Kindle KF8"],
    ["116C", "Kindle KFX"],
  ]);

  // List 221 — Message status
  _lists[221] = new Map([
    ["00", "Message received"],
    ["01", "Message rejected"],
    ["02", "Message part-processed"],
    ["03", "Message processed"],
  ]);

  // List 222 — Message status date role
  _lists[222] = new Map([
    ["01", "Ingest date"],
    ["02", "Export date"],
  ]);

  // List 223 — Status detail code type
  _lists[223] = new Map([
    ["01", "Proprietary status detail code type"],
    ["02", "ONIX Status detail code"],
  ]);

  // List 224 — Status detail type severity
  _lists[224] = new Map([
    ["U", "Unclassifiable"],
    ["I", "Info"],
    ["Q", "Query"],
    ["W", "Warning"],
    ["E", "Error"],
    ["F", "Fatal error"],
  ]);

  // List 225 — Message / Record status detail
  _lists[225] = new Map([
    ["000", "Unknown error"],
    ["001", "Unknown warning"],
  ]);

  // List 226 — Record status
  _lists[226] = new Map([
    ["00", "No record errors"],
    ["01", "No record errors – errors in collateral"],
    ["02", "Record with errors"],
    ["03", "Record rejected"],
    ["09", "Reported previously"],
  ]);

  // List 227 — Chinese School Grade
  _lists[227] = new Map([
    ["P", "Preschool"],
    ["K", "Kindergarten"],
    ["1", "Primary school First grade"],
    ["2", "Primary school Second grade"],
    ["3", "Primary school Third grade"],
    ["4", "Primary school Fourth grade"],
    ["5", "Primary school Fifth grade"],
    ["6", "Primary school Sixth grade"],
    ["7", "Junior secondary school Seventh grade"],
    ["8", "Junior secondary school Eighth grade"],
    ["9", "Junior secondary school Ninth grade"],
    ["10", "Senior secondary school Tenth grade"],
    ["11", "Senior secondary school Eleventh grade"],
    ["12", "Senior secondary school Twelfth grade"],
    ["13", "University first year"],
    ["14", "University second year"],
    ["15", "University third year"],
    ["16", "University fourth year"],
    ["17", "Graduate level"],
  ]);

  // List 228 — Grant identifier type
  _lists[228] = new Map([
    ["01", "Proprietary grant ID scheme"],
    ["06", "DOI"],
  ]);

  // List 229 — Gender – based on ISO 5218
  _lists[229] = new Map([
    ["u", "Unknown or unspecified"],
    ["f", "Female"],
    ["m", "Male"],
  ]);

  // List 230 — Price constraint type
  _lists[230] = new Map([
    ["00", "No price-specific constraints"],
    ["01", "Preview"],
    ["02", "Print"],
    ["03", "Copy / paste"],
    ["06", "Lend"],
    ["07", "Time-limited license"],
    ["08", "Library loan renewal"],
    ["09", "Multi-user license"],
    ["10", "Preview on premises"],
    ["11", "Text and data mining"],
    ["13", "TDM for AI to enhance the private purchase experience"],
    ["14", "TDM for AI to enhance the private reading experience"],
    ["16", "Library loan"],
  ]);

  // List 238 — Brazil educational level
  _lists[238] = new Map([
    ["K", "Educação Infantil"],
    ["1", "Fundamental I 1º ano"],
    ["2", "Fundamental I 2º ano"],
    ["3", "Fundamental I 3º ano"],
    ["4", "Fundamental I 4º ano"],
    ["5", "Fundamental I 5º ano"],
    ["6", "Fundamental II 6º ano"],
    ["7", "Fundamental II 7º ano"],
    ["8", "Fundamental II 8º ano"],
    ["9", "Fundamental II 9º ano"],
    ["10", "Ensino Médio 1º ano"],
    ["11", "Ensino Médio 2º ano"],
    ["12", "Ensino Médio 3º ano"],
    ["13", "Ensino Técnico Integrado"],
    ["14", "Ensino Técnico Concomitante"],
    ["15", "Ensino Técnico Subsequente"],
    ["P", "Ensino pré-vestibular"],
    ["A", "Ensino Superior Graduação Licenciatura/ Bacharelado"],
    ["B", "Ensino Superior Graduação Tecnologia"],
    ["D", "Ensino Superior Pós-graduação Stricto sensu"],
    ["F", "Ensino Superior Pós-graduação Lato sensu"],
  ]);

  // List 239 — Supply contact role
  _lists[239] = new Map([
    ["07", "Return authorization contact"],
    ["10", "Product safety contact"],
    ["11", "Product raw materials contact"],
    ["99", "Customer services contact"],
  ]);

  // List 240 — AV Item type code
  _lists[240] = new Map([
    ["01", "Audiovisual work"],
    ["02", "Front matter"],
    ["03", "Body matter"],
    ["04", "End matter"],
  ]);

  // List 241 — AV Item Identifier type
  _lists[241] = new Map([
    ["01", "Proprietary AV Item ID scheme"],
    ["03", "GTIN-13"],
    ["06", "DOI"],
    ["12", "IMDB"],
    ["18", "ISRC"],
    ["19", "ISAN"],
    ["31", "EIDR Content ID"],
  ]);

  // List 242 — Battery type and safety
  _lists[242] = new Map([
    ["00", "Batteries not required"],
    ["01", "Batteries built in"],
    ["02", "Batteries pre-installed"],
    ["03", "Batteries supplied"],
    ["04", "Batteries required"],
    ["05", "Batteries supplied spare"],
    ["06", "Safety data sheet available"],
    ["07", "Technical data sheet available"],
    ["08", "Rechargeable"],
    ["09", "Non-rechargeable"],
    ["10", "Battery warning text"],
    ["20", "Battery chemistry"],
    ["21", "Lithium-ion"],
    ["22", "Lithium-polymer"],
    ["23", "Lithium-metal"],
    ["24", "Nickel-metal hydride"],
    ["25", "Nickel-Cadmium"],
    ["26", "Zinc-Manganese dioxide"],
    ["27", "Zinc-Carbon"],
    ["28", "Zinc-air"],
    ["29", "Silver oxide"],
    ["97", "WEEE waste"],
    ["98", "Battery safety warning text"],
    ["99", "Battery description"],
  ]);

  // List 243 — Dangerous goods regulations
  _lists[243] = new Map([
    ["00", "Inapplicable"],
    ["01", "GHS"],
    ["02", "Transport"],
    ["03", "Storage"],
    ["04", "Waste"],
    ["05", "Other"],
    ["06", "Safety data sheet available"],
  ]);

  // List 244 — Event identifier type
  _lists[244] = new Map([
    ["01", "Proprietary event ID scheme"],
  ]);

  // List 245 — Event type
  _lists[245] = new Map([
    ["00", "Unspecified – see description"],
    ["01", "Book signing"],
    ["02", "Book reading"],
  ]);

  // List 246 — Event status
  _lists[246] = new Map([
    ["A", "Announced"],
    ["C", "Cancelled"],
  ]);

  // List 247 — Event occurrence date role
  _lists[247] = new Map([
    ["01", "Date of occurrence"],
    ["02", "Date of occurrence end"],
  ]);

  // List 248 — Specification detail code
  _lists[248] = new Map([
    ["A411", "22.05kHz"],
    ["A412", "44.1kHz"],
    ["A413", "48kHz"],
    ["A416", "16-bits per sample"],
    ["A418", "24-bits per sample"],
    ["A424", "ID3v1"],
    ["A425", "ID3v2"],
    ["B001", "Printed long grain"],
    ["B002", "Printed short grain"],
    ["B003", "Printed monochrome"],
    ["B004", "Printed CMYK"],
    ["B005", "Printed higher-quality CMYK"],
    ["B006", "Printed with bleed"],
    ["B007", "Printed higher-quality monochrome"],
  ]);

  // List 249 — Specification feature type
  _lists[249] = new Map([
    ["04", "Filename"],
    ["21", "Audio loudness"],
    ["41", "Paper type"],
    ["42", "Paper weight"],
    ["43", "Paper color"],
    ["44", "Ink color(s)"],
    ["45", "Special finish"],
  ]);

  // List 250 — Resource identifier type
  _lists[250] = new Map([
    ["01", "Proprietary resource ID scheme"],
    ["09", "ISCC"],
  ]);

  // List 251 — Resource role code
  _lists[251] = new Map([
    ["C01", "Cover"],
    ["C02", "Jacket"],
  ]);

  // List 252 — Resource file detail code
  _lists[252] = new Map([
    ["A410", "Mono"],
    ["A411", "22.05kHz"],
    ["A412", "44.1kHz"],
    ["A413", "48kHz"],
    ["A414", "88.2kHz"],
    ["A415", "96kHz"],
    ["A416", "16-bits per sample"],
    ["A417", "20-bits per sample"],
    ["A418", "24-bits per sample"],
    ["A419", "32-bits per sample (FP)"],
    ["A420", "Stereo"],
    ["A421", "Stereo 2.1"],
    ["A422", "ID3v1"],
    ["A423", "ID3v2"],
    ["A441", "Surround 4.1"],
    ["A451", "Surround 5.1"],
    ["B001", "With crop marks"],
    ["B002", "Without crop marks"],
    ["B003", "Monochrome"],
    ["B004", "Preseparated – 2 channels"],
    ["B005", "Preseparated – 3 channels"],
    ["B006", "Preseparated – 4 channels"],
    ["B010", "Composite (CMYK)"],
    ["B011", "Composite (RGB)"],
  ]);

  // List 253 — Resource file feature type
  _lists[253] = new Map([
    ["01", "File format"],
    ["04", "Filename"],
    ["05", "Approximate download file size in megabytes"],
    ["06", "MD5 hash value"],
    ["07", "Exact download file size in bytes"],
    ["08", "SHA-256 hash value"],
    ["31", "Audio loudness"],
  ]);

  // List 254 — Resource file date role code
  _lists[254] = new Map([
    ["17", "Last updated"],
    ["27", "Available from"],
    ["28", "Available until"],
  ]);

  // List 255 — Insert point type
  _lists[255] = new Map([
    ["ALP", "Adjacent to logical page"],
    ["APP", "Adjacent to physical page"],
    ["ATC", "At timecode"],
    ["AHL", "Adjacent to HTML label"],
  ]);

  // List 256 — Resource feature code
  _lists[256] = new Map([
    ["01", "Image shows product in packaging"],
    ["02", "Image shows product (without packaging)"],
    ["03", "Image shows product and accessories"],
    ["04", "Image shows assembled product"],
    ["05", "Image shows product in use"],
    ["06", "Image shows multiple products"],
    ["07", "Image shows detail of product"],
    ["21", "3D perspective ‘front’"],
    ["22", "3D perspective ‘back’"],
    ["23", "3D perspective ‘front foredge’"],
    ["24", "3D perspective ‘back foredge’"],
    ["33", "3D perspective ‘front low’"],
    ["34", "3D perspective ‘back low’"],
    ["35", "3D perspective ‘front foredge low’"],
    ["36", "3D perspective ‘back foredge low’"],
    ["41", "2D front"],
    ["42", "2D back"],
    ["43", "2D spine"],
    ["47", "3D perspective ‘horizontal’"],
    ["48", "3D perspective ‘vertical’"],
    ["49", "3D perspective ‘open’"],
    ["50", "3D perspective ‘closed’"],
  ]);

  // List 257 — Paper color
  _lists[257] = new Map([
    ["BRW", "Bright white"],
    ["WHI", "White"],
    ["OFW", "Off white"],
    ["CRE", "Cream"],
  ]);

  // List 258 — Special finish
  _lists[258] = new Map([
    ["01", "Overall matt laminate"],
    ["02", "Overall gloss laminate"],
    ["03", "Overall textured laminate"],
  ]);

  // List 259 — Collection frequency code
  _lists[259] = new Map([
    ["u", "Unknown"],
    ["i", "Irregular"],
    ["r", "Regular"],
    ["e", "Biennial"],
    ["a", "Annual"],
    ["b", "Biannual"],
    ["t", "Triannual"],
    ["q", "Quarterly"],
    ["s", "Bimonthly"],
    ["m", "Monthly"],
    ["f", "Fortnightly"],
    ["w", "Weekly"],
    ["d", "Daily, or more frequently than weekly"],
    ["x", "No future publications"],
  ]);

  // List 260 — Epublication license date role
  _lists[260] = new Map([
    ["14", "Valid from"],
    ["15", "Valid until"],
    ["24", "From… until date"],
  ]);

  // List 261 — Status detail type severity (master files)
  _lists[261] = new Map([
    ["u", "Unclassifiable"],
    ["i", "Info"],
    ["q", "Query"],
    ["w", "Warning"],
    ["e", "Error"],
    ["f", "Fatal error"],
  ]);

  // List 262 — Carbon / GHG emissions certification scheme
  _lists[262] = new Map([
    ["101", "Blue Angel"],
    ["102", "Cradle to Cradle"],
    ["103", "Nordic Swan"],
    ["104", "ClimatePartner"],
    ["105", "EU Ecolabel"],
    ["201", "ClimateCalc"],
    ["202", "Canadian SPT / Green Book Alliance Carbon calculator"],
    ["501", "V-Label"],
  ]);

  // List 263 — Prize identifier type
  _lists[263] = new Map([
    ["01", "Proprietary prize ID scheme"],
    ["91", "GND"],
    ["B6", "FAST"],
  ]);

  window.OnixViewerCodeLists = Object.create(null);
  window.OnixViewerCodeListMeta = Object.create(null);

  window.OnixViewerCodeLists["AVItemIDType"] = _lists[241];
  window.OnixViewerCodeListMeta["AVItemIDType"] = { listNumber: 241, title: "AV Item Identifier type" };
  window.OnixViewerCodeLists["AVItemType"] = _lists[240];
  window.OnixViewerCodeListMeta["AVItemType"] = { listNumber: 240, title: "AV Item type code" };
  window.OnixViewerCodeLists["AddresseeIDType"] = _lists[44];
  window.OnixViewerCodeListMeta["AddresseeIDType"] = { listNumber: 44, title: "Name identifier type" };
  window.OnixViewerCodeLists["AffiliationIDType"] = _lists[44];
  window.OnixViewerCodeListMeta["AffiliationIDType"] = { listNumber: 44, title: "Name identifier type" };
  window.OnixViewerCodeLists["AgentIDType"] = _lists[92];
  window.OnixViewerCodeListMeta["AgentIDType"] = { listNumber: 92, title: "Supplier identifier type" };
  window.OnixViewerCodeLists["AgentRole"] = _lists[69];
  window.OnixViewerCodeListMeta["AgentRole"] = { listNumber: 69, title: "Agent role" };
  window.OnixViewerCodeLists["AncillaryContentType"] = _lists[25];
  window.OnixViewerCodeListMeta["AncillaryContentType"] = { listNumber: 25, title: "Illustration and other content type" };
  window.OnixViewerCodeLists["AudienceCodeType"] = _lists[29];
  window.OnixViewerCodeListMeta["AudienceCodeType"] = { listNumber: 29, title: "Audience code type" };
  window.OnixViewerCodeLists["AudienceRangePrecision"] = _lists[31];
  window.OnixViewerCodeListMeta["AudienceRangePrecision"] = { listNumber: 31, title: "Audience range precision" };
  window.OnixViewerCodeLists["AudienceRangeQualifier"] = _lists[30];
  window.OnixViewerCodeListMeta["AudienceRangeQualifier"] = { listNumber: 30, title: "Audience range qualifier" };
  window.OnixViewerCodeLists["BarcodeType"] = _lists[141];
  window.OnixViewerCodeListMeta["BarcodeType"] = { listNumber: 141, title: "Barcode indicator" };
  window.OnixViewerCodeLists["BibleContents"] = _lists[82];
  window.OnixViewerCodeListMeta["BibleContents"] = { listNumber: 82, title: "Bible contents" };
  window.OnixViewerCodeLists["BiblePurpose"] = _lists[85];
  window.OnixViewerCodeListMeta["BiblePurpose"] = { listNumber: 85, title: "Bible purpose" };
  window.OnixViewerCodeLists["BibleReferenceLocation"] = _lists[87];
  window.OnixViewerCodeListMeta["BibleReferenceLocation"] = { listNumber: 87, title: "Bible reference location" };
  window.OnixViewerCodeLists["BibleTextFeature"] = _lists[97];
  window.OnixViewerCodeListMeta["BibleTextFeature"] = { listNumber: 97, title: "Bible text feature" };
  window.OnixViewerCodeLists["BibleTextOrganization"] = _lists[86];
  window.OnixViewerCodeListMeta["BibleTextOrganization"] = { listNumber: 86, title: "Bible text organization" };
  window.OnixViewerCodeLists["BibleVersion"] = _lists[83];
  window.OnixViewerCodeListMeta["BibleVersion"] = { listNumber: 83, title: "Bible version" };
  window.OnixViewerCodeLists["CitedContentType"] = _lists[156];
  window.OnixViewerCodeListMeta["CitedContentType"] = { listNumber: 156, title: "Cited content type" };
  window.OnixViewerCodeLists["CollectionElementLevel"] = _lists[149];
  window.OnixViewerCodeListMeta["CollectionElementLevel"] = { listNumber: 149, title: "Title element level" };
  window.OnixViewerCodeLists["CollectionFrequency"] = _lists[259];
  window.OnixViewerCodeListMeta["CollectionFrequency"] = { listNumber: 259, title: "Collection frequency code" };
  window.OnixViewerCodeLists["CollectionIDType"] = _lists[13];
  window.OnixViewerCodeListMeta["CollectionIDType"] = { listNumber: 13, title: "Collection identifier type" };
  window.OnixViewerCodeLists["CollectionSequenceType"] = _lists[197];
  window.OnixViewerCodeListMeta["CollectionSequenceType"] = { listNumber: 197, title: "Collection sequence type" };
  window.OnixViewerCodeLists["CollectionType"] = _lists[148];
  window.OnixViewerCodeListMeta["CollectionType"] = { listNumber: 148, title: "Collection type" };
  window.OnixViewerCodeLists["ComplexitySchemeIdentifier"] = _lists[32];
  window.OnixViewerCodeListMeta["ComplexitySchemeIdentifier"] = { listNumber: 32, title: "Complexity scheme identifier" };
  window.OnixViewerCodeLists["ContentAudience"] = _lists[154];
  window.OnixViewerCodeListMeta["ContentAudience"] = { listNumber: 154, title: "Content audience" };
  window.OnixViewerCodeLists["ContentDateRole"] = _lists[155];
  window.OnixViewerCodeListMeta["ContentDateRole"] = { listNumber: 155, title: "Content date role" };
  window.OnixViewerCodeLists["ContributorDateRole"] = _lists[177];
  window.OnixViewerCodeListMeta["ContributorDateRole"] = { listNumber: 177, title: "Person / organization date role" };
  window.OnixViewerCodeLists["ContributorPlaceRelator"] = _lists[151];
  window.OnixViewerCodeListMeta["ContributorPlaceRelator"] = { listNumber: 151, title: "Contributor place relator" };
  window.OnixViewerCodeLists["ContributorRole"] = _lists[17];
  window.OnixViewerCodeListMeta["ContributorRole"] = { listNumber: 17, title: "Contributor role code" };
  window.OnixViewerCodeLists["CopyrightOwnerIDType"] = _lists[44];
  window.OnixViewerCodeListMeta["CopyrightOwnerIDType"] = { listNumber: 44, title: "Name identifier type" };
  window.OnixViewerCodeLists["CopyrightType"] = _lists[219];
  window.OnixViewerCodeListMeta["CopyrightType"] = { listNumber: 219, title: "Rights type" };
  window.OnixViewerCodeLists["CountryCode"] = _lists[91];
  window.OnixViewerCodeListMeta["CountryCode"] = { listNumber: 91, title: "Country – based on ISO 3166-1" };
  window.OnixViewerCodeLists["CountryOfManufacture"] = _lists[91];
  window.OnixViewerCodeListMeta["CountryOfManufacture"] = { listNumber: 91, title: "Country – based on ISO 3166-1" };
  window.OnixViewerCodeLists["CountryOfPublication"] = _lists[91];
  window.OnixViewerCodeListMeta["CountryOfPublication"] = { listNumber: 91, title: "Country – based on ISO 3166-1" };
  window.OnixViewerCodeLists["CurrencyCode"] = _lists[96];
  window.OnixViewerCodeListMeta["CurrencyCode"] = { listNumber: 96, title: "Currency code – based on ISO 4217" };
  window.OnixViewerCodeLists["DefaultCurrencyCode"] = _lists[96];
  window.OnixViewerCodeListMeta["DefaultCurrencyCode"] = { listNumber: 96, title: "Currency code – based on ISO 4217" };
  window.OnixViewerCodeLists["DefaultLanguageOfText"] = _lists[74];
  window.OnixViewerCodeListMeta["DefaultLanguageOfText"] = { listNumber: 74, title: "Language – based on ISO 639-2/B" };
  window.OnixViewerCodeLists["DefaultPriceType"] = _lists[58];
  window.OnixViewerCodeListMeta["DefaultPriceType"] = { listNumber: 58, title: "Price type" };
  window.OnixViewerCodeLists["DiscountCodeType"] = _lists[100];
  window.OnixViewerCodeListMeta["DiscountCodeType"] = { listNumber: 100, title: "Discount code type" };
  window.OnixViewerCodeLists["DiscountType"] = _lists[170];
  window.OnixViewerCodeListMeta["DiscountType"] = { listNumber: 170, title: "Discount type" };
  window.OnixViewerCodeLists["EditionType"] = _lists[21];
  window.OnixViewerCodeListMeta["EditionType"] = { listNumber: 21, title: "Edition type" };
  window.OnixViewerCodeLists["EpubLicenseDateRole"] = _lists[260];
  window.OnixViewerCodeListMeta["EpubLicenseDateRole"] = { listNumber: 260, title: "Epublication license date role" };
  window.OnixViewerCodeLists["EpubLicenseExpressionType"] = _lists[218];
  window.OnixViewerCodeListMeta["EpubLicenseExpressionType"] = { listNumber: 218, title: "License expression type" };
  window.OnixViewerCodeLists["EpubTechnicalProtection"] = _lists[144];
  window.OnixViewerCodeListMeta["EpubTechnicalProtection"] = { listNumber: 144, title: "E-publication technical protection" };
  window.OnixViewerCodeLists["EpubUsageStatus"] = _lists[146];
  window.OnixViewerCodeListMeta["EpubUsageStatus"] = { listNumber: 146, title: "Usage status" };
  window.OnixViewerCodeLists["EpubUsageType"] = _lists[145];
  window.OnixViewerCodeListMeta["EpubUsageType"] = { listNumber: 145, title: "Usage type" };
  window.OnixViewerCodeLists["EpubUsageUnit"] = _lists[147];
  window.OnixViewerCodeListMeta["EpubUsageUnit"] = { listNumber: 147, title: "Unit of usage" };
  window.OnixViewerCodeLists["EventIDType"] = _lists[244];
  window.OnixViewerCodeListMeta["EventIDType"] = { listNumber: 244, title: "Event identifier type" };
  window.OnixViewerCodeLists["EventRole"] = _lists[20];
  window.OnixViewerCodeListMeta["EventRole"] = { listNumber: 20, title: "Event role" };
  window.OnixViewerCodeLists["EventSponsorIDType"] = _lists[44];
  window.OnixViewerCodeListMeta["EventSponsorIDType"] = { listNumber: 44, title: "Name identifier type" };
  window.OnixViewerCodeLists["EventStatus"] = _lists[246];
  window.OnixViewerCodeListMeta["EventStatus"] = { listNumber: 246, title: "Event status" };
  window.OnixViewerCodeLists["EventType"] = _lists[245];
  window.OnixViewerCodeListMeta["EventType"] = { listNumber: 245, title: "Event type" };
  window.OnixViewerCodeLists["ExtentType"] = _lists[23];
  window.OnixViewerCodeListMeta["ExtentType"] = { listNumber: 23, title: "Extent type" };
  window.OnixViewerCodeLists["ExtentUnit"] = _lists[24];
  window.OnixViewerCodeListMeta["ExtentUnit"] = { listNumber: 24, title: "Extent unit" };
  window.OnixViewerCodeLists["FromLanguage"] = _lists[74];
  window.OnixViewerCodeListMeta["FromLanguage"] = { listNumber: 74, title: "Language – based on ISO 639-2/B" };
  window.OnixViewerCodeLists["FundingIDType"] = _lists[228];
  window.OnixViewerCodeListMeta["FundingIDType"] = { listNumber: 228, title: "Grant identifier type" };
  window.OnixViewerCodeLists["Illustrated"] = _lists[152];
  window.OnixViewerCodeListMeta["Illustrated"] = { listNumber: 152, title: "Illustrated / not illustrated" };
  window.OnixViewerCodeLists["ImprintIDType"] = _lists[44];
  window.OnixViewerCodeListMeta["ImprintIDType"] = { listNumber: 44, title: "Name identifier type" };
  window.OnixViewerCodeLists["InsertPointType"] = _lists[255];
  window.OnixViewerCodeListMeta["InsertPointType"] = { listNumber: 255, title: "Insert point type" };
  window.OnixViewerCodeLists["LanguageCode"] = _lists[74];
  window.OnixViewerCodeListMeta["LanguageCode"] = { listNumber: 74, title: "Language – based on ISO 639-2/B" };
  window.OnixViewerCodeLists["LanguageRole"] = _lists[22];
  window.OnixViewerCodeListMeta["LanguageRole"] = { listNumber: 22, title: "Language role" };
  window.OnixViewerCodeLists["LocationIDType"] = _lists[92];
  window.OnixViewerCodeListMeta["LocationIDType"] = { listNumber: 92, title: "Supplier identifier type" };
  window.OnixViewerCodeLists["MarketDateRole"] = _lists[163];
  window.OnixViewerCodeListMeta["MarketDateRole"] = { listNumber: 163, title: "Publishing date role" };
  window.OnixViewerCodeLists["MarketPublishingStatus"] = _lists[68];
  window.OnixViewerCodeListMeta["MarketPublishingStatus"] = { listNumber: 68, title: "Market publishing status" };
  window.OnixViewerCodeLists["MeasureType"] = _lists[48];
  window.OnixViewerCodeListMeta["MeasureType"] = { listNumber: 48, title: "Measure type" };
  window.OnixViewerCodeLists["MeasureUnitCode"] = _lists[50];
  window.OnixViewerCodeListMeta["MeasureUnitCode"] = { listNumber: 50, title: "Measure unit" };
  window.OnixViewerCodeLists["NameIDType"] = _lists[44];
  window.OnixViewerCodeListMeta["NameIDType"] = { listNumber: 44, title: "Name identifier type" };
  window.OnixViewerCodeLists["NameType"] = _lists[18];
  window.OnixViewerCodeListMeta["NameType"] = { listNumber: 18, title: "Person / organization name type" };
  window.OnixViewerCodeLists["NotificationType"] = _lists[1];
  window.OnixViewerCodeListMeta["NotificationType"] = { listNumber: 1, title: "Notification or update type" };
  window.OnixViewerCodeLists["OccurrenceDateRole"] = _lists[247];
  window.OnixViewerCodeListMeta["OccurrenceDateRole"] = { listNumber: 247, title: "Event occurrence date role" };
  window.OnixViewerCodeLists["PositionOnProduct"] = _lists[142];
  window.OnixViewerCodeListMeta["PositionOnProduct"] = { listNumber: 142, title: "Position on product" };
  window.OnixViewerCodeLists["PriceCodeType"] = _lists[179];
  window.OnixViewerCodeListMeta["PriceCodeType"] = { listNumber: 179, title: "Price code type" };
  window.OnixViewerCodeLists["PriceConditionQuantityType"] = _lists[168];
  window.OnixViewerCodeListMeta["PriceConditionQuantityType"] = { listNumber: 168, title: "Price condition quantity type" };
  window.OnixViewerCodeLists["PriceConditionType"] = _lists[167];
  window.OnixViewerCodeListMeta["PriceConditionType"] = { listNumber: 167, title: "Price condition type" };
  window.OnixViewerCodeLists["PriceConstraintStatus"] = _lists[146];
  window.OnixViewerCodeListMeta["PriceConstraintStatus"] = { listNumber: 146, title: "Usage status" };
  window.OnixViewerCodeLists["PriceConstraintType"] = _lists[230];
  window.OnixViewerCodeListMeta["PriceConstraintType"] = { listNumber: 230, title: "Price constraint type" };
  window.OnixViewerCodeLists["PriceConstraintUnit"] = _lists[147];
  window.OnixViewerCodeListMeta["PriceConstraintUnit"] = { listNumber: 147, title: "Unit of usage" };
  window.OnixViewerCodeLists["PriceDateRole"] = _lists[173];
  window.OnixViewerCodeListMeta["PriceDateRole"] = { listNumber: 173, title: "Price date role" };
  window.OnixViewerCodeLists["PriceIDType"] = _lists[217];
  window.OnixViewerCodeListMeta["PriceIDType"] = { listNumber: 217, title: "Price identifier type" };
  window.OnixViewerCodeLists["PricePer"] = _lists[60];
  window.OnixViewerCodeListMeta["PricePer"] = { listNumber: 60, title: "Unit of pricing" };
  window.OnixViewerCodeLists["PriceQualifier"] = _lists[59];
  window.OnixViewerCodeListMeta["PriceQualifier"] = { listNumber: 59, title: "Price type qualifier" };
  window.OnixViewerCodeLists["PriceStatus"] = _lists[61];
  window.OnixViewerCodeListMeta["PriceStatus"] = { listNumber: 61, title: "Price status" };
  window.OnixViewerCodeLists["PriceType"] = _lists[58];
  window.OnixViewerCodeListMeta["PriceType"] = { listNumber: 58, title: "Price type" };
  window.OnixViewerCodeLists["PrimaryContentType"] = _lists[81];
  window.OnixViewerCodeListMeta["PrimaryContentType"] = { listNumber: 81, title: "Product content type" };
  window.OnixViewerCodeLists["PrintedOnProduct"] = _lists[174];
  window.OnixViewerCodeListMeta["PrintedOnProduct"] = { listNumber: 174, title: "Printed on product" };
  window.OnixViewerCodeLists["PrizeCode"] = _lists[41];
  window.OnixViewerCodeListMeta["PrizeCode"] = { listNumber: 41, title: "Prize or award achievement" };
  window.OnixViewerCodeLists["PrizeCountry"] = _lists[91];
  window.OnixViewerCodeListMeta["PrizeCountry"] = { listNumber: 91, title: "Country – based on ISO 3166-1" };
  window.OnixViewerCodeLists["PrizeIDType"] = _lists[263];
  window.OnixViewerCodeListMeta["PrizeIDType"] = { listNumber: 263, title: "Prize identifier type" };
  window.OnixViewerCodeLists["PrizeRegion"] = _lists[49];
  window.OnixViewerCodeListMeta["PrizeRegion"] = { listNumber: 49, title: "Region – based on ISO 3166-2" };
  window.OnixViewerCodeLists["ProductAvailability"] = _lists[65];
  window.OnixViewerCodeListMeta["ProductAvailability"] = { listNumber: 65, title: "Product availability" };
  window.OnixViewerCodeLists["ProductClassificationType"] = _lists[9];
  window.OnixViewerCodeListMeta["ProductClassificationType"] = { listNumber: 9, title: "Product classification type" };
  window.OnixViewerCodeLists["ProductComposition"] = _lists[2];
  window.OnixViewerCodeListMeta["ProductComposition"] = { listNumber: 2, title: "Product composition" };
  window.OnixViewerCodeLists["ProductContactIDType"] = _lists[44];
  window.OnixViewerCodeListMeta["ProductContactIDType"] = { listNumber: 44, title: "Name identifier type" };
  window.OnixViewerCodeLists["ProductContactRole"] = _lists[198];
  window.OnixViewerCodeListMeta["ProductContactRole"] = { listNumber: 198, title: "Product contact role" };
  window.OnixViewerCodeLists["ProductContentType"] = _lists[81];
  window.OnixViewerCodeListMeta["ProductContentType"] = { listNumber: 81, title: "Product content type" };
  window.OnixViewerCodeLists["ProductForm"] = _lists[150];
  window.OnixViewerCodeListMeta["ProductForm"] = { listNumber: 150, title: "Product form" };
  window.OnixViewerCodeLists["ProductFormDetail"] = _lists[175];
  window.OnixViewerCodeListMeta["ProductFormDetail"] = { listNumber: 175, title: "Product form detail" };
  window.OnixViewerCodeLists["ProductFormFeatureType"] = _lists[79];
  window.OnixViewerCodeListMeta["ProductFormFeatureType"] = { listNumber: 79, title: "Product form feature type" };
  window.OnixViewerCodeLists["ProductIDType"] = _lists[5];
  window.OnixViewerCodeListMeta["ProductIDType"] = { listNumber: 5, title: "Product identifier type" };
  window.OnixViewerCodeLists["ProductPackaging"] = _lists[80];
  window.OnixViewerCodeListMeta["ProductPackaging"] = { listNumber: 80, title: "Product packaging type" };
  window.OnixViewerCodeLists["ProductRelationCode"] = _lists[51];
  window.OnixViewerCodeListMeta["ProductRelationCode"] = { listNumber: 51, title: "Product relation" };
  window.OnixViewerCodeLists["Proximity"] = _lists[215];
  window.OnixViewerCodeListMeta["Proximity"] = { listNumber: 215, title: "Proximity" };
  window.OnixViewerCodeLists["PublisherIDType"] = _lists[44];
  window.OnixViewerCodeListMeta["PublisherIDType"] = { listNumber: 44, title: "Name identifier type" };
  window.OnixViewerCodeLists["PublishingDateRole"] = _lists[163];
  window.OnixViewerCodeListMeta["PublishingDateRole"] = { listNumber: 163, title: "Publishing date role" };
  window.OnixViewerCodeLists["PublishingRole"] = _lists[45];
  window.OnixViewerCodeListMeta["PublishingRole"] = { listNumber: 45, title: "Publishing role" };
  window.OnixViewerCodeLists["PublishingStatus"] = _lists[64];
  window.OnixViewerCodeListMeta["PublishingStatus"] = { listNumber: 64, title: "Publishing status" };
  window.OnixViewerCodeLists["QuantityUnit"] = _lists[169];
  window.OnixViewerCodeListMeta["QuantityUnit"] = { listNumber: 169, title: "Quantity unit" };
  window.OnixViewerCodeLists["ROWSalesRightsType"] = _lists[46];
  window.OnixViewerCodeListMeta["ROWSalesRightsType"] = { listNumber: 46, title: "Sales rights type" };
  window.OnixViewerCodeLists["RecordSourceIDType"] = _lists[44];
  window.OnixViewerCodeListMeta["RecordSourceIDType"] = { listNumber: 44, title: "Name identifier type" };
  window.OnixViewerCodeLists["RecordSourceType"] = _lists[3];
  window.OnixViewerCodeListMeta["RecordSourceType"] = { listNumber: 3, title: "Record source type" };
  window.OnixViewerCodeLists["RegionCode"] = _lists[49];
  window.OnixViewerCodeListMeta["RegionCode"] = { listNumber: 49, title: "Region – based on ISO 3166-2" };
  window.OnixViewerCodeLists["ReligiousTextFeatureCode"] = _lists[90];
  window.OnixViewerCodeListMeta["ReligiousTextFeatureCode"] = { listNumber: 90, title: "Religious text feature" };
  window.OnixViewerCodeLists["ReligiousTextFeatureType"] = _lists[89];
  window.OnixViewerCodeListMeta["ReligiousTextFeatureType"] = { listNumber: 89, title: "Religious text feature type" };
  window.OnixViewerCodeLists["ResourceContentType"] = _lists[158];
  window.OnixViewerCodeListMeta["ResourceContentType"] = { listNumber: 158, title: "Resource content type" };
  window.OnixViewerCodeLists["ResourceFeatureType"] = _lists[160];
  window.OnixViewerCodeListMeta["ResourceFeatureType"] = { listNumber: 160, title: "Resource feature type" };
  window.OnixViewerCodeLists["ResourceFileDateRole"] = _lists[254];
  window.OnixViewerCodeListMeta["ResourceFileDateRole"] = { listNumber: 254, title: "Resource file date role code" };
  window.OnixViewerCodeLists["ResourceFileDetail"] = _lists[252];
  window.OnixViewerCodeListMeta["ResourceFileDetail"] = { listNumber: 252, title: "Resource file detail code" };
  window.OnixViewerCodeLists["ResourceFileFeatureType"] = _lists[253];
  window.OnixViewerCodeListMeta["ResourceFileFeatureType"] = { listNumber: 253, title: "Resource file feature type" };
  window.OnixViewerCodeLists["ResourceForm"] = _lists[161];
  window.OnixViewerCodeListMeta["ResourceForm"] = { listNumber: 161, title: "Resource form" };
  window.OnixViewerCodeLists["ResourceIDType"] = _lists[250];
  window.OnixViewerCodeListMeta["ResourceIDType"] = { listNumber: 250, title: "Resource identifier type" };
  window.OnixViewerCodeLists["ResourceMode"] = _lists[159];
  window.OnixViewerCodeListMeta["ResourceMode"] = { listNumber: 159, title: "Resource mode" };
  window.OnixViewerCodeLists["ResourceRole"] = _lists[251];
  window.OnixViewerCodeListMeta["ResourceRole"] = { listNumber: 251, title: "Resource role code" };
  window.OnixViewerCodeLists["ResourceVersionFeatureType"] = _lists[162];
  window.OnixViewerCodeListMeta["ResourceVersionFeatureType"] = { listNumber: 162, title: "Resource version feature type" };
  window.OnixViewerCodeLists["ReturnsCodeType"] = _lists[53];
  window.OnixViewerCodeListMeta["ReturnsCodeType"] = { listNumber: 53, title: "Returns conditions code type" };
  window.OnixViewerCodeLists["SalesOutletIDType"] = _lists[102];
  window.OnixViewerCodeListMeta["SalesOutletIDType"] = { listNumber: 102, title: "Sales outlet identifier type" };
  window.OnixViewerCodeLists["SalesRestrictionType"] = _lists[71];
  window.OnixViewerCodeListMeta["SalesRestrictionType"] = { listNumber: 71, title: "Sales restriction type" };
  window.OnixViewerCodeLists["SalesRightsType"] = _lists[46];
  window.OnixViewerCodeListMeta["SalesRightsType"] = { listNumber: 46, title: "Sales rights type" };
  window.OnixViewerCodeLists["ScriptCode"] = _lists[121];
  window.OnixViewerCodeListMeta["ScriptCode"] = { listNumber: 121, title: "Text script – based on ISO 15924" };
  window.OnixViewerCodeLists["SenderIDType"] = _lists[44];
  window.OnixViewerCodeListMeta["SenderIDType"] = { listNumber: 44, title: "Name identifier type" };
  window.OnixViewerCodeLists["SourceType"] = _lists[157];
  window.OnixViewerCodeListMeta["SourceType"] = { listNumber: 157, title: "Content source type" };
  window.OnixViewerCodeLists["SpecificationDetail"] = _lists[248];
  window.OnixViewerCodeListMeta["SpecificationDetail"] = { listNumber: 248, title: "Specification detail code" };
  window.OnixViewerCodeLists["SpecificationFeatureType"] = _lists[249];
  window.OnixViewerCodeListMeta["SpecificationFeatureType"] = { listNumber: 249, title: "Specification feature type" };
  window.OnixViewerCodeLists["StockQuantityCodeType"] = _lists[70];
  window.OnixViewerCodeListMeta["StockQuantityCodeType"] = { listNumber: 70, title: "Stock quantity code type" };
  window.OnixViewerCodeLists["StudyBibleType"] = _lists[84];
  window.OnixViewerCodeListMeta["StudyBibleType"] = { listNumber: 84, title: "Study Bible type" };
  window.OnixViewerCodeLists["SubjectDateRole"] = _lists[177];
  window.OnixViewerCodeListMeta["SubjectDateRole"] = { listNumber: 177, title: "Person / organization date role" };
  window.OnixViewerCodeLists["SubjectSchemeIdentifier"] = _lists[27];
  window.OnixViewerCodeListMeta["SubjectSchemeIdentifier"] = { listNumber: 27, title: "Subject scheme identifier" };
  window.OnixViewerCodeLists["SupplierCodeType"] = _lists[165];
  window.OnixViewerCodeListMeta["SupplierCodeType"] = { listNumber: 165, title: "Supplier own code type" };
  window.OnixViewerCodeLists["SupplierIDType"] = _lists[92];
  window.OnixViewerCodeListMeta["SupplierIDType"] = { listNumber: 92, title: "Supplier identifier type" };
  window.OnixViewerCodeLists["SupplierRole"] = _lists[93];
  window.OnixViewerCodeListMeta["SupplierRole"] = { listNumber: 93, title: "Supplier role" };
  window.OnixViewerCodeLists["SupplyContactIDType"] = _lists[44];
  window.OnixViewerCodeListMeta["SupplyContactIDType"] = { listNumber: 44, title: "Name identifier type" };
  window.OnixViewerCodeLists["SupplyContactRole"] = _lists[239];
  window.OnixViewerCodeListMeta["SupplyContactRole"] = { listNumber: 239, title: "Supply contact role" };
  window.OnixViewerCodeLists["SupplyDateRole"] = _lists[166];
  window.OnixViewerCodeListMeta["SupplyDateRole"] = { listNumber: 166, title: "Supply date role" };
  window.OnixViewerCodeLists["TaxRateCode"] = _lists[62];
  window.OnixViewerCodeListMeta["TaxRateCode"] = { listNumber: 62, title: "Tax rate type" };
  window.OnixViewerCodeLists["TaxType"] = _lists[171];
  window.OnixViewerCodeListMeta["TaxType"] = { listNumber: 171, title: "Tax type" };
  window.OnixViewerCodeLists["TextItemIDType"] = _lists[43];
  window.OnixViewerCodeListMeta["TextItemIDType"] = { listNumber: 43, title: "Text item identifier type" };
  window.OnixViewerCodeLists["TextItemType"] = _lists[42];
  window.OnixViewerCodeListMeta["TextItemType"] = { listNumber: 42, title: "Text item type" };
  window.OnixViewerCodeLists["TextType"] = _lists[153];
  window.OnixViewerCodeListMeta["TextType"] = { listNumber: 153, title: "Text type" };
  window.OnixViewerCodeLists["ThesisType"] = _lists[72];
  window.OnixViewerCodeListMeta["ThesisType"] = { listNumber: 72, title: "Thesis type" };
  window.OnixViewerCodeLists["TitleElementLevel"] = _lists[149];
  window.OnixViewerCodeListMeta["TitleElementLevel"] = { listNumber: 149, title: "Title element level" };
  window.OnixViewerCodeLists["TitleType"] = _lists[15];
  window.OnixViewerCodeListMeta["TitleType"] = { listNumber: 15, title: "Title type" };
  window.OnixViewerCodeLists["ToLanguage"] = _lists[74];
  window.OnixViewerCodeListMeta["ToLanguage"] = { listNumber: 74, title: "Language – based on ISO 639-2/B" };
  window.OnixViewerCodeLists["TradeCategory"] = _lists[12];
  window.OnixViewerCodeListMeta["TradeCategory"] = { listNumber: 12, title: "Trade category" };
  window.OnixViewerCodeLists["UnnamedPersons"] = _lists[19];
  window.OnixViewerCodeListMeta["UnnamedPersons"] = { listNumber: 19, title: "Unnamed person(s)" };
  window.OnixViewerCodeLists["UnpricedItemType"] = _lists[57];
  window.OnixViewerCodeListMeta["UnpricedItemType"] = { listNumber: 57, title: "Unpriced item type" };
  window.OnixViewerCodeLists["VelocityMetric"] = _lists[216];
  window.OnixViewerCodeListMeta["VelocityMetric"] = { listNumber: 216, title: "Velocity metric" };
  window.OnixViewerCodeLists["WebsiteRole"] = _lists[73];
  window.OnixViewerCodeListMeta["WebsiteRole"] = { listNumber: 73, title: "Website role" };
  window.OnixViewerCodeLists["WorkIDType"] = _lists[16];
  window.OnixViewerCodeListMeta["WorkIDType"] = { listNumber: 16, title: "Work identifier type" };
  window.OnixViewerCodeLists["WorkRelationCode"] = _lists[164];
  window.OnixViewerCodeListMeta["WorkRelationCode"] = { listNumber: 164, title: "Work relation" };

  // Expose every list by its EDItEUR number too, so consumers can look up
  // lists that are bound to attributes (textcase, dateformat) rather than
  // element names.
  window.OnixViewerCodeListsByNumber = _lists;

  // Titles by list number. OnixViewerCodeListMeta reaches a title only
  // through an element that binds the list, and 35 of the 165 lists are
  // bound to attributes instead (textcase, textformat, dateformat, …) —
  // so a finding about one of those had no name to print.
  window.OnixViewerCodeListTitles = {
    1: "Notification or update type",
    2: "Product composition",
    3: "Record source type",
    5: "Product identifier type",
    9: "Product classification type",
    12: "Trade category",
    13: "Collection identifier type",
    14: "Text case flag",
    15: "Title type",
    16: "Work identifier type",
    17: "Contributor role code",
    18: "Person / organization name type",
    19: "Unnamed person(s)",
    20: "Event role",
    21: "Edition type",
    22: "Language role",
    23: "Extent type",
    24: "Extent unit",
    25: "Illustration and other content type",
    27: "Subject scheme identifier",
    28: "Audience type",
    29: "Audience code type",
    30: "Audience range qualifier",
    31: "Audience range precision",
    32: "Complexity scheme identifier",
    34: "Text format",
    41: "Prize or award achievement",
    42: "Text item type",
    43: "Text item identifier type",
    44: "Name identifier type",
    45: "Publishing role",
    46: "Sales rights type",
    48: "Measure type",
    49: "Region – based on ISO 3166-2",
    50: "Measure unit",
    51: "Product relation",
    53: "Returns conditions code type",
    55: "Date format",
    57: "Unpriced item type",
    58: "Price type",
    59: "Price type qualifier",
    60: "Unit of pricing",
    61: "Price status",
    62: "Tax rate type",
    64: "Publishing status",
    65: "Product availability",
    66: "BISAC returnable indicator",
    68: "Market publishing status",
    69: "Agent role",
    70: "Stock quantity code type",
    71: "Sales restriction type",
    72: "Thesis type",
    73: "Website role",
    74: "Language – based on ISO 639-2/B",
    76: "DVD region",
    77: "US and Canada (excluding Québec) school or college grade",
    79: "Product form feature type",
    80: "Product packaging type",
    81: "Product content type",
    82: "Bible contents",
    83: "Bible version",
    84: "Study Bible type",
    85: "Bible purpose",
    86: "Bible text organization",
    87: "Bible reference location",
    89: "Religious text feature type",
    90: "Religious text feature",
    91: "Country – based on ISO 3166-1",
    92: "Supplier identifier type",
    93: "Supplier role",
    96: "Currency code – based on ISO 4217",
    97: "Bible text feature",
    98: "Binding or page edge color",
    99: "Special cover material",
    100: "Discount code type",
    102: "Sales outlet identifier type",
    121: "Text script – based on ISO 15924",
    139: "ONIX retail sales outlet IDs",
    141: "Barcode indicator",
    142: "Position on product",
    143: "US CPSIA or other international hazard warning type",
    144: "E-publication technical protection",
    145: "Usage type",
    146: "Usage status",
    147: "Unit of usage",
    148: "Collection type",
    149: "Title element level",
    150: "Product form",
    151: "Contributor place relator",
    152: "Illustrated / not illustrated",
    153: "Text type",
    154: "Content audience",
    155: "Content date role",
    156: "Cited content type",
    157: "Content source type",
    158: "Resource content type",
    159: "Resource mode",
    160: "Resource feature type",
    161: "Resource form",
    162: "Resource version feature type",
    163: "Publishing date role",
    164: "Work relation",
    165: "Supplier own code type",
    166: "Supply date role",
    167: "Price condition type",
    168: "Price condition quantity type",
    169: "Quantity unit",
    170: "Discount type",
    171: "Tax type",
    172: "Currency zone",
    173: "Price date role",
    174: "Printed on product",
    175: "Product form detail",
    176: "Operating system",
    177: "Person / organization date role",
    178: "Supporting resource file format",
    179: "Price code type",
    184: "EU General Product Safety Regulation and Toy Safety Directive hazard warning type",
    196: "E-publication Accessibility Details",
    197: "Collection sequence type",
    198: "Product contact role",
    203: "ONIX Adult Audience rating",
    204: "ONIX Returns conditions",
    215: "Proximity",
    216: "Velocity metric",
    217: "Price identifier type",
    218: "License expression type",
    219: "Rights type",
    220: "E-publication version number",
    221: "Message status",
    222: "Message status date role",
    223: "Status detail code type",
    224: "Status detail type severity",
    225: "Message / Record status detail",
    226: "Record status",
    227: "Chinese School Grade",
    228: "Grant identifier type",
    229: "Gender – based on ISO 5218",
    230: "Price constraint type",
    238: "Brazil educational level",
    239: "Supply contact role",
    240: "AV Item type code",
    241: "AV Item Identifier type",
    242: "Battery type and safety",
    243: "Dangerous goods regulations",
    244: "Event identifier type",
    245: "Event type",
    246: "Event status",
    247: "Event occurrence date role",
    248: "Specification detail code",
    249: "Specification feature type",
    250: "Resource identifier type",
    251: "Resource role code",
    252: "Resource file detail code",
    253: "Resource file feature type",
    254: "Resource file date role code",
    255: "Insert point type",
    256: "Resource feature code",
    257: "Paper color",
    258: "Special finish",
    259: "Collection frequency code",
    260: "Epublication license date role",
    261: "Status detail type severity (master files)",
    262: "Carbon / GHG emissions certification scheme",
    263: "Prize identifier type",
  };

  // Codes EDItEUR has withdrawn, by list number → code → the issue at
  // which each was deprecated. Still valid XML, but not to be sent.
  window.OnixViewerDeprecatedCodes = Object.create(null);
  window.OnixViewerDeprecatedCodes[3] = { "10": 71 };
  window.OnixViewerDeprecatedCodes[5] = { "02": 11, "05": 12 };
  window.OnixViewerDeprecatedCodes[16] = { "02": 11, "11": 70 };
  window.OnixViewerDeprecatedCodes[21] = { "ALT": 1 };
  window.OnixViewerDeprecatedCodes[27] = { "12": 69, "13": 69, "14": 69, "15": 69, "16": 69, "17": 69, "27": 18, "33": 60, "39": 28, "59": 59, "64": 58, "65": 58, "67": 58, "68": 58, "69": 58, "70": 58, "75": 61 };
  window.OnixViewerDeprecatedCodes[29] = { "12": 59, "13": 59, "14": 59, "19": 24, "20": 24, "25": 24 };
  window.OnixViewerDeprecatedCodes[30] = { "23": 8, "24": 8, "25": 8 };
  window.OnixViewerDeprecatedCodes[32] = { "01": 12, "02": 12 };
  window.OnixViewerDeprecatedCodes[43] = { "09": 70, "10": 70, "11": 70 };
  window.OnixViewerDeprecatedCodes[44] = { "02": 10, "17": 18, "20": 18, "37": 60 };
  window.OnixViewerDeprecatedCodes[46] = { "07": 24, "08": 24 };
  window.OnixViewerDeprecatedCodes[49] = { "CN-11": 44, "CN-12": 44, "CN-13": 44, "CN-14": 44, "CN-15": 44, "CN-21": 44, "CN-22": 44, "CN-23": 44, "CN-31": 44, "CN-32": 44, "CN-33": 44, "CN-34": 44, "CN-35": 44, "CN-36": 44, "CN-37": 44, "CN-41": 44, "CN-42": 44, "CN-43": 44, "CN-44": 44, "CN-45": 44, "CN-46": 44, "CN-50": 44, "CN-51": 44, "CN-52": 44, "CN-53": 44, "CN-54": 44, "CN-61": 44, "CN-62": 44, "CN-63": 44, "CN-64": 44, "CN-65": 44, "CN-71": 44, "CN-91": 44, "CN-92": 44, "GB-CHA": 13, "GB-EWS": 40, "GB-IOM": 13, "ECZ": 35 };
  window.OnixViewerDeprecatedCodes[64] = { "12": 15 };
  window.OnixViewerDeprecatedCodes[70] = { "02": 48 };
  window.OnixViewerDeprecatedCodes[74] = { "scr": 10, "mol": 10, "scc": 10 };
  window.OnixViewerDeprecatedCodes[91] = { "AN": 13, "CS": 7, "YU": 4 };
  window.OnixViewerDeprecatedCodes[92] = { "02": 11 };
  window.OnixViewerDeprecatedCodes[93] = { "05": 5 };
  window.OnixViewerDeprecatedCodes[96] = { "AFA": 4, "ANG": 69, "ATS": 42, "BEF": 42, "BGL": 4, "BGN": 72, "BYR": 44, "CSD": 17, "CUC": 69, "CYP": 42, "DEM": 42, "EEK": 42, "ESP": 42, "FIM": 42, "FRF": 42, "GHC": 17, "GRD": 42, "HRK": 60, "IEP": 42, "ITL": 42, "LTL": 42, "LUF": 42, "LVL": 42, "MRO": 42, "MTL": 42, "NLG": 42, "PTE": 42, "ROL": 17, "RUR": 4, "SIT": 42, "SKK": 42, "SLL": 58, "SRG": 4, "STD": 42, "TMM": 17, "TPE": 7, "TRL": 17, "VEB": 17, "VEF": 43, "YUM": 4, "ZMK": 27, "ZWD": 17, "ZWL": 66 };
  window.OnixViewerDeprecatedCodes[139] = { "BDL": 47, "BIL": 47, "GOS": 47, "BRB": 47, "BRD": 47, "BRT": 47, "COP": 47, "ECH": 47, "ELS": 67, "ELK": 67, "HST": 47, "JAM": 64, "KNO": 47, "LEG": 72, "MYB": 47, "OYS": 47, "PST": 47, "POK": 58, "RDB": 47, "RMB": 47, "SPL": 64, "SUP": 64, "TXR": 47, "VRG": 47 };
  window.OnixViewerDeprecatedCodes[158] = { "53": 68, "99": 24 };
  window.OnixViewerDeprecatedCodes[167] = { "10": 34 };
  window.OnixViewerDeprecatedCodes[172] = { "EUR": 12 };
  window.OnixViewerDeprecatedCodes[175] = { "A201": 13, "A207": 13, "D317": 51 };
  window.OnixViewerDeprecatedCodes[176] = { "05": 11, "06": 11 };

  // Short tag → reference name, from the short-tag schema. onix.js layers
  // its own additions (Acknowledgement tags, ONIX 2.1 legacy tags) on top.
  window.OnixViewerShortTags = Object.create(null);
  window.OnixViewerShortTags["a001"] = "RecordReference";
  window.OnixViewerShortTags["a002"] = "NotificationType";
  window.OnixViewerShortTags["a194"] = "RecordSourceType";
  window.OnixViewerShortTags["a197"] = "RecordSourceName";
  window.OnixViewerShortTags["a199"] = "DeletionText";
  window.OnixViewerShortTags["addressee"] = "Addressee";
  window.OnixViewerShortTags["addresseeidentifier"] = "AddresseeIdentifier";
  window.OnixViewerShortTags["affiliationidentifier"] = "AffiliationIdentifier";
  window.OnixViewerShortTags["agentidentifier"] = "AgentIdentifier";
  window.OnixViewerShortTags["alternativename"] = "AlternativeName";
  window.OnixViewerShortTags["ancillarycontent"] = "AncillaryContent";
  window.OnixViewerShortTags["audience"] = "Audience";
  window.OnixViewerShortTags["audiencerange"] = "AudienceRange";
  window.OnixViewerShortTags["avitem"] = "AVItem";
  window.OnixViewerShortTags["avitemidentifier"] = "AVItemIdentifier";
  window.OnixViewerShortTags["b012"] = "ProductForm";
  window.OnixViewerShortTags["b014"] = "ProductFormDescription";
  window.OnixViewerShortTags["b020"] = "YearOfAnnual";
  window.OnixViewerShortTags["b029"] = "Subtitle";
  window.OnixViewerShortTags["b030"] = "TitlePrefix";
  window.OnixViewerShortTags["b031"] = "TitleWithoutPrefix";
  window.OnixViewerShortTags["b034"] = "SequenceNumber";
  window.OnixViewerShortTags["b035"] = "ContributorRole";
  window.OnixViewerShortTags["b036"] = "PersonName";
  window.OnixViewerShortTags["b037"] = "PersonNameInverted";
  window.OnixViewerShortTags["b038"] = "TitlesBeforeNames";
  window.OnixViewerShortTags["b039"] = "NamesBeforeKey";
  window.OnixViewerShortTags["b040"] = "KeyNames";
  window.OnixViewerShortTags["b041"] = "NamesAfterKey";
  window.OnixViewerShortTags["b042"] = "LettersAfterNames";
  window.OnixViewerShortTags["b043"] = "TitlesAfterNames";
  window.OnixViewerShortTags["b044"] = "BiographicalNote";
  window.OnixViewerShortTags["b045"] = "ProfessionalPosition";
  window.OnixViewerShortTags["b046"] = "Affiliation";
  window.OnixViewerShortTags["b047"] = "CorporateName";
  window.OnixViewerShortTags["b048"] = "ContributorDescription";
  window.OnixViewerShortTags["b049"] = "ContributorStatement";
  window.OnixViewerShortTags["b051"] = "ConferenceRole";
  window.OnixViewerShortTags["b052"] = "ConferenceName";
  window.OnixViewerShortTags["b053"] = "ConferenceNumber";
  window.OnixViewerShortTags["b054"] = "ConferenceDate";
  window.OnixViewerShortTags["b055"] = "ConferencePlace";
  window.OnixViewerShortTags["b057"] = "EditionNumber";
  window.OnixViewerShortTags["b058"] = "EditionStatement";
  window.OnixViewerShortTags["b061"] = "NumberOfPages";
  window.OnixViewerShortTags["b062"] = "IllustrationsNote";
  window.OnixViewerShortTags["b063"] = "MapScale";
  window.OnixViewerShortTags["b067"] = "SubjectSchemeIdentifier";
  window.OnixViewerShortTags["b068"] = "SubjectSchemeVersion";
  window.OnixViewerShortTags["b069"] = "SubjectCode";
  window.OnixViewerShortTags["b070"] = "SubjectHeadingText";
  window.OnixViewerShortTags["b073"] = "AudienceCode";
  window.OnixViewerShortTags["b074"] = "AudienceRangeQualifier";
  window.OnixViewerShortTags["b075"] = "AudienceRangePrecision";
  window.OnixViewerShortTags["b076"] = "AudienceRangeValue";
  window.OnixViewerShortTags["b077"] = "ComplexitySchemeIdentifier";
  window.OnixViewerShortTags["b078"] = "ComplexityCode";
  window.OnixViewerShortTags["b079"] = "ImprintName";
  window.OnixViewerShortTags["b081"] = "PublisherName";
  window.OnixViewerShortTags["b083"] = "CountryOfPublication";
  window.OnixViewerShortTags["b087"] = "CopyrightYear";
  window.OnixViewerShortTags["b089"] = "SalesRightsType";
  window.OnixViewerShortTags["b125"] = "NumberOfIllustrations";
  window.OnixViewerShortTags["b171"] = "SubjectSchemeName";
  window.OnixViewerShortTags["b201"] = "WorkIDType";
  window.OnixViewerShortTags["b202"] = "TitleType";
  window.OnixViewerShortTags["b203"] = "TitleText";
  window.OnixViewerShortTags["b204"] = "AudienceCodeType";
  window.OnixViewerShortTags["b205"] = "AudienceCodeTypeName";
  window.OnixViewerShortTags["b206"] = "AudienceCodeValue";
  window.OnixViewerShortTags["b207"] = "AudienceDescription";
  window.OnixViewerShortTags["b209"] = "CityOfPublication";
  window.OnixViewerShortTags["b217"] = "EditionVersionNumber";
  window.OnixViewerShortTags["b218"] = "ExtentType";
  window.OnixViewerShortTags["b219"] = "ExtentValue";
  window.OnixViewerShortTags["b220"] = "ExtentUnit";
  window.OnixViewerShortTags["b221"] = "ProductIDType";
  window.OnixViewerShortTags["b225"] = "ProductPackaging";
  window.OnixViewerShortTags["b233"] = "IDTypeName";
  window.OnixViewerShortTags["b244"] = "IDValue";
  window.OnixViewerShortTags["b247"] = "PrefixToKey";
  window.OnixViewerShortTags["b248"] = "SuffixToKey";
  window.OnixViewerShortTags["b249"] = "UnnamedPersons";
  window.OnixViewerShortTags["b251"] = "CountryCode";
  window.OnixViewerShortTags["b252"] = "LanguageCode";
  window.OnixViewerShortTags["b253"] = "LanguageRole";
  window.OnixViewerShortTags["b257"] = "Number";
  window.OnixViewerShortTags["b274"] = "ProductClassificationType";
  window.OnixViewerShortTags["b275"] = "ProductClassificationCode";
  window.OnixViewerShortTags["b284"] = "LevelSequenceNumber";
  window.OnixViewerShortTags["b285"] = "TextItemIDType";
  window.OnixViewerShortTags["b286"] = "FirstPageNumber";
  window.OnixViewerShortTags["b287"] = "LastPageNumber";
  window.OnixViewerShortTags["b288"] = "ComponentTypeName";
  window.OnixViewerShortTags["b289"] = "ComponentNumber";
  window.OnixViewerShortTags["b290"] = "TextItemType";
  window.OnixViewerShortTags["b291"] = "PublishingRole";
  window.OnixViewerShortTags["b294"] = "WebsiteDescription";
  window.OnixViewerShortTags["b295"] = "WebsiteLink";
  window.OnixViewerShortTags["b306"] = "Date";
  window.OnixViewerShortTags["b324"] = "StartDate";
  window.OnixViewerShortTags["b325"] = "EndDate";
  window.OnixViewerShortTags["b333"] = "ProductFormDetail";
  window.OnixViewerShortTags["b334"] = "ProductFormFeatureType";
  window.OnixViewerShortTags["b335"] = "ProductFormFeatureValue";
  window.OnixViewerShortTags["b336"] = "ProductFormFeatureDescription";
  window.OnixViewerShortTags["b337"] = "Percent";
  window.OnixViewerShortTags["b341"] = "ConferenceAcronym";
  window.OnixViewerShortTags["b342"] = "ConferenceTheme";
  window.OnixViewerShortTags["b352"] = "BibleContents";
  window.OnixViewerShortTags["b353"] = "BibleVersion";
  window.OnixViewerShortTags["b354"] = "BiblePurpose";
  window.OnixViewerShortTags["b355"] = "BibleTextOrganization";
  window.OnixViewerShortTags["b356"] = "BibleReferenceLocation";
  window.OnixViewerShortTags["b357"] = "BibleTextFeature";
  window.OnixViewerShortTags["b358"] = "ReligiousTextFeatureType";
  window.OnixViewerShortTags["b359"] = "ReligiousTextFeatureCode";
  window.OnixViewerShortTags["b360"] = "ReligiousTextFeatureDescription";
  window.OnixViewerShortTags["b367"] = "WebsiteRole";
  window.OnixViewerShortTags["b368"] = "ThesisType";
  window.OnixViewerShortTags["b369"] = "ThesisPresentedTo";
  window.OnixViewerShortTags["b370"] = "ThesisYear";
  window.OnixViewerShortTags["b374"] = "TextSourceCorporate";
  window.OnixViewerShortTags["b376"] = "ReligiousTextIdentifier";
  window.OnixViewerShortTags["b381"] = "SalesRestrictionType";
  window.OnixViewerShortTags["b382"] = "SalesOutletName";
  window.OnixViewerShortTags["b384"] = "TradeCategory";
  window.OnixViewerShortTags["b385"] = "ProductContentType";
  window.OnixViewerShortTags["b389"] = "StudyBibleType";
  window.OnixViewerShortTags["b391"] = "ConferenceSponsorIDType";
  window.OnixViewerShortTags["b392"] = "CopyrightOwnerIDType";
  window.OnixViewerShortTags["b393"] = "SalesOutletIDType";
  window.OnixViewerShortTags["b394"] = "PublishingStatus";
  window.OnixViewerShortTags["b395"] = "PublishingStatusNote";
  window.OnixViewerShortTags["b398"] = "RegionCode";
  window.OnixViewerShortTags["barcode"] = "Barcode";
  window.OnixViewerShortTags["batchbonus"] = "BatchBonus";
  window.OnixViewerShortTags["bible"] = "Bible";
  window.OnixViewerShortTags["bodymanifest"] = "BodyManifest";
  window.OnixViewerShortTags["bodyresource"] = "BodyResource";
  window.OnixViewerShortTags["c094"] = "Measurement";
  window.OnixViewerShortTags["c095"] = "MeasureUnitCode";
  window.OnixViewerShortTags["citedcontent"] = "CitedContent";
  window.OnixViewerShortTags["collateraldetail"] = "CollateralDetail";
  window.OnixViewerShortTags["collection"] = "Collection";
  window.OnixViewerShortTags["collectionidentifier"] = "CollectionIdentifier";
  window.OnixViewerShortTags["collectionsequence"] = "CollectionSequence";
  window.OnixViewerShortTags["comparisonproductprice"] = "ComparisonProductPrice";
  window.OnixViewerShortTags["complexity"] = "Complexity";
  window.OnixViewerShortTags["conference"] = "Conference";
  window.OnixViewerShortTags["conferencesponsor"] = "ConferenceSponsor";
  window.OnixViewerShortTags["conferencesponsoridentifier"] = "ConferenceSponsorIdentifier";
  window.OnixViewerShortTags["contentdate"] = "ContentDate";
  window.OnixViewerShortTags["contentdetail"] = "ContentDetail";
  window.OnixViewerShortTags["contentitem"] = "ContentItem";
  window.OnixViewerShortTags["contributor"] = "Contributor";
  window.OnixViewerShortTags["contributordate"] = "ContributorDate";
  window.OnixViewerShortTags["contributorplace"] = "ContributorPlace";
  window.OnixViewerShortTags["contributorreference"] = "ContributorReference";
  window.OnixViewerShortTags["copyrightowner"] = "CopyrightOwner";
  window.OnixViewerShortTags["copyrightowneridentifier"] = "CopyrightOwnerIdentifier";
  window.OnixViewerShortTags["copyrightstatement"] = "CopyrightStatement";
  window.OnixViewerShortTags["covermanifest"] = "CoverManifest";
  window.OnixViewerShortTags["coverresource"] = "CoverResource";
  window.OnixViewerShortTags["d104"] = "Text";
  window.OnixViewerShortTags["d107"] = "TextAuthor";
  window.OnixViewerShortTags["descriptivedetail"] = "DescriptiveDetail";
  window.OnixViewerShortTags["discount"] = "Discount";
  window.OnixViewerShortTags["discountcoded"] = "DiscountCoded";
  window.OnixViewerShortTags["epublicense"] = "EpubLicense";
  window.OnixViewerShortTags["epublicensedate"] = "EpubLicenseDate";
  window.OnixViewerShortTags["epublicenseexpression"] = "EpubLicenseExpression";
  window.OnixViewerShortTags["epubusageconstraint"] = "EpubUsageConstraint";
  window.OnixViewerShortTags["epubusagelimit"] = "EpubUsageLimit";
  window.OnixViewerShortTags["event"] = "Event";
  window.OnixViewerShortTags["eventidentifier"] = "EventIdentifier";
  window.OnixViewerShortTags["eventoccurrence"] = "EventOccurrence";
  window.OnixViewerShortTags["eventsponsor"] = "EventSponsor";
  window.OnixViewerShortTags["eventsponsoridentifier"] = "EventSponsorIdentifier";
  window.OnixViewerShortTags["extent"] = "Extent";
  window.OnixViewerShortTags["funding"] = "Funding";
  window.OnixViewerShortTags["fundingidentifier"] = "FundingIdentifier";
  window.OnixViewerShortTags["g126"] = "PrizeName";
  window.OnixViewerShortTags["g127"] = "PrizeYear";
  window.OnixViewerShortTags["g128"] = "PrizeCountry";
  window.OnixViewerShortTags["g129"] = "PrizeCode";
  window.OnixViewerShortTags["g343"] = "PrizeJury";
  window.OnixViewerShortTags["header"] = "Header";
  window.OnixViewerShortTags["imprint"] = "Imprint";
  window.OnixViewerShortTags["imprintidentifier"] = "ImprintIdentifier";
  window.OnixViewerShortTags["insertmanifest"] = "InsertManifest";
  window.OnixViewerShortTags["insertpoint"] = "InsertPoint";
  window.OnixViewerShortTags["insertresource"] = "InsertResource";
  window.OnixViewerShortTags["j137"] = "SupplierName";
  window.OnixViewerShortTags["j144"] = "OrderTime";
  window.OnixViewerShortTags["j145"] = "PackQuantity";
  window.OnixViewerShortTags["j151"] = "PriceAmount";
  window.OnixViewerShortTags["j152"] = "CurrencyCode";
  window.OnixViewerShortTags["j192"] = "UnpricedItemType";
  window.OnixViewerShortTags["j239"] = "PricePer";
  window.OnixViewerShortTags["j260"] = "DateFormat";
  window.OnixViewerShortTags["j261"] = "PriceQualifier";
  window.OnixViewerShortTags["j262"] = "PriceTypeDescription";
  window.OnixViewerShortTags["j263"] = "MinimumOrderQuantity";
  window.OnixViewerShortTags["j264"] = "BatchQuantity";
  window.OnixViewerShortTags["j265"] = "FreeQuantity";
  window.OnixViewerShortTags["j266"] = "PriceStatus";
  window.OnixViewerShortTags["j267"] = "DiscountPercent";
  window.OnixViewerShortTags["j268"] = "ReturnsCodeType";
  window.OnixViewerShortTags["j269"] = "ReturnsCode";
  window.OnixViewerShortTags["j270"] = "TelephoneNumber";
  window.OnixViewerShortTags["j271"] = "FaxNumber";
  window.OnixViewerShortTags["j272"] = "EmailAddress";
  window.OnixViewerShortTags["j292"] = "SupplierRole";
  window.OnixViewerShortTags["j293"] = "StockQuantityCodeType";
  window.OnixViewerShortTags["j296"] = "StockQuantityCodeTypeName";
  window.OnixViewerShortTags["j297"] = "StockQuantityCode";
  window.OnixViewerShortTags["j302"] = "ExpectedDate";
  window.OnixViewerShortTags["j345"] = "SupplierIDType";
  window.OnixViewerShortTags["j349"] = "LocationName";
  window.OnixViewerShortTags["j350"] = "OnHand";
  window.OnixViewerShortTags["j351"] = "OnOrder";
  window.OnixViewerShortTags["j363"] = "DiscountCodeType";
  window.OnixViewerShortTags["j364"] = "DiscountCode";
  window.OnixViewerShortTags["j365"] = "ReissueDate";
  window.OnixViewerShortTags["j366"] = "ReissueDescription";
  window.OnixViewerShortTags["j375"] = "CBO";
  window.OnixViewerShortTags["j377"] = "LocationIDType";
  window.OnixViewerShortTags["j378"] = "DiscountCodeTypeName";
  window.OnixViewerShortTags["j396"] = "ProductAvailability";
  window.OnixViewerShortTags["j400"] = "AgentIDType";
  window.OnixViewerShortTags["j401"] = "AgentName";
  window.OnixViewerShortTags["j402"] = "AgentRole";
  window.OnixViewerShortTags["j407"] = "MarketPublishingStatus";
  window.OnixViewerShortTags["j408"] = "MarketDateRole";
  window.OnixViewerShortTags["k165"] = "PromotionCampaign";
  window.OnixViewerShortTags["k166"] = "PromotionContact";
  window.OnixViewerShortTags["k167"] = "InitialPrintRun";
  window.OnixViewerShortTags["k168"] = "CopiesSold";
  window.OnixViewerShortTags["k169"] = "BookClubAdoption";
  window.OnixViewerShortTags["k309"] = "ReprintDetail";
  window.OnixViewerShortTags["language"] = "Language";
  window.OnixViewerShortTags["locationidentifier"] = "LocationIdentifier";
  window.OnixViewerShortTags["m180"] = "MessageNumber";
  window.OnixViewerShortTags["m181"] = "MessageRepeat";
  window.OnixViewerShortTags["m183"] = "MessageNote";
  window.OnixViewerShortTags["m184"] = "DefaultLanguageOfText";
  window.OnixViewerShortTags["m186"] = "DefaultCurrencyCode";
  window.OnixViewerShortTags["m379"] = "SenderIDType";
  window.OnixViewerShortTags["m380"] = "AddresseeIDType";
  window.OnixViewerShortTags["market"] = "Market";
  window.OnixViewerShortTags["marketdate"] = "MarketDate";
  window.OnixViewerShortTags["marketpublishingdetail"] = "MarketPublishingDetail";
  window.OnixViewerShortTags["measure"] = "Measure";
  window.OnixViewerShortTags["n339"] = "NoContributor";
  window.OnixViewerShortTags["n386"] = "NoEdition";
  window.OnixViewerShortTags["nameassubject"] = "NameAsSubject";
  window.OnixViewerShortTags["nameidentifier"] = "NameIdentifier";
  window.OnixViewerShortTags["newsupplier"] = "NewSupplier";
  window.OnixViewerShortTags["occurrencedate"] = "OccurrenceDate";
  window.OnixViewerShortTags["onixmessage"] = "ONIXMessage";
  window.OnixViewerShortTags["onorderdetail"] = "OnOrderDetail";
  window.OnixViewerShortTags["pagerun"] = "PageRun";
  window.OnixViewerShortTags["price"] = "Price";
  window.OnixViewerShortTags["pricecoded"] = "PriceCoded";
  window.OnixViewerShortTags["pricecondition"] = "PriceCondition";
  window.OnixViewerShortTags["priceconditionquantity"] = "PriceConditionQuantity";
  window.OnixViewerShortTags["priceconstraint"] = "PriceConstraint";
  window.OnixViewerShortTags["priceconstraintlimit"] = "PriceConstraintLimit";
  window.OnixViewerShortTags["pricedate"] = "PriceDate";
  window.OnixViewerShortTags["priceidentifier"] = "PriceIdentifier";
  window.OnixViewerShortTags["prize"] = "Prize";
  window.OnixViewerShortTags["prizeidentifier"] = "PrizeIdentifier";
  window.OnixViewerShortTags["product"] = "Product";
  window.OnixViewerShortTags["productclassification"] = "ProductClassification";
  window.OnixViewerShortTags["productcontact"] = "ProductContact";
  window.OnixViewerShortTags["productcontactidentifier"] = "ProductContactIdentifier";
  window.OnixViewerShortTags["productformfeature"] = "ProductFormFeature";
  window.OnixViewerShortTags["productidentifier"] = "ProductIdentifier";
  window.OnixViewerShortTags["productiondetail"] = "ProductionDetail";
  window.OnixViewerShortTags["productionmanifest"] = "ProductionManifest";
  window.OnixViewerShortTags["productpart"] = "ProductPart";
  window.OnixViewerShortTags["productsupply"] = "ProductSupply";
  window.OnixViewerShortTags["professionalaffiliation"] = "ProfessionalAffiliation";
  window.OnixViewerShortTags["promotionalevent"] = "PromotionalEvent";
  window.OnixViewerShortTags["promotiondetail"] = "PromotionDetail";
  window.OnixViewerShortTags["publisher"] = "Publisher";
  window.OnixViewerShortTags["publisheridentifier"] = "PublisherIdentifier";
  window.OnixViewerShortTags["publisherrepresentative"] = "PublisherRepresentative";
  window.OnixViewerShortTags["publishingdate"] = "PublishingDate";
  window.OnixViewerShortTags["publishingdetail"] = "PublishingDetail";
  window.OnixViewerShortTags["recordsourceidentifier"] = "RecordSourceIdentifier";
  window.OnixViewerShortTags["reissue"] = "Reissue";
  window.OnixViewerShortTags["relatedmaterial"] = "RelatedMaterial";
  window.OnixViewerShortTags["relatedproduct"] = "RelatedProduct";
  window.OnixViewerShortTags["relatedwork"] = "RelatedWork";
  window.OnixViewerShortTags["religioustext"] = "ReligiousText";
  window.OnixViewerShortTags["religioustextfeature"] = "ReligiousTextFeature";
  window.OnixViewerShortTags["resourcefeature"] = "ResourceFeature";
  window.OnixViewerShortTags["resourcefiledate"] = "ResourceFileDate";
  window.OnixViewerShortTags["resourcefilefeature"] = "ResourceFileFeature";
  window.OnixViewerShortTags["resourceidentifier"] = "ResourceIdentifier";
  window.OnixViewerShortTags["resourceversion"] = "ResourceVersion";
  window.OnixViewerShortTags["resourceversionfeature"] = "ResourceVersionFeature";
  window.OnixViewerShortTags["returnsconditions"] = "ReturnsConditions";
  window.OnixViewerShortTags["reviewrating"] = "ReviewRating";
  window.OnixViewerShortTags["salesoutlet"] = "SalesOutlet";
  window.OnixViewerShortTags["salesoutletidentifier"] = "SalesOutletIdentifier";
  window.OnixViewerShortTags["salesrestriction"] = "SalesRestriction";
  window.OnixViewerShortTags["salesrights"] = "SalesRights";
  window.OnixViewerShortTags["sender"] = "Sender";
  window.OnixViewerShortTags["senderidentifier"] = "SenderIdentifier";
  window.OnixViewerShortTags["specificationbundlename"] = "SpecificationBundleName";
  window.OnixViewerShortTags["specificationfeature"] = "SpecificationFeature";
  window.OnixViewerShortTags["stock"] = "Stock";
  window.OnixViewerShortTags["stockquantitycoded"] = "StockQuantityCoded";
  window.OnixViewerShortTags["subject"] = "Subject";
  window.OnixViewerShortTags["subjectdate"] = "SubjectDate";
  window.OnixViewerShortTags["supplementmanifest"] = "SupplementManifest";
  window.OnixViewerShortTags["supplier"] = "Supplier";
  window.OnixViewerShortTags["supplieridentifier"] = "SupplierIdentifier";
  window.OnixViewerShortTags["supplierowncoding"] = "SupplierOwnCoding";
  window.OnixViewerShortTags["supplycontact"] = "SupplyContact";
  window.OnixViewerShortTags["supplycontactidentifier"] = "SupplyContactIdentifier";
  window.OnixViewerShortTags["supplydate"] = "SupplyDate";
  window.OnixViewerShortTags["supplydetail"] = "SupplyDetail";
  window.OnixViewerShortTags["supportingresource"] = "SupportingResource";
  window.OnixViewerShortTags["tax"] = "Tax";
  window.OnixViewerShortTags["territory"] = "Territory";
  window.OnixViewerShortTags["textcontent"] = "TextContent";
  window.OnixViewerShortTags["textitem"] = "TextItem";
  window.OnixViewerShortTags["textitemidentifier"] = "TextItemIdentifier";
  window.OnixViewerShortTags["textsource"] = "TextSource";
  window.OnixViewerShortTags["timerun"] = "TimeRun";
  window.OnixViewerShortTags["titledetail"] = "TitleDetail";
  window.OnixViewerShortTags["titleelement"] = "TitleElement";
  window.OnixViewerShortTags["velocity"] = "Velocity";
  window.OnixViewerShortTags["website"] = "Website";
  window.OnixViewerShortTags["workidentifier"] = "WorkIdentifier";
  window.OnixViewerShortTags["x298"] = "SenderName";
  window.OnixViewerShortTags["x299"] = "ContactName";
  window.OnixViewerShortTags["x300"] = "AddresseeName";
  window.OnixViewerShortTags["x301"] = "PrintedOnProduct";
  window.OnixViewerShortTags["x307"] = "SentDateTime";
  window.OnixViewerShortTags["x310"] = "DefaultPriceType";
  window.OnixViewerShortTags["x311"] = "RecordSourceIDType";
  window.OnixViewerShortTags["x312"] = "BarcodeType";
  window.OnixViewerShortTags["x313"] = "PositionOnProduct";
  window.OnixViewerShortTags["x314"] = "ProductComposition";
  window.OnixViewerShortTags["x315"] = "MeasureType";
  window.OnixViewerShortTags["x316"] = "CountryOfManufacture";
  window.OnixViewerShortTags["x317"] = "EpubTechnicalProtection";
  window.OnixViewerShortTags["x318"] = "EpubUsageType";
  window.OnixViewerShortTags["x319"] = "EpubUsageStatus";
  window.OnixViewerShortTags["x320"] = "Quantity";
  window.OnixViewerShortTags["x321"] = "EpubUsageUnit";
  window.OnixViewerShortTags["x322"] = "NumberOfItemsOfThisForm";
  window.OnixViewerShortTags["x323"] = "NumberOfCopies";
  window.OnixViewerShortTags["x329"] = "CollectionType";
  window.OnixViewerShortTags["x330"] = "SourceName";
  window.OnixViewerShortTags["x344"] = "CollectionIDType";
  window.OnixViewerShortTags["x406"] = "MarketPublishingStatusNote";
  window.OnixViewerShortTags["x409"] = "TitleElementLevel";
  window.OnixViewerShortTags["x410"] = "PartNumber";
  window.OnixViewerShortTags["x411"] = "NoCollection";
  window.OnixViewerShortTags["x412"] = "FromLanguage";
  window.OnixViewerShortTags["x413"] = "ToLanguage";
  window.OnixViewerShortTags["x414"] = "NameType";
  window.OnixViewerShortTags["x415"] = "NameIDType";
  window.OnixViewerShortTags["x416"] = "PrimaryContentType";
  window.OnixViewerShortTags["x417"] = "ContributorDateRole";
  window.OnixViewerShortTags["x418"] = "ContributorPlaceRelator";
  window.OnixViewerShortTags["x419"] = "EditionType";
  window.OnixViewerShortTags["x420"] = "ScriptCode";
  window.OnixViewerShortTags["x421"] = "ExtentValueRoman";
  window.OnixViewerShortTags["x422"] = "Illustrated";
  window.OnixViewerShortTags["x423"] = "AncillaryContentType";
  window.OnixViewerShortTags["x424"] = "AncillaryContentDescription";
  window.OnixViewerShortTags["x425"] = "MainSubject";
  window.OnixViewerShortTags["x426"] = "TextType";
  window.OnixViewerShortTags["x427"] = "ContentAudience";
  window.OnixViewerShortTags["x428"] = "SourceTitle";
  window.OnixViewerShortTags["x429"] = "ContentDateRole";
  window.OnixViewerShortTags["x430"] = "CitedContentType";
  window.OnixViewerShortTags["x431"] = "SourceType";
  window.OnixViewerShortTags["x432"] = "ListName";
  window.OnixViewerShortTags["x433"] = "PositionOnList";
  window.OnixViewerShortTags["x434"] = "CitationNote";
  window.OnixViewerShortTags["x435"] = "ResourceLink";
  window.OnixViewerShortTags["x436"] = "ResourceContentType";
  window.OnixViewerShortTags["x437"] = "ResourceMode";
  window.OnixViewerShortTags["x438"] = "ResourceFeatureType";
  window.OnixViewerShortTags["x439"] = "FeatureValue";
  window.OnixViewerShortTags["x440"] = "FeatureNote";
  window.OnixViewerShortTags["x441"] = "ResourceForm";
  window.OnixViewerShortTags["x442"] = "ResourceVersionFeatureType";
  window.OnixViewerShortTags["x443"] = "CorporateNameInverted";
  window.OnixViewerShortTags["x445"] = "ImprintIDType";
  window.OnixViewerShortTags["x446"] = "LatestReprintNumber";
  window.OnixViewerShortTags["x447"] = "PublisherIDType";
  window.OnixViewerShortTags["x448"] = "PublishingDateRole";
  window.OnixViewerShortTags["x449"] = "CountriesIncluded";
  window.OnixViewerShortTags["x450"] = "RegionsIncluded";
  window.OnixViewerShortTags["x451"] = "CountriesExcluded";
  window.OnixViewerShortTags["x452"] = "RegionsExcluded";
  window.OnixViewerShortTags["x453"] = "SalesRestrictionNote";
  window.OnixViewerShortTags["x454"] = "WorkRelationCode";
  window.OnixViewerShortTags["x455"] = "ProductRelationCode";
  window.OnixViewerShortTags["x456"] = "ROWSalesRightsType";
  window.OnixViewerShortTags["x457"] = "PrimaryPart";
  window.OnixViewerShortTags["x458"] = "SupplierCodeType";
  window.OnixViewerShortTags["x459"] = "SupplierCodeValue";
  window.OnixViewerShortTags["x460"] = "ReturnsCodeTypeName";
  window.OnixViewerShortTags["x461"] = "SupplyDateRole";
  window.OnixViewerShortTags["x462"] = "PriceType";
  window.OnixViewerShortTags["x463"] = "PriceConditionType";
  window.OnixViewerShortTags["x464"] = "PriceConditionQuantityType";
  window.OnixViewerShortTags["x465"] = "PriceCodeType";
  window.OnixViewerShortTags["x466"] = "QuantityUnit";
  window.OnixViewerShortTags["x467"] = "DiscountType";
  window.OnixViewerShortTags["x468"] = "PriceCode";
  window.OnixViewerShortTags["x469"] = "DiscountAmount";
  window.OnixViewerShortTags["x470"] = "TaxType";
  window.OnixViewerShortTags["x471"] = "TaxRateCode";
  window.OnixViewerShortTags["x472"] = "TaxRatePercent";
  window.OnixViewerShortTags["x473"] = "TaxableAmount";
  window.OnixViewerShortTags["x474"] = "TaxAmount";
  window.OnixViewerShortTags["x475"] = "CurrencyZone";
  window.OnixViewerShortTags["x476"] = "PriceDateRole";
  window.OnixViewerShortTags["x477"] = "PriceCodeTypeName";
  window.OnixViewerShortTags["x478"] = "TitleStatement";
  window.OnixViewerShortTags["x479"] = "CollectionSequenceType";
  window.OnixViewerShortTags["x480"] = "CollectionSequenceTypeName";
  window.OnixViewerShortTags["x481"] = "CollectionSequenceNumber";
  window.OnixViewerShortTags["x482"] = "ProductContactRole";
  window.OnixViewerShortTags["x483"] = "ProductContactIDType";
  window.OnixViewerShortTags["x484"] = "ProductContactName";
  window.OnixViewerShortTags["x501"] = "NoPrefix";
  window.OnixViewerShortTags["x502"] = "Proximity";
  window.OnixViewerShortTags["x503"] = "PrizeStatement";
  window.OnixViewerShortTags["x504"] = "VelocityMetric";
  window.OnixViewerShortTags["x505"] = "Rate";
  window.OnixViewerShortTags["x506"] = "PriceIDType";
  window.OnixViewerShortTags["x507"] = "NoProduct";
  window.OnixViewerShortTags["x508"] = "EpubLicenseExpressionType";
  window.OnixViewerShortTags["x509"] = "EpubLicenseExpressionTypeName";
  window.OnixViewerShortTags["x510"] = "EpubLicenseExpressionLink";
  window.OnixViewerShortTags["x511"] = "EpubLicenseName";
  window.OnixViewerShortTags["x512"] = "CopyrightType";
  window.OnixViewerShortTags["x513"] = "SupplierCodeTypeName";
  window.OnixViewerShortTags["x514"] = "ToQuantity";
  window.OnixViewerShortTags["x515"] = "EventRole";
  window.OnixViewerShortTags["x516"] = "EventName";
  window.OnixViewerShortTags["x517"] = "EventAcronym";
  window.OnixViewerShortTags["x518"] = "EventNumber";
  window.OnixViewerShortTags["x519"] = "EventTheme";
  window.OnixViewerShortTags["x520"] = "EventDate";
  window.OnixViewerShortTags["x521"] = "EventPlace";
  window.OnixViewerShortTags["x522"] = "EventSponsorIDType";
  window.OnixViewerShortTags["x523"] = "FundingIDType";
  window.OnixViewerShortTags["x524"] = "Gender";
  window.OnixViewerShortTags["x525"] = "Rating";
  window.OnixViewerShortTags["x526"] = "RatingLimit";
  window.OnixViewerShortTags["x527"] = "RatingUnits";
  window.OnixViewerShortTags["x528"] = "ReturnsNote";
  window.OnixViewerShortTags["x529"] = "PriceConstraintType";
  window.OnixViewerShortTags["x530"] = "PriceConstraintStatus";
  window.OnixViewerShortTags["x531"] = "PriceConstraintUnit";
  window.OnixViewerShortTags["x532"] = "OrderQuantityMinimum";
  window.OnixViewerShortTags["x533"] = "OrderQuantityMultiple";
  window.OnixViewerShortTags["x534"] = "SubjectDateRole";
  window.OnixViewerShortTags["x535"] = "PricePartDescription";
  window.OnixViewerShortTags["x536"] = "Reserved";
  window.OnixViewerShortTags["x537"] = "SupplyContactRole";
  window.OnixViewerShortTags["x538"] = "SupplyContactIDType";
  window.OnixViewerShortTags["x539"] = "SupplyContactName";
  window.OnixViewerShortTags["x540"] = "AVItemType";
  window.OnixViewerShortTags["x541"] = "AVItemIDType";
  window.OnixViewerShortTags["x542"] = "StartTime";
  window.OnixViewerShortTags["x543"] = "EndTime";
  window.OnixViewerShortTags["x544"] = "AVDuration";
  window.OnixViewerShortTags["x545"] = "PalletQuantity";
  window.OnixViewerShortTags["x546"] = "TaxExempt";
  window.OnixViewerShortTags["x547"] = "EventIDType";
  window.OnixViewerShortTags["x548"] = "EventType";
  window.OnixViewerShortTags["x549"] = "EventStatus";
  window.OnixViewerShortTags["x550"] = "EventDescription";
  window.OnixViewerShortTags["x551"] = "VenueName";
  window.OnixViewerShortTags["x552"] = "StreetAddress";
  window.OnixViewerShortTags["x553"] = "VenueNote";
  window.OnixViewerShortTags["x554"] = "OccurrenceDateRole";
  window.OnixViewerShortTags["x555"] = "ProductClassificationTypeName";
  window.OnixViewerShortTags["x556"] = "PrizeRegion";
  window.OnixViewerShortTags["x557"] = "TextSourceDescription";
  window.OnixViewerShortTags["x558"] = "SpecificationBundleNameTypeName";
  window.OnixViewerShortTags["x559"] = "SpecificationBundleNameValue";
  window.OnixViewerShortTags["x560"] = "SpecificationDetail";
  window.OnixViewerShortTags["x561"] = "SpecificationFeatureType";
  window.OnixViewerShortTags["x562"] = "SpecificationFeatureValue";
  window.OnixViewerShortTags["x563"] = "SpecificationFeatureDescription";
  window.OnixViewerShortTags["x564"] = "SpecificationDescription";
  window.OnixViewerShortTags["x565"] = "ResourceIDType";
  window.OnixViewerShortTags["x566"] = "ResourceRole";
  window.OnixViewerShortTags["x567"] = "ResourceFileDetail";
  window.OnixViewerShortTags["x568"] = "ResourceFileFeatureType";
  window.OnixViewerShortTags["x569"] = "ResourceFileFeatureValue";
  window.OnixViewerShortTags["x570"] = "ResourceFileFeatureDescription";
  window.OnixViewerShortTags["x571"] = "ResourceFileDescription";
  window.OnixViewerShortTags["x572"] = "ResourceFileLink";
  window.OnixViewerShortTags["x573"] = "ResourceFileDateRole";
  window.OnixViewerShortTags["x574"] = "InsertPointType";
  window.OnixViewerShortTags["x575"] = "InsertPointValue";
  window.OnixViewerShortTags["x576"] = "ResourceFileContentDescription";
  window.OnixViewerShortTags["x577"] = "NoResource";
  window.OnixViewerShortTags["x578"] = "AudienceHeadingText";
  window.OnixViewerShortTags["x579"] = "NoSupplement";
  window.OnixViewerShortTags["x580"] = "AffiliationIDType";
  window.OnixViewerShortTags["x581"] = "TextSourceLink";
  window.OnixViewerShortTags["x582"] = "CollectionFrequency";
  window.OnixViewerShortTags["x583"] = "MainAudience";
  window.OnixViewerShortTags["x584"] = "AwardingBody";
  window.OnixViewerShortTags["x585"] = "EpubLicenseDateRole";
  window.OnixViewerShortTags["x586"] = "CollectionElementLevel";
  window.OnixViewerShortTags["x587"] = "MarketReference";
  window.OnixViewerShortTags["x588"] = "CopyrightStatementText";
  window.OnixViewerShortTags["x589"] = "PrizeIDType";
  window.OnixViewerShortTags["x590"] = "PostalCode";
  window.OnixViewerShortTags["x591"] = "SubjectDescription";
  window.OnixViewerShortTags["x592"] = "PublisherNameInverted";
  window.OnixViewerShortTags["x593"] = "ImprintNameInverted";

  window.OnixViewerCodeListSchema = {"version":"3.1","issue":74,"releaseDate":null};
})();
